...
interface TradeDetail {
  ...
  timestamp: number;
  /**
   * Fee actually charged for THIS fill, in the order's `feeCurrency`.
   *
   * Optional because it is only present on the copy stored in the order owner's
   * own `trades` JSON — never on the copy published to the public trade tape —
   * and because rows written before this field existed do not carry it. Clients
   * that need a fee for a legacy row must pro-rate the order-level fee by
   * `amount / order.amount` (the same ratio the matching engine settles on);
   * pro-rating by "filled so far" instead inflates every partial fill.
   */
  fee?: number;
}
