  ...
  implements depositGatewayAttributes
{
  ...
  /**
   * Read one of the JSON fee/limit columns, which may hold EITHER a scalar OR a
   * per-currency map ({"USD": 1.5, ...}) — the admin gateway editor writes both
   * shapes. Values are coerced with Number() because a hand-edited row (or a
   * form post) can store the entry as a string, and a string here propagates
   * into `(amount * pct) / 100 + fixedFee`, which then CONCATENATES instead of
   * adding. Non-numeric entries resolve to null so callers apply their default.
   */
  private currencyScopedNumber(
    raw: unknown,
    currency?: string
  ): number | null {
    let value: unknown = raw;
    if (value !== null && typeof value === "object" && currency) {
      value = (value as Record<string, unknown>)[currency.toUpperCase()];
    }
  ...
  public getFixedFee(currency?: string): number {
    return this.currencyScopedNumber(this.fixedFee, currency) ?? 0;
  }

  public getPercentageFee(currency?: string): number {
    return this.currencyScopedNumber(this.percentageFee, currency) ?? 0;
  }

  public getMinAmount(currency?: string): number {
    return this.currencyScopedNumber(this.minAmount, currency) ?? 0;
  }

  public getMaxAmount(currency?: string): number | null {
    return this.currencyScopedNumber(this.maxAmount, currency);
  }
}
