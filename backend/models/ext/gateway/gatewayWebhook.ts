    ...
    return gatewayWebhook.init(
      {
        ...
        payload: {
          ...
          get(this: any) {
             ...
             * authentication.
             */
            const value = this.getDataValue("payload") as unknown;
        ...
        /* Written by the delivery attempt, not at creation, because the HMAC
           covers a timestamp only decided at send time — and a retry re-signs
           with a fresh one, since the receiver rejects timestamps older than
           five minutes. So a row legitimately has no signature until its first
           attempt, and this is nullable to say so rather than storing a value
           that never went out on the wire. */
        signature: {
