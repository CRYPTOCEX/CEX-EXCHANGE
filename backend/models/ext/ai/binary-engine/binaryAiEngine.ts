 ...
 * - Platform edge = (1 - w) - w * payoutRate, where w is the user win rate and payoutRate
 *   is the advertised payout as a fraction. At w = 0.35 and the default 72% payout that
 *   is 0.65 - 0.35 * 0.72 = 29.8% of turnover. (An earlier note here claimed "65% profit
 *   at 35% user wins", which ignored the payout entirely and overstated the edge by more
 *   than double.) Break-even sits at w = 1 / (1 + payoutRate) — 58.1% for a 72% payout.
 *
 * Related Models:
  ...
  implements binaryAiEngineAttributes
{
  ...
  cooldownWinRateReduction!: number;
  /** Enable cooldown after a win streak */
  enableStreakCooldown!: boolean;
  /** Consecutive wins that trigger a streak cooldown */
  streakThreshold!: number;
  /** Streak cooldown duration in minutes */
  streakCooldownDuration!: number;
  ...
  // NOTE: there is deliberately no `payoutMultiplier` here. What a winner is paid
  // comes from the binary settings' per-type `profitPercentage`
  // (admin/finance/binary/settings) and is stamped on the order row at placement;
  // the engine never had a say in it. The column that used to sit here only fed
  // A/B-test and cohort profit PROJECTIONS, so an operator editing it changed a
  // report and nothing else. Those reports now sum the realised
  // binaryAiEnginePosition.platformProfit instead.
  ...
  // ML Configuration
  mlModelWeights?: Record<string, any>;
  /** Opt-in: auto-apply the ML-recommended win rate (bounded + clamped) */
  enableMlAutoApply!: boolean;
  enableWhaleAlerts?: boolean;
    ...
    return binaryAiEngine.init(
      {
        ...
        marketMakerId: {
          ...
          validate: {
            ...
            isUUID: { args: ANY_UUID_VERSION, msg: "marketMakerId: Must be a valid UUID" },
          },
        },
        status: {
        ...
        correlationConfig: {
           ...
           */
          get(this: binaryAiEngine) {
            const value = this.getDataValue("correlationConfig") as unknown;
        ...
        allowedOrderTypes: {
          ...
          /**
           * THE ONE JSON COLUMN ON THIS MODEL THAT HAD NO GETTER.
           *
           * A `DataTypes.JSON` column arrives PARSED on MySQL and as a RAW
           * STRING on MariaDB, and this value is read as an array in three
           * places that all degrade silently on the string:
           *
           *   - the admin engine page tests membership with
           *     `currentTypes.includes(id)`. On a string that is
           *     `String.prototype.includes` — a SUBSTRING match — so the
           *     checkboxes render correctly by accident, and then unchecking one
           *     calls `.filter` on a string and throws inside the click handler,
           *     while checking one spreads the string into twelve single
           *     characters and PUTs them.
           *   - `BinaryAiEngine.loadEngine` does `allowedOrderTypes || [...]`;
           *     a string is truthy, so the running engine's config holds a
           *     string too.
           *
           * Parsed defensively rather than with a bare `JSON.parse`: on MySQL
           * the value is already an array, and parsing it again throws.
           * Mirrors `readJsonColumn` in the addon's `utils/json-column.ts`,
           * which models cannot import.
           */
          get() {
            const value = this.getDataValue("allowedOrderTypes") as any;
            if (Array.isArray(value)) return value;
            if (typeof value === "string") {
              try {
                const parsed = JSON.parse(value);
                return Array.isArray(parsed) ? parsed : ["RISE_FALL"];
              } catch {
                return ["RISE_FALL"];
              }
            }
        ...
        mlModelWeights: {
           ...
           */
          get(this: binaryAiEngine) {
            const value = this.getDataValue("mlModelWeights") as unknown;
            if (value == null) return null;
            if (typeof value === "string") {
              try {
                return JSON.parse(value);
  ...
  public static associate(models: any) {
    ...
    // Engine has many correlation history records
    if (models.binaryAiEngineCorrelationHistory) {
      binaryAiEngine.hasMany(models.binaryAiEngineCorrelationHistory, {
        foreignKey: "engineId",
        as: "correlationHistory",
        onDelete: "CASCADE",
      });
    }
  }
