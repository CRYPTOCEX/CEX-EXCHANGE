...
export interface binaryAiEnginePositionAttributes {
  ...
  settledAt: Date | null;
  /** Actual settlement close price used to resolve the order */
  closePrice: number | null;
  /** Platform profit realized on this position (LOSS: +stake, WIN: -payout, DRAW: 0) */
  platformProfit: number;
  wasManipulated: boolean;
  manipulationDetails: Record<string, any> | null;
  /**
   * The A/B test that governed this position, if one was RUNNING when the engine
   * analysed it. Attribution is per POSITION and not per user: a user is enrolled
   * the first time the engine sees an order of theirs, so crediting an arm with
   * everything that user has ever traded would count pre-test history on both
   * sides. Null on every position placed outside a test.
   */
  abTestId: string | null;
  /** The arm in force for this position. Null when abTestId is null. */
  ...
  implements binaryAiEnginePositionAttributes
{
  ...
  /** Actual settlement close price used to resolve the order */
  closePrice!: number | null;
  /** Platform profit realized on this position */
  platformProfit!: number;
  /** Whether price was manipulated for this position */
  wasManipulated!: boolean;
  /** Details of any manipulation applied */
  manipulationDetails!: Record<string, any> | null;
  /** A/B test that governed this position (null outside a test) */
  abTestId!: string | null;
  /** A/B arm in force for this position (null outside a test) */
  abVariant!: "CONTROL" | "TREATMENT" | null;
    ...
    return binaryAiEnginePosition.init(
      {
        ...
        binaryOrderId: {
          ...
          validate: {
            ...
            isUUID: { args: ANY_UUID_VERSION, msg: "binaryOrderId: Must be a valid UUID" },
          },
        },
        userId: {
          type: DataTypes.UUID,
        ...
        manipulationDetails: {
           ...
           */
          get(this: binaryAiEnginePosition) {
            const value = this.getDataValue("manipulationDetails") as unknown;
        ...
        // Nullable and unconstrained on purpose: a test row can be deleted while
        // its positions remain, and losing the settled history of an experiment
        // would be worse than carrying a dangling id.
        abTestId: {
          type: DataTypes.UUID,
          allowNull: true,
        },
      ...
      },
      {
        ...
        indexes: [
          ...
          { fields: ["userTier"] },
          { fields: ["isDemo"] },
          // The exact shape calculateVariantMetrics queries on.
