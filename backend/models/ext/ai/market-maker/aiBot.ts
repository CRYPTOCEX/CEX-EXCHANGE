    ...
    return aiBot.init(
      {
        ...
        /**
         * When this bot's PER-TRADE ledger starts.
         *
         * `totalRealizedPnL` is a lifetime accumulator that has been correct
         * since the addon shipped, but `ai_bot_real_trades` — the only source of
         * the daily / weekly / monthly split — had no writer until v6.1.0. The
         * two therefore cover different spans, and there is nothing to backfill
         * from: the individual fills were never captured.
         *
         * Stamped once, by the ecosystem matcher on the first fill it records.
         * The analytics endpoint takes MIN() of it across a market's bots and
         * labels the period figures "history begins <date>", so an all-time
         * figure sitting beside three small ones reads as a short ledger rather
         * than as a collapse in performance.
         */
        firstRealTradeAt: {
          type: DataTypes.DATE,
          allowNull: true,
        ...
        /**
         * When a COOLDOWN status expires and the bot returns to ACTIVE.
         *
         * COOLDOWN was previously a one-way door: the admin UI could post it (with a
         * cooldownMinutes value the route only ever interpolated into a log string) but
         * no column recorded a duration and no code path ever moved a bot back to
         * ACTIVE. A cooled-down bot stayed out of the active set until an operator
         * noticed and clicked Activate by hand.
         */
        cooldownUntil: {
          type: DataTypes.DATE,
          allowNull: true,
