  ...
  implements nftListingAttributes
{
  ...
  endedAt?: Date;
  /**
   * Set when the settlement cron gave up on an ended auction and left it for
   * manual review (it won with a bid but has no auction contract holding the
   * money). The driving query excludes flagged rows, otherwise the same
   * unsettleable auctions are re-selected on every tick and eventually fill the
   * whole batch window, starving auctions that CAN settle.
   */
  settlementBlockedAt?: Date;
  views?: number;
  likes?: number;
  metadata?: any;
  createdAt?: Date;
    ...
    return nftListing.init(
      {
        ...
        currentBid: {
          ...
          // Hand back a number so the arithmetic means what it reads as.
          get(this: any) {
            const value = this.getDataValue("currentBid");
            return value === null || value === undefined ? value : Number(value);
          },
          allowNull: true,
          validate: {
        ...
        startingBid: {
          ...
          // Hand back a number so the arithmetic means what it reads as.
          get(this: any) {
            const value = this.getDataValue("startingBid");
            return value === null || value === undefined ? value : Number(value);
          },
          allowNull: true,
          validate: {
        ...
        reservePrice: {
          ...
          // Hand back a number so the arithmetic means what it reads as.
          get(this: any) {
            const value = this.getDataValue("reservePrice");
            return value === null || value === undefined ? value : Number(value);
          },
          allowNull: true,
          validate: {
        ...
        minBidIncrement: {
          ...
          // Hand back a number so the arithmetic means what it reads as.
          get(this: any) {
            const value = this.getDataValue("minBidIncrement");
            return value === null || value === undefined ? value : Number(value);
          },
          allowNull: true,
          validate: {
        ...
        buyNowPrice: {
          ...
          // Hand back a number so the arithmetic means what it reads as.
          get(this: any) {
            const value = this.getDataValue("buyNowPrice");
            return value === null || value === undefined ? value : Number(value);
          },
          allowNull: true,
          validate: {
