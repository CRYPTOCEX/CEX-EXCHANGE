...
export interface copyTradingLeaderStatsAttributes {
  ...
  // on-demand using stats-calculator.ts - no longer stored in DB. The
  // startEquity/endEquity/highEquity/lowEquity columns were removed too — they
  // were only ever written as 0 and read by nothing (drawdown is computed from
  // the profit curve in stats-calculator/calculations, not stored equity).
    ...
    return copyTradingLeaderStats.init(
      {
        ...
        // Note: Derived metrics (dailyWinRate, dailyRoi, maxDrawdown) and the
        // startEquity/endEquity/highEquity/lowEquity columns removed - stats are
