    ...
    return tradingBotOrder.init(
      {
        ...
        botId: {
          ...
          validate: {
            ...
            isUUID: { args: ANY_UUID_VERSION, msg: "botId: Must be a valid UUID" },
          },
        },
        userId: {
          type: DataTypes.UUID,
        ...
        type: {
          // MARKET belongs here even though a market order does not rest.
          //
          // A live market order is placed with `placeEcosystemOrder`, which
          // ENQUEUES into the matching engine rather than crossing the book
          // inline — so it fills asynchronously and has to be tracked and
          // reconciled exactly like a limit order. Without a row for it, a market
          // order whose fill arrived after the request returned would move the
          // user's funds with nothing on our side recording that it existed.
          type: DataTypes.ENUM("MARKET", "LIMIT", "STOP_LIMIT"),
          allowNull: false,
          validate: {
            isIn: {
              args: [["MARKET", "LIMIT", "STOP_LIMIT"]],
              msg: "type: Must be MARKET, LIMIT or STOP_LIMIT",
            },
          },
        },
        status: {
      ...
      },
      {
        ...
        indexes: [
          ...
          {
            // The bot's hot path, not a convenience: getWorkingOrders,
            // computeUsedAmount and hasWorkingOrderAt each filter on exactly
            // (botId, status) several times every 5s for every running bot.
            // botId leads because it is the selective column — a bot owns a
            // handful of orders where a status owns the whole table — and the
            // status leg keeps the scan off the FILLED/CANCELLED history.
            // tradingBotOrderBotIdIdx stays: it backs the botId foreign key.
            name: "tradingBotOrderBotStatusIdx",
            using: "BTREE",
