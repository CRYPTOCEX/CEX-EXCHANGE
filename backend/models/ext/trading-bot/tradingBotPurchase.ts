    ...
    return tradingBotPurchase.init(
      {
        ...
        strategyId: {
          ...
          validate: {
            ...
            isUUID: { args: ANY_UUID_VERSION, msg: "strategyId: Must be a valid UUID" },
          },
        },
        sellerId: {
          type: DataTypes.UUID,
        ...
        /*
         * Guarded getter — see the note in tradingBotStrategy.ts. This one is
         * the frozen copy of what the buyer actually paid for, and it is what a
         * dispute is settled against, so serving it as a JSON STRING on MariaDB
         * (and an object on prod MySQL) meant the two installs disagreed about
         * what a purchase even contained. The live driver caught it comparing a
         * snapshot to the config it was taken from and finding two values that
         * printed identically and were not equal.
         */
        strategySnapshot: {
          type: DataTypes.JSON,
          allowNull: false,
          get(this: tradingBotPurchase) {
            const value = this.getDataValue("strategySnapshot") as unknown;
