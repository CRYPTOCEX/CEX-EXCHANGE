...
export default class fxOrder
  extends Model<fxOrderAttributes, fxOrderCreationAttributes>
  implements fxOrderAttributes
{
  id!: string;
  userId!: string;
  accountId!: string;
  ...
  externalFillPrice?: string;
  externalFilledAt?: Date;
  ...
  public static initModel(sequelize: Sequelize.Sequelize): typeof fxOrder {
    return fxOrder.init(
      {
        id: {
          type: DataTypes.UUID,
        ...
        routing: {
          ...
          comment:
            "Execution routing decided at placement — INTERNAL (B-book desk) or EXTERNAL (A-book hedge at a broker)",
        },
        executionProviderId: {
          type: DataTypes.UUID,
          allowNull: true,
        ...
        reservedMargin: {
          ...
          allowNull: true,
          comment:
            "Margin + commission held while status=ROUTING — counted into every outbound-money gate; released on terminal status",
        },
        externalRef: {
