...
import { generatePayoutId } from "../gateway";
...
async function processMerchantBalancePayout(
  ...
  try {
    ...
    try {
      ...
      logError(
        `gatewayPayout - notification for merchant ${merchant.id}`,
        notifErr,
        __filename
      );
    }
    ...
    logError(
      `processMerchantBalancePayout - merchant ${merchant.id}`,
      error,
      __filename
    );
    return false;
...
export async function processGatewayPayouts() {
  ...
  try {
    ...
    logError("processGatewayPayouts", error, __filename);
