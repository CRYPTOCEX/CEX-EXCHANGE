...
export default async (data: { query?: any; user?: any }) => {
  ...
  try {
    ...
    const summaryData = await models.p2pTrade.findAll({
      where,
      attributes: [
        [models.sequelize.fn("SUM", models.sequelize.col("totalAmount")), "totalVolume"],
        [models.sequelize.fn("COUNT", models.sequelize.literal("CASE WHEN status = 'COMPLETED' THEN 1 END")), "completedCount"],
