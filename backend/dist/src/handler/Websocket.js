...
function processSubscriptionChange(ws, message) {
    ...
    const clientId = ws.user.id;
    const route = ws.path;
    // Security: if the subscription payload includes a userId, it MUST match the connected client's id.
    // Without this, a client (especially on unauthenticated WS routes where ws.user.id is taken from
    // a query param) can subscribe to another user's private stream by sending their userId.
    const payloadUserId = (_b = (_a = message.payload) === null || _a === void 0 ? void 0 : _a.userId) !== null && _b !== void 0 ? _b : null;
    if (payloadUserId && payloadUserId !== clientId) {
        console_1.logger.warn("WS", `Rejected cross-user subscription on ${route}: client=${clientId} payload.userId=${payloadUserId}`);
        throw (0, error_1.createError)({ statusCode: 403, message: "Cannot subscribe to another user's stream" });
    }
