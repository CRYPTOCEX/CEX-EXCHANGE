...
const funding = require("./exchange-funding");
...
function resolveCcxtId(provider) {
    return CCXT_ID_MAP[provider] || provider;
}
function isUsableSecret(value) {
    ...
    if (/^x+$/i.test(s))
        return false;
    if (s.toLowerCase().includes("xxxxxxxx"))
        return false;
    return true;
}
function createCcxtClient(provider, config) {
    const id = resolveCcxtId(provider);
    const Ctor = (ccxt.pro && ccxt.pro[id]) || ccxt[id];
    if (!Ctor) {
        throw new Error(`No CCXT adapter for ${provider} (resolved ${id}). Public CCXT 4.x rest/pro class is missing.`);
    }
    ...
    return funding.installFundingCapabilities(new Ctor(Object.assign({}, config, { options })));
}
...
class ExchangeManager {
    ...
    async initializeExchange(provider, retries = 3, ctx) {
        ...
        if (this.exchangeCache.has(provider)) {
            ...
            const cached = this.exchangeCache.get(provider);
            this.maybeResyncTime(cached);
            return cached;
        }
        const now = Date.now();
        ...
        const needsPassphrase = ["kucoin", "okx", "bitget", "coinbase"].includes(provider);
        if (!isUsableSecret(apiKey) || !isUsableSecret(apiSecret) || (needsPassphrase && !isUsableSecret(apiPassphrase))) {
            console_1.logger.warn("EXCHANGE", `API credentials for ${provider} are missing — using public market data only. Import and trade prices still work; deposit/withdraw need keys.`);
            (_d = ctx === null || ctx === void 0 ? void 0 : ctx.step) === null || _d === void 0 ? void 0 : _d.call(ctx, `Public-only ${provider} (no API keys)`);
            const pub = createCcxtClient(provider, {
                agent,
                timeout: 30000,
            ...
            try {
                ...
                console_1.logger.error("EXCHANGE", `Public loadMarkets failed for ${provider}: ${publicError === null || publicError === void 0 ? void 0 : publicError.message}`, publicError);
            }
            this.exchangeCache.set(provider, pub);
            this.attemptCount = 0;
            this.lastAttemptTime = null;
            return pub;
        }
        ...
        try {
            ...
            let exchange = createCcxtClient(provider,{
                apiKey,
                secret: apiSecret,
            ...
            if (!credentialsValid) {
                ...
                exchange = createCcxtClient(provider,{
                    agent,
                    timeout: 30000,
                    enableRateLimit: true,
                });
            }
            if (!(await this.syncExchangeTime(exchange, ctx))) {
                this.applyFallbackTimeOffset(exchange);
            }
            try {
                ...
                if (this.isRateLimitError(error)) {
                    ...
                    const freshExchange = createCcxtClient(provider,{
                        apiKey,
                        secret: apiSecret,
                        password: apiPassphrase,
                        agent,
                    ...
                    if (!syncSuccess) {
                        ...
                        this.applyFallbackTimeOffset(freshExchange);
                    }
                    try {
                    ...
                    console_1.logger.warn("EXCHANGE", `Falling back to an unauthenticated ${provider} instance — authenticated data (deposit/withdraw networks, balances) will be unavailable until valid API credentials are configured.`);
                    (_q = ctx === null || ctx === void 0 ? void 0 : ctx.step) === null || _q === void 0 ? void 0 : _q.call(ctx, `Falling back to public-only ${provider} instance`);
                    await exchange.close();
                    exchange = createCcxtClient(provider,{
                        agent,
                        timeout: 30000,
                    ...
                    try {
                        ...
                        console_1.logger.error("EXCHANGE", `Failed to load markets on the public ${provider} fallback: ${publicError === null || publicError === void 0 ? void 0 : publicError.message}`, publicError);
                    }
                }
            }
            ...
            this.attemptCount = 0;
            this.lastAttemptTime = null;
            (_r = ctx === null || ctx === void 0 ? void 0 : ctx.step) === null || _r === void 0 ? void 0 : _r.call(ctx, `Exchange ${provider} initialized successfully`);
            return exchange;
        }
        catch (error) {
            ...
            if (retries > 0 &&
                ...
                (_s = ctx === null || ctx === void 0 ? void 0 : ctx.step) === null || _s === void 0 ? void 0 : _s.call(ctx, `Retrying exchange initialization for ${provider} (${retries} retries left)`);
    ...
    async syncExchangeTime(exchange, ctx) {
        ...
        try {
            ...
            const localAtServerStamp = before + (after - before) / 2;
            const drift = Math.round(localAtServerStamp - serverTime);
            const timeDifference = drift + ExchangeManager.TIME_SAFETY_MARGIN_MS;
            exchange.options = exchange.options || {};
            exchange.options.adjustForTimeDifference = true;
            exchange.options.recvWindow = 60000;
            exchange.options.timeDifference = timeDifference;
            exchange.lastTimeSyncAt = after;
            if (Math.abs(drift) > 5000) {
                (_b = ctx === null || ctx === void 0 ? void 0 : ctx.step) === null || _b === void 0 ? void 0 : _b.call(ctx, `Significant time offset detected: ${drift}ms, adjusting...`);
                console_1.logger.warn("EXCHANGE", `Significant time offset with server: ${drift}ms. Consider syncing your system clock.`);
            }
            (_c = ctx === null || ctx === void 0 ? void 0 : ctx.step) === null || _c === void 0 ? void 0 : _c.call(ctx, `Time synchronized. Clock drift: ${drift}ms, signing offset: ${timeDifference}ms`);
            console_1.logger.debug("EXCHANGE", `Time synchronized with server. Clock drift: ${drift}ms, signing offset: ${timeDifference}ms`);
            return true;
        }
        catch (error) {
    ...
    applyFallbackTimeOffset(exchange) {
        exchange.options = exchange.options || {};
        exchange.options.adjustForTimeDifference = true;
        exchange.options.recvWindow = 60000;
        exchange.options.timeDifference = ExchangeManager.FALLBACK_TIME_OFFSET_MS;
    }
    maybeResyncTime(exchange) {
        if (!exchange)
            return;
        const last = exchange.lastTimeSyncAt || 0;
        if (Date.now() - last < ExchangeManager.TIME_RESYNC_INTERVAL_MS)
            return;
        exchange.lastTimeSyncAt = Date.now();
        void this.syncExchangeTime(exchange);
    }
    ...
    async startExchange(ctx) {
        ...
        const activeName = (providerData && providerData.name) || null;
        if (this.provider !== activeName) {
            (_c = ctx === null || ctx === void 0 ? void 0 : ctx.step) === null || _c === void 0 ? void 0 : _c.call(ctx, `Switching exchange provider to ${activeName || "none"}`);
            await this.stopExchange();
            this.provider = activeName;
            this.attemptCount = 0;
            this.lastAttemptTime = null;
        ...
        try {
            ...
            if (this.exchangeCache.has(this.provider)) {
                ...
                this.maybeResyncTime(this.exchange);
    ...
    async startExchangeProvider(provider, ctx) {
        ...
        this.exchangeProvider =
            cached || (await this.initializeExchange(provider, 3, ctx));
    ...
    async testExchangeCredentials(provider, ctx, retryCount = 0) {
        ...
        try {
            ...
            const apiSecret = process.env[`APP_${provider.toUpperCase()}_API_SECRET`];
            const apiPassphrase = process.env[`APP_${provider.toUpperCase()}_API_PASSPHRASE`];
            if (!isUsableSecret(apiKey) || !isUsableSecret(apiSecret)) {
    ...
    async stopExchange() {
        if (this.exchange) {
            try {
                ...
                console_1.logger.warn("EXCHANGE", "Failed to close exchange cleanly", error);
            }
        }
        this.exchange = null;
        this.provider = null;
...
ExchangeManager.TIME_SAFETY_MARGIN_MS = 500;
ExchangeManager.FALLBACK_TIME_OFFSET_MS = 1000;
ExchangeManager.TIME_RESYNC_INTERVAL_MS = 5 * 60 * 1000;
