...
const hb_paths_1 = require("../handler/utils/hb-paths");
const mime_1 = require("./mime");
const validation_1 = require("./validation");
const console_1 = require("./console");
const abort_latch_1 = require("../handler/utils/abort-latch");
...
`;
const notFoundFn = (res, req) => {
    const path = String((req === null || req === void 0 ? void 0 : req.url) || "").split("?")[0];
    if ((0, hb_paths_1.isHbBotApiPath)(path)) {
        res.status(404);
        res.json({ code: -1000, msg: `Unknown endpoint: ${path}` });
        return;
    }
...
function isRecoverableNetworkError(error, depth = 0) {
    if (!error || depth > 3)
        return false;
    if (typeof error.code === "string" && RECOVERABLE_ERROR_CODES.has(error.code))
        return true;
    if (typeof error.errno === "string" &&
        RECOVERABLE_ERROR_CODES.has(error.errno))
        return true;
    ...
    if (message.includes("Unexpected server response") ||
        ...
        [...RECOVERABLE_ERROR_CODES].some((code) => message.includes(code)))
        return true;
    if (Array.isArray(error.errors))
        return error.errors.some((inner) => isRecoverableNetworkError(inner, depth + 1));
    if (error.cause)
        return isRecoverableNetworkError(error.cause, depth + 1);
    return false;
}
function setupProcessEventHandlers() {
    process.on("uncaughtException", (error) => {
        ...
        if (isRecoverableNetworkError(error)) {
            ...
            const origin = ((_a = String((error === null || error === void 0 ? void 0 : error.stack) || "").split("\n")[1]) === null || _a === void 0 ? void 0 : _a.trim()) || "";
            console_1.logger.warn("SYSTEM", `Recoverable network error caught (${code}), server will continue running${origin ? ` — ${origin}` : ""}`);
            return;
        }
        console.error("[FATAL] Uncaught exception:", (error === null || error === void 0 ? void 0 : error.stack) || error);
...
const COMPRESSIBLE_STATIC_EXT = new Set(['.svg', '.txt', '.csv']);
...
function serveStaticFile(res, req, filePath, markResponseSent) {
    let aborted = false;
    const rawPath = String(filePath || "").split("?")[0].replace(/\\/g, "/");
    if (PRIVATE_UPLOAD_PREFIXES.some((prefix) => rawPath.startsWith(prefix))) {
    ...
    try {
        ...
        if (fs_1.default.existsSync(fullPath) && fs_1.default.lstatSync(fullPath).isFile()) {
            ...
            const fileExtension = path_1.default.extname(fullPath).toLowerCase();
            if (COMPRESSIBLE_STATIC_EXT.has(fileExtension)) {
