...
class Logger {
    ...
    getTimestamp() {
        const now = new Date();
        return `${process_role_1.roleLogPrefix}${colors_1.colors.gray}${now.toISOString().split("T")[1].slice(0, 8)}${colors_1.colors.reset}`;
    }
    ...
    live(module, title) {
        ...
        const indent = " ".repeat(module.length + 12 + process_role_1.ROLE_PREFIX_WIDTH);
        const closing = (message, fallbackLabel) => {
            const trimmed = message === null || message === void 0 ? void 0 : message.trim();
            const ms = trimmed !== undefined ? Number(trimmed) : NaN;
            const isDurationOnly = trimmed !== "" && Number.isFinite(ms);
            return {
                label: isDurationOnly ? fallbackLabel : message || fallbackLabel,
                duration: this.formatDuration(isDurationOnly ? ms : Date.now() - startTime),
            };
        };
        return {
            step: (message, status) => {
            ...
            succeed: (message) => {
                const { label, duration } = closing(message, "Done");
                console.log(`${indent}└─ ${colors_1.colors.green}${colors_1.icons.success}${colors_1.colors.reset} ${colors_1.colors.green}${label}${colors_1.colors.reset} ${colors_1.colors.gray}(${duration})${colors_1.colors.reset}`);
            },
            fail: (message) => {
                const { label, duration } = closing(message, "Failed");
                console.log(`${indent}└─ ${colors_1.colors.red}${colors_1.icons.error}${colors_1.colors.reset} ${colors_1.colors.red}${label}${colors_1.colors.reset} ${colors_1.colors.gray}(${duration})${colors_1.colors.reset}`);
            },
            warn: (message) => {
                const { label, duration } = closing(message, "Warning");
                console.log(`${indent}└─ ${colors_1.colors.yellow}${colors_1.icons.warning}${colors_1.colors.reset} ${colors_1.colors.yellow}${label}${colors_1.colors.reset} ${colors_1.colors.gray}(${duration})${colors_1.colors.reset}`);
            },
            setRequest: () => { },
        };
    }
    banner(name, version, env) {
        ...
        const split = role !== "inline";
        const width = split ? 56 : 50;
        const line = colors_1.box.horizontal.repeat(width - 2);
        const edge = split ? (0, process_role_1.roleColor)() : colors_1.colors.cyan;
        const bar = `${edge}${colors_1.box.vertical}${colors_1.colors.reset}`;
        console.log("");
        console.log(`${edge}${colors_1.box.topLeft}${line}${colors_1.box.topRight}${colors_1.colors.reset}`);
        console.log(`${bar}${this.center(` ${colors_1.icons.rocket} ${colors_1.colors.bold}${colors_1.colors.brightCyan}${name}${colors_1.colors.reset} `, width - 2)}${bar}`);
        console.log(`${bar}${this.center(`${colors_1.colors.gray}v${version} • ${env}${colors_1.colors.reset}`, width - 2)}${bar}`);
        if (split) {
            console.log(`${edge}${colors_1.box.teeRight}${line}${colors_1.box.teeLeft}${colors_1.colors.reset}`);
            console.log(`${bar}${this.center((0, process_role_1.roleBadge)(), width - 2)}${bar}`);
            console.log(`${bar}${this.center(`${colors_1.colors.gray}${(0, process_role_1.roleSummary)()}${colors_1.colors.reset}`, width - 2)}${bar}`);
        }
        console.log(`${edge}${colors_1.box.bottomLeft}${line}${colors_1.box.bottomRight}${colors_1.colors.reset}`);
        console.log("");
    }
    section(title) {
    ...
    ready(port, startTime) {
        ...
        const timeStr = duration > 0 ? ` ${colors_1.colors.gray}(${this.formatDuration(duration)})${colors_1.colors.reset}` : "";
        const role = (0, process_role_1.processRole)();
        const what = role === "cron" ? "Scheduler ready" : role === "web" ? "Web tier ready" : "Server ready";
        console.log("");
        console.log(`${process_role_1.roleLogPrefix}${colors_1.colors.green}${colors_1.icons.success}${colors_1.colors.reset} ${colors_1.colors.bold}${colors_1.colors.green}${what}${colors_1.colors.reset} ${colors_1.colors.dim}on port${colors_1.colors.reset} ${colors_1.colors.cyan}${port}${colors_1.colors.reset}${timeStr}`);
        console.log("");
    }
