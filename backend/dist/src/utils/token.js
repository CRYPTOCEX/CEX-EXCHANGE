...
// Security: enforce strong JWT secrets. Previously the code silently fell back to literal "secret"
// when env vars were missing, allowing trivial token forgery. We now require each secret to be
// at least 32 chars and not match the literal "secret".
const requireJwtSecret = (envName) => {
    const val = process.env[envName];
    if (!val || val === "secret" || val.length < 32) {
        const msg = `${envName} is missing or weak (must be at least 32 chars and not literal "secret"). Refusing to operate with an insecure JWT key.`;
        console_1.logger.error("AUTH", msg, new Error(msg));
        throw (0, error_1.createError)({ statusCode: 500, message: "Server JWT secret is not configured correctly. Contact administrator." });
    }
    return val;
};
...
const generateAccessToken = async (user) => {
    ...
    const APP_ACCESS_TOKEN_SECRET = requireJwtSecret("APP_ACCESS_TOKEN_SECRET");
...
const generateRefreshToken = async (user) => {
    ...
    const APP_REFRESH_TOKEN_SECRET = requireJwtSecret("APP_REFRESH_TOKEN_SECRET");
    const JWT_REFRESH_EXPIRY = process.env.JWT_REFRESH_EXPIRY || "14d";
    return new jose_1.SignJWT(jwtClaims)
        .setProtectedHeader({ alg: "HS256" })
        .setIssuedAt()
...
const generateEmailCode = async (userId) => {
    // CSPRNG: 4 bytes interpreted as uint32, modulo 900000, plus 100000 → 6-digit code with crypto entropy.
    const verificationCode = (100000 + (crypto_1.default.randomBytes(4).readUInt32BE(0) % 900000)).toString();
...
const generateResetToken = async (user) => {
    ...
    const APP_RESET_TOKEN_SECRET = requireJwtSecret("APP_RESET_TOKEN_SECRET");
