...
function convertBooleanStrings(value, schema) {
    if (typeof value === "string") {
        const declared = Array.isArray(schema === null || schema === void 0 ? void 0 : schema.type)
            ? schema.type
            : typeof (schema === null || schema === void 0 ? void 0 : schema.type) === "string"
                ? [schema.type]
                : [];
        if (declared.some((t) => t === "string" || t === "number" || t === "integer")) {
            return value;
        }
        ...
        if (Array.isArray(value)) {
            ...
            for (let i = 0; i < value.length; i++) {
                value[i] = convertBooleanStrings(value[i], itemSchema);
            }
        }
        else {
            const props = schema && schema.properties ? schema.properties : undefined;
            for (const key in value) {
                value[key] = convertBooleanStrings(value[key], props ? props[key] : undefined);
            }
        }
    }
    return value;
...
const schemaAjv = new ajv_1.default({ allErrors: true, coerceTypes: true, strict: false });
(0, ajv_formats_1.default)(schemaAjv);
...
function validateSchema(value, schema) {
    ...
    const transformedValue = convertBooleanStrings(value, schema);
