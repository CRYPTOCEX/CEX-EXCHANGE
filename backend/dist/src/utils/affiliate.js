...
async function processDirectRewards(condition, referredId, amount, ctx, sourceId) {
    ...
    try {
        ...
        if (!referral)
            return false;
        // Reject self-referrals: legacy users have a self-referral row used as a binary-tree root
        // placeholder. Awarding them commissions on their own actions is unintended.
...
async function findUplines(userId, systemType, levels) {
    ...
    for (let i = 0; i < levels; i++) {
        try {
            ...
            // Stop traversal at self-referrals (legacy tree-root placeholders)
            if (referral.referrerId === currentUserId) {
                console_1.logger.info("MLM", `Reached self-referral root for user ${currentUserId} at level ${i + 1}`);
                break;
            }
            uplines.push({
                level: i + 1,
