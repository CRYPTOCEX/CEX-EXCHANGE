...
function applyDemoMask(data, maskPaths) {
    if (!isDemo)
        return data;
    return applyMask(data, maskPaths);
}
function applyMask(data, maskPaths) {
    if (!data || !maskPaths || maskPaths.length === 0) {
        return data;
    }
...
function stripFields(value, denied) {
    const deny = new Set(Array.from(denied, (k) => k.toLowerCase()));
