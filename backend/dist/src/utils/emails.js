...
exports.mailDisabled = mailDisabled;
exports.isUndeliverableAddress = isUndeliverableAddress;
...
function mailDisabled() {
    const flag = String(process.env.MAIL_DISABLED || "").trim().toLowerCase();
    return flag === "true" || flag === "1" || flag === "yes" || flag === "on";
}
function isUndeliverableAddress(address) {
    return (0, system_accounts_1.isUnmailableRecipient)(address);
}
let bullEmailQueue = null;
let queueErrorLogged = false;
function createBullEmailQueue() {
    const queue = new bull_1.default("emailQueue", {
        redis: {
    ...
    queue.process(async (job) => {
        ...
        try {
            await processEmailJobData(job.data);
...
function getBullEmailQueue() {
    if (!redis_1.RedisSingleton.isRedisUp())
        return null;
    if (!bullEmailQueue)
        bullEmailQueue = createBullEmailQueue();
    return bullEmailQueue;
}
...
async function processEmailJobData(data) {
    const { emailData, emailType } = data;
...
async function enqueueEmail(payload) {
    var _a, _b, _c, _d;
    if (mailDisabled()) {
        console_1.logger.warn("EMAIL", `MAIL_DISABLED: dropping ${payload.emailType} to ${(_a = payload.emailData) === null || _a === void 0 ? void 0 : _a.TO}`);
        return;
    }
    if (isUndeliverableAddress((_b = payload.emailData) === null || _b === void 0 ? void 0 : _b.TO)) {
        console_1.logger.debug("EMAIL", `Skipping ${payload.emailType} to ${(_c = payload.emailData) === null || _c === void 0 ? void 0 : _c.TO}: reserved undeliverable domain`);
        return;
    }
    const queue = getBullEmailQueue();
    if (queue) {
        await queue.add(payload);
        return;
    }
    try {
        await processEmailJobData(payload);
    }
    catch (error) {
        console_1.logger.error("EMAIL", `Inline email send failed (Redis down): ${payload.emailType} to ${(_d = payload.emailData) === null || _d === void 0 ? void 0 : _d.TO}`, error instanceof Error ? error : new Error(String(error)));
    }
}
exports.emailQueue = {
    add: (payload) => enqueueEmail(payload),
};
async function sendEmail(specificVariables, templateName, ctx) {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j;
    if (mailDisabled()) {
        console_1.logger.warn("EMAIL", `MAIL_DISABLED: dropping ${templateName} to ${specificVariables === null || specificVariables === void 0 ? void 0 : specificVariables.TO}`);
        (_a = ctx === null || ctx === void 0 ? void 0 : ctx.step) === null || _a === void 0 ? void 0 : _a.call(ctx, "Email suppressed (MAIL_DISABLED)");
        return;
    }
    if (isUndeliverableAddress(specificVariables === null || specificVariables === void 0 ? void 0 : specificVariables.TO)) {
        console_1.logger.debug("EMAIL", `Skipping ${templateName} to ${specificVariables === null || specificVariables === void 0 ? void 0 : specificVariables.TO}: reserved undeliverable domain`);
        (_b = ctx === null || ctx === void 0 ? void 0 : ctx.success) === null || _b === void 0 ? void 0 : _b.call(ctx, "Email suppressed (undeliverable domain)");
        return;
    }
    let processedTemplate;
    let processedSubject;
    try {
        (_c = ctx === null || ctx === void 0 ? void 0 : ctx.step) === null || _c === void 0 ? void 0 : _c.call(ctx, `Processing email template: ${templateName}`);
    ...
    let finalEmailHtml;
    try {
        (_e = ctx === null || ctx === void 0 ? void 0 : ctx.step) === null || _e === void 0 ? void 0 : _e.call(ctx, "Preparing email template");
    ...
    try {
        (_g = ctx === null || ctx === void 0 ? void 0 : ctx.step) === null || _g === void 0 ? void 0 : _g.call(ctx, `Sending email to ${specificVariables["TO"]}`);
        await (0, mailer_1.sendEmailWithProvider)(emailer, options);
        (_h = ctx === null || ctx === void 0 ? void 0 : ctx.success) === null || _h === void 0 ? void 0 : _h.call(ctx, `Email sent successfully to ${specificVariables["TO"]}`);
    }
    catch (error) {
        console_1.logger.error("EMAIL", "Error sending email with provider", error);
        (_j = ctx === null || ctx === void 0 ? void 0 : ctx.fail) === null || _j === void 0 ? void 0 : _j.call(ctx, error.message);
        throw error;
    }
}
...
async function sendFiatTransactionEmail(user, transaction, currency, newBalance, ctx) {
    ...
    try {
        await enqueueEmail({ emailData, emailType });
...
async function sendSpotWalletWithdrawalConfirmationEmail(user, transaction, wallet, ctx) {
    ...
    const emailData = {
        ...
        AMOUNT: transaction.amount,
        CURRENCY: wallet.currency,
        ADDRESS: (_e = (_d = txMetadata.address) !== null && _d !== void 0 ? _d : txMetadata.toAddress) !== null && _e !== void 0 ? _e : "N/A",
        FEE: transaction.fee,
        CHAIN: (_f = txMetadata.chain) !== null && _f !== void 0 ? _f : "N/A",
        MEMO: txMetadata.memo || "N/A",
...
async function sendAiInvestmentEmail(user, plan, duration, investment, emailType, ctx) {
    ...
    const emailData = {
        ...
        USER_ID: user.id,
        FIRSTNAME: user.firstName,
        PLAN_NAME: (_d = plan === null || plan === void 0 ? void 0 : plan.title) !== null && _d !== void 0 ? _d : "AI Investment",
        AMOUNT: investment.amount.toString(),
        CURRENCY: investment.symbol.split("/")[1],
        DURATION: (duration === null || duration === void 0 ? void 0 : duration.duration) != null ? duration.duration.toString() : "",
        TIMEFRAME: (_e = duration === null || duration === void 0 ? void 0 : duration.timeframe) !== null && _e !== void 0 ? _e : "",
...
async function sendInvestmentEmail(user, plan, duration, investment, emailType, ctx) {
    ...
    const emailData = {
        ...
        USER_ID: user.id,
        FIRSTNAME: user.firstName,
        PLAN_NAME: (_d = plan === null || plan === void 0 ? void 0 : plan.title) !== null && _d !== void 0 ? _d : "Investment",
        AMOUNT: investment.amount.toString(),
        DURATION: (duration === null || duration === void 0 ? void 0 : duration.duration) != null ? duration.duration.toString() : "",
        TIMEFRAME: (_e = duration === null || duration === void 0 ? void 0 : duration.timeframe) !== null && _e !== void 0 ? _e : "",
        STATUS: investment.status,
...
async function sendStakingRewardEmail(user, stake, pool, reward, ctx) {
    ...
    try {
        await enqueueEmail({ emailData, emailType: "StakingRewardDistribution" });
...
async function sendOrderConfirmationEmail(user, order, product, ctx) {
    ...
    const fallbackSubtotal = ((_b = fullOrder === null || fullOrder === void 0 ? void 0 : fullOrder.ecommerceOrderItems) === null || _b === void 0 ? void 0 : _b.reduce((total, item) => {
        var _a;
        var _b;
        return total + (((_b = (_a = item.product) === null || _a === void 0 ? void 0 : _a.price) !== null && _b !== void 0 ? _b : 0) * item.quantity);
    }, 0)) || product.price;
    const subtotal = Number((_g = order.subtotal) !== null && _g !== void 0 ? _g : fallbackSubtotal);
    const discountAmount = Number((_h = order.discount) !== null && _h !== void 0 ? _h : 0);
    const shippingCost = Number((_j = order.shippingCost) !== null && _j !== void 0 ? _j : 0);
    const taxAmount = Number((_k = order.tax) !== null && _k !== void 0 ? _k : 0);
    const orderTotal = Number((_l = order.total) !== null && _l !== void 0 ? _l : subtotal - discountAmount + shippingCost + taxAmount);
    const emailData = {
        TO: user.email,
        ...
        QUANTITY: ((_d = (_c = fullOrder === null || fullOrder === void 0 ? void 0 : fullOrder.ecommerceOrderItems) === null || _c === void 0 ? void 0 : _c[0]) === null || _d === void 0 ? void 0 : _d.quantity) || 1,
        PRODUCT_PRICE: product.price.toString(),
        PRODUCT_CURRENCY: (_m = order.currency) !== null && _m !== void 0 ? _m : product.currency,
    ...
    try {
        await enqueueEmail({ emailData, emailType: "OrderConfirmation" });
...
async function sendKycEmail(user, kyc, type, ctx) {
    ...
    try {
        await enqueueEmail({ emailData, emailType: type });
