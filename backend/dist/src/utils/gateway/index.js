...
exports.validateApiKeyWalletScope = validateApiKeyWalletScope;
...
function generateRandomString(length) {
    ...
    for (let i = 0; i < length; i++) {
        result += CHARSET.charAt(crypto_1.default.randomInt(0, CHARSET.length));
    }
    return result;
}
...
function calculateFees(amount, feeType, feePercentage, feeFixed, currency) {
    let feeAmount = 0;
    switch (feeType) {
        case "PERCENTAGE":
    ...
    const places = currency ? (0, wallet_1.getPrecision)(currency) : 8;
    const scale = Math.pow(10, places);
    feeAmount = Math.round(feeAmount * scale) / scale;
    const netAmount = Math.round((amount - feeAmount) * scale) / scale;
...
async function returnGatewayFee(params) {
    ...
    const feeToReturn = (0, wallet_1.roundToPrecision)(feeAmount, currency);
    if (feeToReturn <= 0)
        return;
    await (0, fees_1.recordPlatformLoss)({
        currency,
        walletType,
        lossAmount: feeToReturn,
        type: "GATEWAY_PAYMENT",
        description: `Gateway fee returned for refund ${refundId}`,
        referenceId: `${refundId}_fee_return_${currency}`,
        metadata: {
            refundId,
            merchantId,
            reason: "GATEWAY_FEE_RETURN",
        },
        transaction: t,
    });
}
...
async function collectGatewayFee(params) {
    ...
    await (0, fees_1.ensureAdminProfitRow)(creditResult.transactionId, {
        type: "GATEWAY_PAYMENT",
        amount: feeAmount,
        currency,
        chain: null,
        description: `Gateway payment fee from merchant ${merchantId}`,
    }, { referenceId: `${paymentId}_fee`, repairing: false, transaction: t });
}
async function updateMerchantBalanceForPayment(params) {
    const { merchantId, currency, walletType, amount, feeAmount, transaction: t } = params;
    let balance = await db_1.models.gatewayMerchantBalance.findOne({
        where: { merchantId, currency, walletType },
        transaction: t,
    });
...
async function sendWebhook(merchantId, paymentId, refundId, eventType, url, payload, webhookSecret) {
    ...
    await attemptWebhookDelivery(webhook, webhookSecret);
}
async function attemptWebhookDelivery(webhook, webhookSecret) {
    var _a;
    const startTime = Date.now();
    const secret = webhookSecret !== null && webhookSecret !== void 0 ? webhookSecret : (_a = webhook.merchant) === null || _a === void 0 ? void 0 : _a.webhookSecret;
    if (!secret) {
        console_1.logger.error("GATEWAY_WEBHOOK", `No webhook secret available for merchant ${webhook.merchantId}; refusing to send an unsigned webhook`);
        await webhook.update({
            status: "FAILED",
            lastAttemptAt: new Date(),
            errorMessage: "Merchant webhook secret unavailable — webhook not sent",
        });
        return;
    }
    try {
        const { signature, timestamp } = signWebhookPayload(webhook.payload, secret);
        ...
        const exhausted = attempt >= webhook.maxAttempts;
        await webhook.update({
            attempts: attempt,
            lastAttemptAt: new Date(),
            signature,
            ...
            responseTime,
            status: response.ok ? "SENT" : exhausted ? "FAILED" : "RETRYING",
            nextRetryAt: response.ok || exhausted
                ? null
...
function validateApiKeyWalletScope(apiKey, walletType) {
    const scope = apiKey === null || apiKey === void 0 ? void 0 : apiKey.allowedWalletTypes;
    if (!scope || typeof scope !== "object" || Array.isArray(scope))
        return;
    const enabled = Object.entries(scope).filter(([, config]) => config === null || config === void 0 ? void 0 : config.enabled);
    if (enabled.length === 0)
        return;
    const match = enabled.find(([type]) => type === walletType);
    if (!match) {
        throw (0, error_1.createError)({
            statusCode: 403,
            message: `This API key is not permitted to create ${walletType} payments`,
        });
    }
}
