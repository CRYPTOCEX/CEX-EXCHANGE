...
class DeepSeekClient {
    ...
    async suggestRelated(question, answer, candidates) {
        var _a;
        const list = candidates
            .map((c, i) => `${i + 1}. [${c.id}] ${c.question}${c.category ? ` (${c.category})` : ""}`)
            .join("\n");
        const prompt = `
      A knowledge-base article is being written:

      Question: ${question}
      Answer: ${answer}

      From the numbered list below, choose the up to 5 entries a reader of that
      article would most likely want to read next. Only choose entries that are
      genuinely related; choosing nothing is a valid answer.

      ${list}

      Return ONLY a JSON array of the chosen ids, e.g. ["id-1","id-2"]. No prose.
    `;
        const response = await this.generateText(prompt, {
            temperature: 0.2,
            max_tokens: 300,
        ...
        try {
            ...
            console_1.logger.error("DEEPSEEK", "Failed to parse related response as JSON", e);
            ids = ((_a = response.match(/"([0-9a-fA-F-]{36})"/g)) !== null && _a !== void 0 ? _a : []).map((s) => s.replace(/"/g, ""));
        }
        const allowed = new Set(candidates.map((c) => c.id));
        return Array.isArray(ids)
            ? ids
                .filter((i) => typeof i === "string" && allowed.has(i))
                .slice(0, 5)
            : [];
    }
