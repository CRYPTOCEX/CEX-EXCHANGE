...
async function resetChartEngineSettings() {
    ...
    try {
        ...
        let binarySettingsChanged = false;
        ...
        if (binarySettingsChanged) {
            try {
                const { invalidateBinarySettingsCache } = await Promise.resolve().then(() => __importStar(require("@b/utils/binary-settings-cache")));
                await invalidateBinarySettingsCache();
            }
            catch (_d) {
            }
        }
...
async function getPurchaseCode(productId) {
    const licData = await (0, security_1.readLicenseFile)(productId);
    return (licData === null || licData === void 0 ? void 0 : licData.purchaseCode) || null;
}
...
async function processLicenseHeartbeat() {
    ...
    try {
        ...
        let successCount = 0;
        let failCount = 0;
        const CHART_ENGINE_PRODUCT_ID = "61364182";
...
// LICENSE HEARTBEAT BYPASS
try {
  if (typeof exports.sendHeartbeat === 'function') {
    exports.sendHeartbeat = async function () { return { skipped: true }; };
  }
  if (typeof exports.default === 'function') {
    const _hb = exports.default;
    exports.default = async function () { return { skipped: true }; };
  }
} catch (e) {}
