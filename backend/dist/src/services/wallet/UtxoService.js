...
class UtxoService {
    ...
    async selectUtxos(walletId, targetAmount, chain, feePerByte = 10, transaction) {
        if (!db_1.models.ecosystemUtxo) {
            throw new errors_1.UtxoError("EcosystemUtxo model not available");
        }
        const utxos = await db_1.models.ecosystemUtxo.findAll({
            where: { walletId, status: "UNSPENT" },
            order: [["amount", "DESC"]],
            ...(transaction && {
                transaction,
    ...
    async markAsSpent(utxoIds, transaction) {
        ...
        await db_1.models.ecosystemUtxo.update({ status: "SPENT" }, {
    ...
    async markAsUnspent(utxoIds, transaction) {
        ...
        await db_1.models.ecosystemUtxo.update({ status: "UNSPENT" }, {
            where: { id: { [sequelize_1.Op.in]: utxoIds } },
            ...(transaction && { transaction }),
        });
