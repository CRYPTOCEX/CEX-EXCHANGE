...
class WalletCreationService {
    ...
    async createFiatWallet(userId, currency, transaction) {
        ...
        await AuditLogger_1.auditLogger.logWalletCreated(wallet.id, userId, "FIAT", currency, undefined, transaction);
    ...
    async createSpotWallet(userId, currency, transaction) {
        ...
        await AuditLogger_1.auditLogger.logWalletCreated(wallet.id, userId, "SPOT", currency, undefined, transaction);
        return this.buildResult(this.toWalletAttributes(wallet));
    }
    async createEcoWallet(userId, currency, generateAddresses, transaction) {
        ...
        if (generateAddresses) {
            ...
            for (const token of tokens) {
                try {
                    ...
                    const stage = addresses[token.chain] ? "record the wallet" : "set up";
                    console_1.logger.error("WALLET_CREATION", `Failed to ${stage} ${token.chain} for wallet ${wallet.id}: ${error.message}. ` +
                        `This chain is omitted from the wallet; the other chains continue.`);
                }
            }
        ...
        await AuditLogger_1.auditLogger.logWalletCreated(wallet.id, userId, "ECO", currency, Object.keys(addresses), transaction);
    ...
    async createFuturesWallet(userId, currency, transaction) {
        ...
        await AuditLogger_1.auditLogger.logWalletCreated(wallet.id, userId, "FUTURES", currency, undefined, transaction);
        return this.buildResult(this.toWalletAttributes(wallet));
    }
    async createCopyTradingWallet(userId, currency, transaction) {
        ...
        await AuditLogger_1.auditLogger.logWalletCreated(wallet.id, userId, "COPY_TRADING", currency, undefined, transaction);
        return this.buildResult(this.toWalletAttributes(wallet));
    }
    async ensureEcoWalletComplete(wallet, transaction) {
        ...
        let updated = false;
        for (const token of tokens) {
            if (this.isChainMissing(addresses, token)) {
                try {
                    const generated = await this.addressGenService.generateAddress({
                        walletId: wallet.id,
        ...
        const updatedWallet = updated
            ? await db_1.models.wallet.findByPk(wallet.id, { transaction })
            : wallet;
        return this.buildResult(this.toWalletAttributes(updatedWallet), walletDataRecords, addresses);
    }
    async getActiveTokens(currency) {
        ...
        const eligible = tokens.filter((token) => (0, token_network_1.tokenNetworkMatchesEnv)(token, catalogue));
        if (eligible.length < tokens.length) {
            const skipped = tokens.filter((token) => !eligible.includes(token));
            console_1.logger.warn("WALLET_CREATION", `No ${currency} address issued on ${[
                ...new Set(skipped.map((token) => String(token.chain).toUpperCase())),
            ].join(", ")}: ${(0, token_network_1.describeTokenNetworkMismatch)(skipped, catalogue)}`);
        }
        return eligible;
    }
    ...
    isChainMissing(addresses, token) {
        return !addresses[token.chain];
    }
    async getOrCreateWallet(userId, type, currency, transaction) {
        ...
        if (existing) {
            if (type === "ECO") {
                ...
                const addresses = { ...this.parseAddresses(existing.address) };
                const tokens = await this.getActiveTokens(existing.currency);
                if (!tokens.some((token) => this.isChainMissing(addresses, token))) {
                    return this.buildResult(this.toWalletAttributes(existing), [], addresses);
                }
                return await db_1.sequelize.transaction(async (t) => {
                    return await this.ensureEcoWalletComplete(existing, t);
                });
