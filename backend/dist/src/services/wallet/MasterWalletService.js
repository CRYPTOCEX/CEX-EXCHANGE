...
class MasterWalletService {
    ...
    getUtxoNetwork(chain, bitcoin) {
        const btcNetwork = (process.env.BTC_NETWORK || "mainnet").toLowerCase();
        const btcIsTestnet = btcNetwork !== "mainnet" && btcNetwork !== "main";
        const networks = {
            BTC: btcIsTestnet ? bitcoin.networks.testnet : bitcoin.networks.bitcoin,
            LTC: {
    ...
    async incrementBalance(chain, amount, transaction) {
        const run = async (t) => {
            var _a;
            const walletRecord = await db_1.models.ecosystemMasterWallet.findOne({
                where: { chain },
            ...
            const currentBalance = parseFloat(((_a = walletRecord.balance) === null || _a === void 0 ? void 0 : _a.toString()) || "0");
            const newBalance = currentBalance + amount;
            await db_1.models.ecosystemMasterWallet.update({ balance: newBalance }, { where: { id: walletRecord.id }, transaction: t });
            console_1.logger.debug("MASTER_WALLET", `Updated balance for ${chain}: ${newBalance}`);
        };
        if (transaction) {
            ...
            await db_1.sequelize.transaction((t) => run(t));
        }
    }
    async decrementBalance(chain, amount, transaction) {
        const run = async (t) => {
            ...
            const newBalance = currentBalance - amount;
            if (newBalance < 0) {
                throw new errors_1.WalletError(`Insufficient master wallet balance: ${currentBalance} < ${amount}`, "INSUFFICIENT_MASTER_BALANCE", 400);
            }
            await db_1.models.ecosystemMasterWallet.update({ balance: newBalance }, { where: { id: walletRecord.id }, transaction: t });
            console_1.logger.debug("MASTER_WALLET", `Updated balance for ${chain}: ${newBalance}`);
        };
    ...
    async getNextIndex(chain, transaction) {
        ...
        return transaction
            ? await allocate(transaction)
            : await db_1.sequelize.transaction((t) => allocate(t));
    }
