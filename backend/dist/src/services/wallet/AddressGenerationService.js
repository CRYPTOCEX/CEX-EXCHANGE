...
const xmr_1 = require("@b/utils/xmr");
    ...
    async generateMoneroAddress(params) {
        try {
            ...
            return {
                chain: "XMR",
                address,
                network: (0, xmr_1.xmrNetworkFromAddress)(address) ||
    ...
    async generatePermitAddress(params) {
        ...
        try {
            ...
            const allocate = async (t) => {
                ...
                const idx = (masterWallet.lastIndex || 0) + 1;
                await db_1.models.ecosystemMasterWallet.update({ lastIndex: idx }, {
                    where: { id: masterWallet.id },
                    transaction: t,
                });
                return { nextIndex: idx, masterData: masterWallet.data };
            };
            const { nextIndex, masterData } = transaction
                ? await allocate(transaction)
                : await db_1.sequelize.transaction((t) => allocate(t));
            const decryptedMasterData = JSON.parse((0, encrypt_1.decrypt)(masterData));
