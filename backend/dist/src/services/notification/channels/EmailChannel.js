...
const transport_mode_1 = require("../providers/email/transport-mode");
...
class EmailChannel extends BaseChannel_1.BaseChannel {
    ...
    async send(operation, transaction) {
        try {
            ...
            const recipientEmail = data.testMode === true && data.overrideEmail
                ? data.overrideEmail
                : user.email;
            const emailData = {
                to: recipientEmail,
            ...
            const job = await NotificationQueue_1.notificationQueue.addEmailJob(provider, emailData, data.relatedId || `notif-${Date.now()}`, operation.userId, queuePriority, {
                notificationType: operation.type,
    ...
    validateConfig() {
        ...
        if (emailer.includes("sendgrid")) {
            ...
            const mode = (0, transport_mode_1.resolveMailTransportMode)(emailer);
            if (mode === "sendmail") {
                return true;
            }
            if (mode === "service") {
                if (!process.env.APP_NODEMAILER_SERVICE) {
                    this.logError("Nodemailer service not configured", {});
                    return false;
                }
                ...
                if (!process.env.APP_NODEMAILER_SMTP_HOST) {
                    this.logError("Nodemailer SMTP host not configured", {});
                    return false;
                }
