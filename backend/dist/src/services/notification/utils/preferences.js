...
async function filterChannelsByPreferences(userId, requestedChannels) {
    return channelsAllowed(await getUserPreferences(userId), requestedChannels);
}
async function isNotificationTypeEnabled(userId, type) {
    return typeAllowed(await getUserPreferences(userId), type);
}
async function filterChannelsByPreferencesAndType(userId, requestedChannels, type) {
    const prefs = await getUserPreferences(userId);
    if (!typeAllowed(prefs, type)) {
        return [];
    }
    return channelsAllowed(prefs, requestedChannels);
}
function typeAllowed(prefs, type) {
    if (!prefs.types)
        return true;
    return prefs.types[type.toLowerCase()] !== false;
}
function channelsAllowed(prefs, requestedChannels) {
