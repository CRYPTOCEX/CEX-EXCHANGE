...
exports.default = async (data) => {
    ...
    try {
        ...
        if (!user) {
            ctx === null || ctx === void 0 ? void 0 : ctx.success("Verification resend requested for non-existent email (neutral response)");
            return NEUTRAL;
        }
        if ((0, system_accounts_1.isSystemAccount)(user)) {
            ctx === null || ctx === void 0 ? void 0 : ctx.success("Verification resend requested for a platform system account (neutral response, no code minted)");
            return NEUTRAL;
        }
        ctx === null || ctx === void 0 ? void 0 : ctx.step("Checking email verification status");
        if (user.emailVerified) {
            ctx === null || ctx === void 0 ? void 0 : ctx.success("Verification resend requested for an already-verified account (neutral response)");
            return NEUTRAL;
        }
        const cacheManager = cache_1.CacheManager.getInstance();
        const verifyEmailEnabled = await cacheManager.getSettingBool("verifyEmailStatus", true);
        if (!verifyEmailEnabled) {
            ctx === null || ctx === void 0 ? void 0 : ctx.success("Verification resend requested while email verification is off (neutral response)");
            return NEUTRAL;
        }
        try {
