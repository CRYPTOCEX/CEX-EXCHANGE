...
exports.default = async (data) => {
    ...
    try {
        ...
        // Fee rate is determined by order TYPE (market = taker, limit = maker), not side.
        ...
        // Helper: cancel exchange-side order if local DB step fails AFTER this point.
        // Prevents the user from receiving fills on the exchange while the local wallet wasn't debited.
        const cancelExchangeOrderOnLocalFailure = async () => {
            try {
                if (order && order.id) {
                    await exchange.cancelOrder(order.id, symbol);
                    console_1.logger.warn("EXCHANGE", `Cancelled exchange order ${order.id} due to local DB failure`);
                }
            }
            catch (cancelErr) {
                console_1.logger.error("EXCHANGE", `CRITICAL: failed to cancel exchange order ${order.id} after local DB failure: ${cancelErr.message}. Manual reconciliation required.`);
            }
        };
        let orderData;
        try {
            ctx === null || ctx === void 0 ? void 0 : ctx.step("Fetching and adjusting order data");
            orderData = await exchange.fetchOrder(order.id, symbol);
        }
        catch (fetchErr) {
            await cancelExchangeOrderOnLocalFailure();
        ...
        let response;
        try {
            response = await db_1.sequelize.transaction(async (transaction) => {
            if (side === "BUY") {
            ...
            const orderRecord = await createOrder(user.id, {
                ...orderData,
            ...
            await cancelExchangeOrderOnLocalFailure();
            throw (0, error_1.createError)({ statusCode: 500, message: `Order placed on exchange but local recording failed (cancellation attempted): ${(0, utils_1.sanitizeErrorMessage)(localErr.message)}` });
        }
