...
const utils_1 = require("@b/api/user/kyc/document/utils");
exports.metadata = {
...
exports.default = async (data) => {
    ...
    try {
        ...
        if (fileSizeBytes > maxSizeBytes) {
            ctx === null || ctx === void 0 ? void 0 : ctx.fail(`File size ${Math.round(fileSizeBytes / 1024 / 1024)}MB exceeds 10MB limit`);
            throw (0, error_1.createError)({
                statusCode: 400,
                message: "File size exceeds maximum limit of 10MB",
            });
        }
        ctx === null || ctx === void 0 ? void 0 : ctx.step("Resolving private storage directory");
        const mediaDir = (0, utils_1.resolveKycOwnerDir)(user.id);
        if (!mediaDir) {
            ctx === null || ctx === void 0 ? void 0 : ctx.fail("Could not resolve a storage directory for this user");
            throw (0, error_1.createError)({
                statusCode: 500,
                message: "Could not resolve upload directory",
            });
        }
        ...
        const randomString = (0, crypto_1.randomBytes)(24).toString("hex");
        ...
        const generatedFilename = `${randomString}${extension}`;
        ...
        if (oldPath) {
            ...
            try {
                await removeOldFileSecurely(oldPath, user.id);
            }
            catch (error) {
        ...
        const response = {
            url: (0, utils_1.kycDocumentUrl)(user.id, generatedFilename),
...
async function removeOldFileSecurely(oldPath, userId) {
    const expectedBaseDir = (0, utils_1.resolveKycOwnerDir)(userId);
    if (!expectedBaseDir)
        return;
    const raw = String(oldPath || "");
    const prefix = utils_1.KYC_DOCUMENT_URL_PREFIX;
    const withoutPrefix = raw.startsWith(prefix) ? raw.slice(prefix.length) : null;
    if (!withoutPrefix)
        return;
    const [pathUserId, ...filenameParts] = withoutPrefix.split("/");
    if (filenameParts.length !== 1)
        return;
    if (String(pathUserId) !== String(userId))
        return;
    const oldFileFullPath = (0, utils_1.resolveKycDocumentPath)(userId, filenameParts[0]);
    if (!oldFileFullPath)
        return;
    if (!oldFileFullPath.startsWith(expectedBaseDir + path_1.default.sep) &&
        oldFileFullPath !== expectedBaseDir) {
        throw (0, error_1.createError)({
            statusCode: 403,
