...
exports.default = async (data) => {
    ...
    const investmentStatus = await cacheManager.getSettingBool("investment", true);
    if (!investmentStatus) {
    ...
    await (0, kyc_1.assertKycFeature)(user.id, String(type).toLowerCase() === "forex"
        ? kyc_1.KYC_FEATURES.INVEST_FOREX
        : kyc_1.KYC_FEATURES.INVEST_GENERAL, "invest");
    ...
    if (plan.minAmount != null &&
        ...
        ctx === null || ctx === void 0 ? void 0 : ctx.fail(`Amount below plan minimum: ${amount} < ${plan.minAmount}`);
        throw (0, error_1.createError)({
            statusCode: 400,
            message: `Amount must be at least ${plan.minAmount}`,
        });
    }
    if (plan.maxAmount != null &&
        Number(plan.maxAmount) > 0 &&
        amount > Number(plan.maxAmount)) {
        ctx === null || ctx === void 0 ? void 0 : ctx.fail(`Amount above plan maximum: ${amount} > ${plan.maxAmount}`);
        throw (0, error_1.createError)({
            statusCode: 400,
            message: `Amount must not exceed ${plan.maxAmount}`,
        });
    }
    ...
    const newInvestment = await db_1.sequelize.transaction(async (transaction) => {
        const existingActive = await model.findOne({
            where: {
                userId: user.id,
                planId,
                status: "ACTIVE",
        ...
        const idempotencyKey = `investment_${newInvestment.id}`;
