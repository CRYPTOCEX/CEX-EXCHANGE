...
exports.withoutChainBalances = withoutChainBalances;
exports.markUncreditableChains = markUncreditableChains;
...
function withoutChainBalances(wallet) {
    const raw = wallet === null || wallet === void 0 ? void 0 : wallet.address;
    if (!raw)
        return wallet;
    let parsed = raw;
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
        return wallet;
    const address = {};
    for (const [chain, entry] of Object.entries(parsed)) {
...
async function markUncreditableChains(wallet) {
    if (!wallet || wallet.type !== "ECO")
        return wallet;
    const raw = wallet.address;
    let parsed = raw;
    if (typeof parsed === "string") {
        try {
            parsed = JSON.parse(parsed);
    ...
    for (const [chain, entry] of Object.entries(parsed)) {
        ...
        const { network } = entry;
        const depositable = (0, token_network_1.tokenNetworkMatchesEnv)({ chain, network }, catalogue);
        address[chain] = depositable
            ? { ...entry, depositable: true }
            : { ...entry, address: null, depositable: false };
    }
    return { ...wallet, address };
}
async function getWallet(userId, type, currency, hasTransactions = false, ctx) {
    ...
    return withoutChainBalances(response.get({ plain: true }));
}
