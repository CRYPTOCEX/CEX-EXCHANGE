                ...
                "application/json": {
                    schema: {
                        ...
                        properties: {
                            ...
                            totalPending: {
                                type: "number",
                                description: "USD value of DEPOSIT transactions still awaiting approval. NOT included in totalBalance."
                            },
                            unpricedCurrencies: {
                                type: "array",
                                items: { type: "string" },
                                description: "Currencies held by this user that have no usable USD rate. Their balances are excluded from every USD total."
                            },
                            walletsByType: {
                                type: "object",
                                properties: {
...
exports.default = async (data) => {
    ...
    try {
        ...
        const walletsByType = {
            FIAT: { count: 0, balance: 0, balanceUSD: 0, pending: 0, pendingUSD: 0 },
            SPOT: { count: 0, balance: 0, balanceUSD: 0, pending: 0, pendingUSD: 0 },
            ECO: { count: 0, balance: 0, balanceUSD: 0, pending: 0, pendingUSD: 0 },
            FUTURES: { count: 0, balance: 0, balanceUSD: 0, pending: 0, pendingUSD: 0 }
        };
        let activeWallets = 0;
        ...
        for (const deposit of pendingDeposits) {
            if (!deposit.walletId)
                continue;
            const netAmount = Math.max((Number(deposit.amount) || 0) - (Number(deposit.fee) || 0), 0);
        ...
        const wantedSpot = Array.from(uniqueCurrencies.values())
            .filter(({ type }) => type === "SPOT")
            .map(({ currency }) => currency);
        const localSpotPrices = await (0, utils_1.getSpotPricesInUSD)(wantedSpot);
        const pricePromises = Array.from(uniqueCurrencies.values()).map(async ({ currency, type }) => {
            var _a;
            const key = `${currency}-${type}`;
            if (type === "SPOT" && localSpotPrices.size) {
                return { key, price: (_a = localSpotPrices.get(currency)) !== null && _a !== void 0 ? _a : 0 };
            }
        ...
        const unpricedCurrencies = new Set();
        for (const wallet of wallets) {
            const balance = Number(wallet.balance) || 0;
            const inOrder = Number(wallet.inOrder) || 0;
            const settledBalance = balance + inOrder;
            ...
            if (!walletsByType[type]) {
                walletsByType[type] = { count: 0, balance: 0, balanceUSD: 0, pending: 0, pendingUSD: 0 };
            }
            walletsByType[type].count++;
            walletsByType[type].balance += settledBalance;
            walletsByType[type].pending += pendingDeposit;
            const price = priceMap.get(`${wallet.currency}-${type}`) || 0;
            if (!price && (settledBalance > 0 || pendingDeposit > 0)) {
                unpricedCurrencies.add(wallet.currency);
            }
            const balanceUSD = settledBalance * price;
            const pendingUSD = pendingDeposit * price;
            walletsByType[type].balanceUSD += balanceUSD;
            walletsByType[type].pendingUSD += pendingUSD;
        ...
        const [yesterdayPnl, todayPnl] = await Promise.all([
        ...
        return {
            ...
            totalPending: parseFloat(totalPendingUSD.toFixed(2)),
            totalChange: parseFloat(totalChange.toFixed(2)),
            totalChangePercent: parseFloat(totalChangePercent.toFixed(2)),
            totalWallets: wallets.length,
            activeWallets,
            unpricedCurrencies: Array.from(unpricedCurrencies),
            walletsByType: Object.fromEntries(Object.entries(walletsByType).map(([type, data]) => [
                type,
                {
                    ...
                    pending: parseFloat(data.pending.toFixed(8)),
                    pendingUSD: parseFloat(data.pendingUSD.toFixed(2))
                }
            ]))
        };
    }
