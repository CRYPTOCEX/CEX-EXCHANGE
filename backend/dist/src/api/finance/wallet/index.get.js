...
exports.default = async (data) => {
    ...
    const { items: rows, pagination } = (await (0, query_1.getFiltered)({
    ...
    for (const wallet of rows) {
        items.push(await (0, utils_2.markUncreditableChains)((0, utils_2.withoutChainBalances)(wallet)));
    }
...
const handlePnl = async (user) => {
    ...
    const currencyMap = new Map(currencyPrices.map((item) => [item.id, (0, utils_3.fiatUsdPriceFromStored)(item.price)]));
    ...
    const ecoPriceMap = (0, utils_3.indexTickersByBaseAsset)(await engine.getTickers());
    const balances = { FIAT: 0, SPOT: 0, ECO: 0, FUTURES: 0 };
    wallets.forEach((wallet) => {
        let price;
        if (wallet.type === "FIAT") {
            price = currencyMap.get(wallet.currency) || 0;
        }
            ...
            price =
                exchangeMap.get(wallet.currency) ||
                    ecoPriceMap.get(wallet.currency) ||
                    0;
        }
        if (price) {
    ...
    const dailyPnl = pnlRecords.reduce((acc, record) => {
        ...
        acc[dateKey].FUNDING +=
            (((_c = record.balances) === null || _c === void 0 ? void 0 : _c.ECO) || 0) + (((_d = record.balances) === null || _d === void 0 ? void 0 : _d.FUTURES) || 0);
        return acc;
    }, {});
    const pnlChart = [];
