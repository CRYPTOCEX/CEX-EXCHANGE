...
exports.default = async (data) => {
    ...
    try {
        ...
        const wallet = await db_1.models.wallet.findOne({
            where: { userId: user.id, currency: currency.toUpperCase(), type: "FIAT" },
        });
        if (!wallet) {
        ...
        const majorAmount = (0, utils_1.ewayFromMinorUnits)(Number(amount), currencyCode);
        const percentageFee = gateway.getPercentageFee(currencyCode);
        const fixedFee = gateway.getFixedFee(currencyCode);
        const calculatedFee = Math.max((majorAmount * percentageFee) / 100 + fixedFee, 0);
        const totalAmount = (0, utils_1.ewayToMinorUnits)(majorAmount + calculatedFee, currencyCode);
        ...
        const transaction = await db_1.models.transaction.create({
            ...
            type: "DEPOSIT",
            status: "PENDING",
            amount: majorAmount + calculatedFee,
            fee: calculatedFee,
