...
exports.default = async (data) => {
    ...
    try {
        ...
        const checksum = (0, utils_2.generateChecksumHash)(statusRequest.body, utils_2.PAYTM_CONFIG.MERCHANT_KEY);
        statusRequest.body['checksumHash'] = checksum;
        const statusResponse = await (0, utils_2.makePaytmRequest)('/merchant-status/api/v1/getPaymentStatus', {
            method: 'POST',
            body: statusRequest,
            headers: {
        ...
        const txnAmount = (0, utils_2.parsePaytmAmount)(statusResponse.body.txnAmount || '0', metadata.currency || 'INR');
                ...
                await db_1.sequelize.transaction(async (t) => {
                    try {
                        await (0, utils_1.processFiatDeposit)({
                            ...
                            method: 'PAYTM',
                            description: `Paytm deposit - ${statusResponse.body.txnId || orderId}`,
                            metadata: {
                                gateway: 'paytm',
                                txnId: statusResponse.body.txnId,
                                bankTxnId: statusResponse.body.bankTxnId,
                    ...
                    await transaction.update({ status: 'COMPLETED', metadata: updatedMetadata }, { transaction: t });
                });
            }
            else {
                await transaction.update({
