    ...
    try {
        ...
        if (order_id) {
            transaction = await db_1.models.transaction.findOne({
                where: { referenceId: order_id, userId: user.id, type: "DEPOSIT" },
                include: [
                    {
            ...
            transaction = await db_1.models.transaction.findOne({
                where: { referenceId: payment_id, userId: user.id, type: "DEPOSIT" },
                include: [
                    {
                        model: db_1.models.user,
                        as: "user",
        ...
        const dLocalPaymentId = payment_id || (0, utils_3.parseTransactionMetadata)(transaction.metadata).dlocal_payment_id;
        if (!dLocalPaymentId) {
            ...
            await db_1.sequelize.transaction(async (t) => {
                ...
                await transaction.update({
                    ...
                    metadata: JSON.stringify({
                        ...(0, utils_3.parseTransactionMetadata)(transaction.metadata),
                        dlocal_payment_id: paymentData.id,
