...
exports.default = async (data) => {
    ...
    try {
        ...
        if (orderStatus === "COMPLETED" || orderResponse.status === "AUTHORIZED") {
            ...
            const klarnaOrderId = orderResponse.order_id;
            const depositResult = await db_1.sequelize.transaction(async (t) => {
                const res = await (0, utils_2.processFiatDeposit)({
                    userId: user.id,
                    currency,
                    amount: transaction.amount,
                    fee: transactionFee,
                    referenceId: klarnaOrderId,
                    method: "KLARNA",
                    ...
                    idempotencyKey: `klarna_deposit_${klarnaOrderId}`,
                    ctx,
                    transaction: t,
                });
