...
exports.PAYSTACK_CONFIG = {
    ...
    RETURN_URL: process.env.APP_PAYSTACK_RETURN_URL || '/finance/deposit?gateway=paystack&status=returned',
    TIMEOUT: 30000,
    VERSION: 'v1',
};
...
function buildCancelUrl() {
    const baseUrl = process.env.APP_PUBLIC_URL || 'http://localhost:3000';
    return `${baseUrl}/finance/deposit?gateway=paystack&status=cancelled`;
}
function isTestMode() {
...
function validateWebhookSignature(body, signature) {
    try {
        if (!exports.PAYSTACK_CONFIG.SECRET_KEY || !signature) {
            return false;
        }
