            ...
            if (verifyResponse.status && verifyResponse.data) {
                ...
                if (newStatus !== transactionStatus) {
                    if (newStatus === 'COMPLETED' && transactionStatus !== 'COMPLETED') {
                        ...
                        const idempotencyKey = (0, credit_contract_1.depositIdempotencyKey)('paystack', String(paystackTransaction.reference || transaction.referenceId || ''));
                        try {
                            ...
                            if (!(creditError instanceof wallet_1.DuplicateOperationError))
                                throw creditError;
                            console_1.logger.info("PAYSTACK", `Deposit ${reference} already credited (idempotency hit)`);
                        }
                    }
