...
exports.default = async (data) => {
    ...
    try {
        if (!utils_1.PAYU_CONFIG.MERCHANT_KEY || !utils_1.PAYU_CONFIG.MERCHANT_SALT || !utils_1.PAYU_CONFIG.MERCHANT_ID) {
            return { success: true, message: 'PayU not enabled' };
        }
        ...
        if (newStatus === 'REFUNDED') {
            const result = await (0, reversal_1.reverseDepositByReference)({
                ...
                eventReference: String(webhookData.mihpayid || webhookData.txnid || ''),
                reportedAmount: txnAmount > 0 ? txnAmount : undefined,
                metadata: { payuStatus: webhookData.status, mihpayid: webhookData.mihpayid },
            });
            if (result.outcome === "no_wallet") {
                console_1.logger.error("PAYU", `[CRITICAL] refund on ${transaction.referenceId} could not be applied: no wallet. Manual reversal required.`);
            }
            return {
                success: true,
        ...
        try {
            ...
            if (newStatus === 'COMPLETED') {
                ...
                if (txnAmount + 0.01 < transaction.amount) {
                    console_1.logger.error('PAYU', `Under-payment for txn ${webhookData.txnid}: expected=${transaction.amount}, got=${txnAmount}`);
                    await transaction.update({
                        status: 'REJECTED',
                        metadata: JSON.stringify({
                            ...metadata,
                ...
                try {
                    const depositResult = await (0, utils_2.processFiatDeposit)({
                        ...
                        referenceId: webhookData.txnid,
                        method: 'PAYU',
                        description: `PayU deposit - ${transaction.amount} ${currency}`,
                        metadata: {
                            mihpayid: webhookData.mihpayid,
                            paymentMode: webhookData.mode,
                            source: 'webhook',
                        },
                        idempotencyKey: `payu_deposit_${webhookData.txnid}`,
                        transaction: dbTransaction,
                    });
                    ...
                    if (err instanceof wallet_1.DuplicateOperationError) {
                        console_1.logger.info('PAYU', `Deposit ${webhookData.txnid} already credited (idempotency hit)`);
                    }
                    else {
                        throw err;
                    }
                ...
                if (typeof newBalance === 'number') {
                    try {
                        ...
                        console_1.logger.error('PAYU', 'Failed to send confirmation email', emailError);
                    }
                }
            }
            await dbTransaction.commit();
