    ...
    try {
        ...
        try {
            ...
            if (mappedStatus === 'COMPLETED') {
                ...
                const depositAmount = txnAmount - feeAmount;
                const idempotencyKey = `payu_deposit_${txnid}`;
                await wallet_1.walletService.credit({
                    ...
                    description: `PayU deposit of ${depositAmount} ${currency}`,
                    metadata: {
                        method: "PAYU",
                ...
                if (feeAmount > 0) {
                    await (0, fees_1.collectPlatformFee)({
                        ...
                        feeAmount,
                        type: "DEPOSIT",
                        description: `Platform fee from PayU deposit of ${feeAmount} ${currency}`,
                        referenceId: `payu_${txnid}`,
                        metadata: { method: "PAYU", txnid },
                        transaction: dbTransaction,
                    });
                }
                const newBalance = parseFloat(String(wallet.balance)) + depositAmount;
