        ...
        try {
            ...
            if (transaction.status !== mappedStatus) {
                ...
                if (mappedStatus === "COMPLETED" && transaction.status !== "COMPLETED") {
                    const currency = (0, utils_2.parseTransactionMetadata)(transaction.metadata).currency;
                    await db_1.sequelize.transaction(async (t) => {
                        const walletResult = await wallet_1.walletCreationService.getOrCreateWallet(transaction.userId, "FIAT", currency, t);
                        const wallet = walletResult.wallet;
                        const idempotencyKey = `ipay88_deposit_${transaction.referenceId}`;
                        try {
                            await wallet_1.walletService.credit({
                                ...
                                referenceId: (0, credit_contract_1.creditReferenceId)(transaction, reference),
                                description: `iPay88 deposit of ${transaction.amount} ${currency}`,
                                metadata: {
                                    method: "IPAY88",
                            ...
                            if (!(err instanceof wallet_1.DuplicateOperationError))
                                throw err;
                            console_1.logger.info("IPAY88", `Deposit ${reference} already credited (idempotency hit)`);
                        }
            ...
            return {
                ...
                data: {
                    ...
                    payment_method: ((_a = (0, utils_2.parseTransactionMetadata)(transaction.metadata).payment_method_details) === null || _a === void 0 ? void 0 : _a.type) || "unknown",
                    signature_valid: true,
            ...
            return {
                ...
                data: {
                    ...
                    amount: transaction.amount,
                    currency: (0, utils_2.parseTransactionMetadata)(transaction.metadata).currency,
                    gateway: "ipay88",
                    ipay88_transaction_id: (0, utils_2.parseTransactionMetadata)(transaction.metadata).ipay88_transaction_id || "",
                    auth_code: (0, utils_2.parseTransactionMetadata)(transaction.metadata).ipay88_auth_code || "",
                    response_message: "Status check from local database (requery unavailable)",
                    payment_method: ((_b = (0, utils_2.parseTransactionMetadata)(transaction.metadata).payment_method_details) === null || _b === void 0 ? void 0 : _b.type) || "unknown",
                    signature_valid: (0, utils_2.parseTransactionMetadata)(transaction.metadata).signature_valid || false,
                    last_updated: transaction.updatedAt,
