...
exports.default = async (data) => {
    ...
    try {
        ...
        const gateway = await db_1.models.depositGateway.findOne({
            where: { alias: "ipay88", status: true },
        });
        if (!gateway) {
            ctx === null || ctx === void 0 ? void 0 : ctx.fail("iPay88 gateway is not enabled");
            throw (0, error_1.createError)({
                statusCode: 400,
                message: "iPay88 payment gateway is not available",
            });
        }
        const currencyCode = currency.toUpperCase();
        const supportedCurrencies = gateway.currencies || [];
        if (supportedCurrencies.length && !supportedCurrencies.includes(currencyCode)) {
            throw (0, error_1.createError)({
                statusCode: 400,
                message: `Currency ${currency} is not supported by this gateway`,
            });
        }
        ...
        if (maxAmount != null && maxAmount > 0 && amount > maxAmount) {
            throw (0, error_1.createError)({
                statusCode: 400,
                message: `Maximum deposit amount is ${maxAmount} ${currency}`,
            });
        }
