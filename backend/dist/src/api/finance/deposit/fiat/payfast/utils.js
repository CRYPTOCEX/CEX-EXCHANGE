...
exports.PAYFAST_CONFIG = {
    ...
    RETURN_URL: process.env.APP_PAYFAST_RETURN_URL || '/finance/deposit?gateway=payfast&status=returned',
    CANCEL_URL: process.env.APP_PAYFAST_CANCEL_URL || '/finance/deposit?gateway=payfast&status=cancelled',
    TIMEOUT: 30000,
};
...
function validateSignature(data, passphrase) {
    ...
    const providedBuf = Buffer.from(providedSignature.toLowerCase(), 'utf8');
    const calculatedBuf = Buffer.from(calculatedSignature.toLowerCase(), 'utf8');
    if (providedBuf.length !== calculatedBuf.length)
        return false;
    return crypto_1.default.timingSafeEqual(providedBuf, calculatedBuf);
}
