...
exports.default = async (data) => {
    ...
    try {
        if (!utils_1.PAYFAST_CONFIG.PASSPHRASE) {
            console_1.logger.error('PAYFAST', 'ITN rejected: passphrase not configured');
            throw (0, error_1.createError)({
                statusCode: 500,
                message: 'PayFast passphrase not configured',
            });
        }
        ...
        if (!itnValidation.valid) {
            console_1.logger.error('PAYFAST', `ITN validation failed: ${itnValidation.error}`);
            throw (0, error_1.createError)({
                statusCode: 400,
                message: 'ITN validation failed',
            });
        }
        ...
        try {
            ...
            if (newStatus === 'COMPLETED' && currentStatus !== 'COMPLETED') {
                ...
                const idempotencyKey = `payfast_deposit_${transaction.id}`;
