...
exports.default = async (data) => {
    ...
    try {
        // Re-fetch pool with row lock to prevent concurrent stakes from overselling availability
        const lockedPool = await db_1.models.stakingPool.findByPk(poolId, {
            transaction,
            lock: transaction.LOCK.UPDATE,
        });
        if (!lockedPool || lockedPool.status !== "ACTIVE") {
            throw (0, error_1.createError)({ statusCode: 400, message: "Staking pool is no longer active" });
        }
        if (amount > lockedPool.availableToStake) {
            throw (0, error_1.createError)({
                statusCode: 400,
                message: `Insufficient available amount to stake. Available: ${lockedPool.availableToStake}`,
            });
        }
        ...
        lockedPool.availableToStake = lockedPool.availableToStake - amount;
        await lockedPool.save({ transaction });
