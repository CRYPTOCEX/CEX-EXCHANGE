...
exports.default = async (data) => {
    ...
    const { id } = params;
    const fields = ["name", "email", "phone", "street", "city", "state", "postalCode", "country"];
    if (!body.shippingAddress || fields.some(field => typeof body.shippingAddress[field] !== "string" || !body.shippingAddress[field].trim())) throw (0, error_1.createError)({ statusCode: 400, message: "Complete shipping address required" });
    const shippingAddress = Object.fromEntries(fields.map(field => [field, body.shippingAddress[field].trim()]));
    ctx === null || ctx === void 0 ? void 0 : ctx.step("Validating order ID");
    const transaction = await db_1.sequelize.transaction();
    try {
        ctx === null || ctx === void 0 ? void 0 : ctx.step(`Finding order: ${id}`);
        const order = await db_1.models.ecommerceOrder.findByPk(id, { transaction, lock: transaction.LOCK.UPDATE });
        if (!order) {
        ...
        const existingAddress = await db_1.models.ecommerceShippingAddress.findOne({
            where: { orderId: id }, transaction, lock: transaction.LOCK.UPDATE,
        });
        if (existingAddress) {
            ...
            await db_1.models.ecommerceShippingAddress.create({ ...shippingAddress, orderId: id, userId: order.userId }, { transaction });
        }
