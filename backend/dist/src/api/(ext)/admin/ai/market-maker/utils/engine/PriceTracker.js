...
class PriceTracker {
    ...
    addToHistory(price) {
        ...
        while (this.priceHistory.length > 100) {
    ...
    calculateVolatility() {
        ...
        const perSampleSd = Math.sqrt(variance);
        const first = this.priceHistory[0].timestamp.getTime();
        const last = this.priceHistory[this.priceHistory.length - 1].timestamp.getTime();
        const spanMs = last - first;
        if (!(spanMs > 0)) {
            this.volatility = perSampleSd * 100;
            return;
        }
        const meanIntervalMs = spanMs / returns.length;
        const samplesPerDay = 86400000 / meanIntervalMs;
        this.volatility = perSampleSd * Math.sqrt(samplesPerDay) * 100;
    }
    observeEnginePrice(price) {
        if (price <= BigInt(0))
            return;
