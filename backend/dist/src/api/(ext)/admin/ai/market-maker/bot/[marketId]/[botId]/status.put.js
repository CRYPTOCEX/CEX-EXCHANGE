...
exports.default = async (data) => {
    ...
    const minutes = Number(cooldownMinutes) > 0
        ? Number(cooldownMinutes)
        : DEFAULT_COOLDOWN_MINUTES;
    await bot.update({
        status,
        cooldownUntil: status === "COOLDOWN"
    ...
    await db_1.models.aiMarketMakerHistory.create({
        ...
        details: {
            botId: bot.id,
            botName: bot.name,
            reason: `Bot status changed from ${previousStatus} to ${status}${status === "COOLDOWN" ? ` (cooldown: ${minutes} minutes)` : ""}`,
            triggeredBy: "ADMIN",
        },
        priceAtAction: marketMaker.targetPrice,
        poolValueAtAction: 0,
    });
    ctx === null || ctx === void 0 ? void 0 : ctx.step("Apply status change to the running engine");
