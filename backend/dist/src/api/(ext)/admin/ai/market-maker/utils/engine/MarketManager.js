...
class MarketManager {
    ...
    async loadActiveMarkets() {
        try {
            const activeMarkers = (0, market_resolver_1.hydrateMakerMarkets)(await db_1.models.aiMarketMaker.findAll({
                where: {
                    status: "ACTIVE",
                },
                include: [
    ...
    async startMarket(makerOrId) {
        ...
        try {
            ...
            if (!maker) {
                maker = (0, market_resolver_1.hydrateMakerMarket)(await db_1.models.aiMarketMaker.findByPk(marketMakerId, {
                    include: [
                        {
                            model: db_1.models.aiMarketMakerPool,
                            as: "pool",
    ...
    async stopMarket(marketMakerId) {
        ...
        try {
            ...
            if (!instance) {
                console_1.logger.warn("AI_MM", `Market ${marketMakerId} is not loaded in the engine; recording STOPPED anyway`);
                await db_1.models.aiMarketMaker.update({ status: "STOPPED" }, { where: { id: marketMakerId } });
                await this.clearMarketOrderbook(marketMakerId);
                (_b = (_a = this.engine).forgetMarketState) === null || _b === void 0 ? void 0 : _b.call(_a, marketMakerId);
                await this.logMarketHistory(marketMakerId, "STOP", {
                    reason: "Manual stop (market was not loaded in the engine)",
                });
                return true;
            }
            ...
            (_d = (_c = this.engine).forgetMarketState) === null || _d === void 0 ? void 0 : _d.call(_c, marketMakerId);
            await db_1.models.aiMarketMaker.update({ status: "STOPPED" }, { where: { id: marketMakerId } });
    ...
    async pauseMarket(marketMakerId) {
        try {
            const instance = this.markets.get(marketMakerId);
            if (!instance) {
                console_1.logger.warn("AI_MM", `Market ${marketMakerId} is not loaded in the engine; recording PAUSED anyway`);
                ...
                await this.logMarketHistory(marketMakerId, "PAUSE", {
                    reason: "Manual pause (market was not loaded in the engine)",
                });
                return true;
            }
    ...
    async releaseAllMarkets() {
        const marketMakerIds = Array.from(this.markets.keys());
        console_1.logger.info("AI_MM", `Releasing ${marketMakerIds.length} market maker instance(s); every status row is left as the operator set it`);
        await Promise.all(marketMakerIds.map((id) => this.releaseMarket(id)));
    }
    async releaseMarket(marketMakerId) {
        var _a, _b;
        const instance = this.markets.get(marketMakerId);
        if (!instance)
            return;
        ...
        try {
            ...
            console_1.logger.error("AI_MM", `Error releasing market ${marketMakerId}`, error);
        }
        (_b = (_a = this.engine).forgetMarketState) === null || _b === void 0 ? void 0 : _b.call(_a, marketMakerId);
    }
    ...
    async processAllMarkets(tradingAllowed = true) {
    ...
    getErrorPausedMarkets() {
        const paused = [];
        for (const [marketMakerId, instance] of this.markets) {
            const state = instance.getErrorPauseState();
            if (!state.errorPaused)
                continue;
            paused.push({ marketMakerId, symbol: instance.getSymbol(), ...state });
        }
        return paused;
    }
    getPausedMarkets() {
        ...
        for (const [marketMakerId, instance] of this.markets) {
            if (instance.getStatus() !== "PAUSED")
                continue;
            if (instance.isErrorPaused())
                continue;
            paused.push({ marketMakerId, symbol: instance.getSymbol() });
        }
        return paused;
    }
    getMarketIds() {
    ...
    async refreshAllMarkets() {
        ...
        for (const [marketMakerId, instance] of this.markets) {
            try {
                const makerData = (0, market_resolver_1.hydrateMakerMarket)(await db_1.models.aiMarketMaker.findByPk(marketMakerId, {
                    include: [
    ...
    async refreshMarket(marketMakerId) {
        const instance = this.markets.get(marketMakerId);
        if (!instance)
            return false;
        try {
            ...
            if (!makerData)
                return false;
            instance.updateConfig(makerData);
            console_1.logger.info("AI_MM", `Applied live config update to market ${marketMakerId}`);
            return true;
        }
        catch (error) {
            console_1.logger.error("AI_MM", `Live config refresh failed for ${marketMakerId}`, error);
            return false;
        }
    }
    ...
    async clearMarketOrderbook(marketMakerId) {
        try {
            const maker = (0, market_resolver_1.hydrateMakerMarket)(await db_1.models.aiMarketMaker.findByPk(marketMakerId, {
                include: [...(0, market_resolver_1.makerMarketIncludes)()],
            }));
            const market = maker === null || maker === void 0 ? void 0 : maker.market;
            if (!(market === null || market === void 0 ? void 0 : market.currency) || !(market === null || market === void 0 ? void 0 : market.pair))
                return;
            const { clearOrderbookForSymbol } = await Promise.resolve().then(() => __importStar(require("../scylla/queries")));
            await clearOrderbookForSymbol((0, market_resolver_1.marketSymbol)(market.currency, market.pair), (0, market_resolver_1.normaliseVenue)(maker === null || maker === void 0 ? void 0 : maker.marketType));
        }
        catch (error) {
            console_1.logger.debug("AI_MM", `Could not clear the orderbook for ${marketMakerId}; levels will expire via TTL`, error);
        }
    }
