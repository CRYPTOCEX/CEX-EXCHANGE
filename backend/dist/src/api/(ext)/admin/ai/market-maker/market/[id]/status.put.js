...
exports.default = async (data) => {
    ...
    if (action === "START") {
        ...
        if (minLiquidity > 0 && quoteBalance < minLiquidity) {
            const market = await (0, market_resolver_1.loadMarketForMaker)(marketMaker);
        ...
        case "START":
            ...
            if (marketManager) {
                ...
                if (!success) {
                    ...
                    await db_1.models.aiBot.update({ status: "PAUSED" }, { where: { marketMakerId: marketMaker.id } });
                }
            }
            break;
        case "PAUSE":
    ...
    const engineAttached = !!marketManager;
    const pendingStart = !engineAttached && (action === "START" || action === "RESUME");
    return {
        message: pendingStart
            ? `AI Market Maker recorded as ${newStatus}, but the market-making engine is not running in this process yet. ` +
                `It will begin on the next engine cycle.`
            : `AI Market Maker ${(_c = past[action]) !== null && _c !== void 0 ? _c : action.toLowerCase()} successfully`,
        status: newStatus,
        engineAttached,
    };
};
