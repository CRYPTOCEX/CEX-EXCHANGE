...
class AiMarketMakerDataHandler {
    ...
    async sendInitialData(marketMakerId) {
        ...
        const errors = [];
        try {
            const marketMaker = (0, market_resolver_1.hydrateMakerMarket)(await db_1.models.aiMarketMaker.findByPk(marketMakerId, {
                include: [
        ...
        const broadcastData = {
            ...
            data: {
                ...
                lastKnownPrice: makerData.lastKnownPrice,
                currentPhase: makerData.currentPhase,
                phaseStartedAt: makerData.phaseStartedAt,
                nextPhaseChangeAt: makerData.nextPhaseChangeAt,
                phaseTargetPrice: makerData.phaseTargetPrice,
                marketBias: makerData.marketBias,
                biasStrength: makerData.biasStrength,
                trendMomentum: makerData.trendMomentum,
                priceMode: makerData.priceMode,
                externalSymbol: makerData.externalSymbol,
                correlationStrength: makerData.correlationStrength,
                baseVolatility: makerData.baseVolatility,
                volatilityMultiplier: makerData.volatilityMultiplier,
                assessment: (0, assessment_1.assessMarketMaker)({
                    status: makerData.status,
                    lastKnownPrice: makerData.lastKnownPrice,
                    targetPrice: makerData.targetPrice,
                    priceRangeLow: makerData.priceRangeLow,
                    ...
                    currentDailyVolume: makerData.currentDailyVolume,
                    realLiquidityPercent: makerData.realLiquidityPercent,
                    activeBots: (makerData.bots || []).filter((bot) => bot.status === "ACTIVE").length,
                    pool: makerData.pool
                        ? {
                            ...
                            initialBaseBalance: makerData.pool.initialBaseBalance,
                            initialQuoteBalance: makerData.pool.initialQuoteBalance,
                        }
                        : null,
                }),
...
exports.default = async (data, message) => {
    ...
    if (action === "SUBSCRIBE") {
        ...
        if (!(0, Websocket_1.hasClientOnRoute)(data.path, clientId)) {
            handler.removeSubscription(marketMakerId, clientId);
        }
    }
};
const onClose = (ws, route, clientId) => {
    if (!(0, Websocket_1.isLastSocketForClient)(route, clientId, ws))
        return;
    const handler = AiMarketMakerDataHandler.getInstance();
