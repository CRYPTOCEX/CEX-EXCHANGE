...
exports.metadata = {
    ...
    requiresAuth: true,
    permission: "edit.ai.market_maker.bot",
};
exports.default = async (data) => {
    ...
    if (riskTolerance !== undefined) {
        if (!(riskTolerance >= 0.1) || riskTolerance > 1) {
            throw (0, error_1.createError)(400, "Risk tolerance must be between 0.1 and 1");
        }
    }
    if (orderSizeVariance !== undefined) {
        if (!(orderSizeVariance >= 0.1) || orderSizeVariance > 0.5) {
            throw (0, error_1.createError)(400, "Order size variance must be between 0.1 and 0.5 (50%)");
        }
    }
    if (avgOrderSize !== undefined && !(avgOrderSize > 0)) {
        throw (0, error_1.createError)(400, "Average order size must be greater than 0");
    }
    if (maxDailyTrades !== undefined && !(maxDailyTrades > 0)) {
        throw (0, error_1.createError)(400, "Max daily trades must be greater than 0");
    }
    if (tradeFrequency !== undefined &&
        !["HIGH", "MEDIUM", "LOW"].includes(tradeFrequency)) {
        throw (0, error_1.createError)(400, "Trade frequency must be HIGH, MEDIUM or LOW");
    }
    if (preferredSpread !== undefined && !(preferredSpread > 0)) {
        throw (0, error_1.createError)(400, "Preferred spread must be greater than 0");
    }
    ...
    if (orderSizeVariance !== undefined &&
        orderSizeVariance !== Number(bot.orderSizeVariance)) {
        changes.orderSizeVariance = {
            old: bot.orderSizeVariance,
            new: orderSizeVariance,
        };
    }
    if (preferredSpread !== undefined &&
        preferredSpread !== Number(bot.preferredSpread)) {
        changes.preferredSpread = { old: bot.preferredSpread, new: preferredSpread };
    }
    ...
    ctx === null || ctx === void 0 ? void 0 : ctx.step("Apply configuration to the running engine");
    await (0, liveConfig_1.applyConfigToRunningEngine)(params.marketId);
