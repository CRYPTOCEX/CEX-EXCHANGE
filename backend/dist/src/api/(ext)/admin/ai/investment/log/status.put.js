...
exports.default = async (data) => {
    ...
    for (const id of ids) {
        try {
            const outcome = await (0, settle_1.applyAdminInvestmentStatus)(id, status, ctx);
            ...
            console_1.logger.error("ADMIN_AI", `Bulk status change to ${status} failed for investment ${id}: ${error === null || error === void 0 ? void 0 : error.message}`);
        }
    }
    if (settled.length === 0 && failed.length > 0) {
        throw (0, error_1.createError)({
            ...
            message: `No investments could be set to ${status}. ` +
                failed.map((f) => `${f.id.slice(0, 8)}: ${f.reason}`).join("; "),
        });
    }
    ctx === null || ctx === void 0 ? void 0 : ctx.success(`Status updated for ${settled.length} of ${ids.length} investment(s)` +
