...
exports.default = async (data) => {
    ...
    const [investmentStats, statusStats, resultStats, typeStats, planStats, durationStats,] = await Promise.all([
        db_1.models.aiInvestment.findOne({
            attributes: [
                ...
                [
                    (0, sequelize_1.literal)("COALESCE(SUM(CASE WHEN result = 'WIN' THEN profit " +
                        "WHEN result = 'LOSS' THEN -profit ELSE 0 END), 0)"),
                    "totalProfit",
                ],
    ...
    const recentInvestments = recentInvestmentsRaw.map((inv) => { var _a, _b; return ({
        id: inv.id,
