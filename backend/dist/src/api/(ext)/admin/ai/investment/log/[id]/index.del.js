...
exports.default = async (data) => {
    ...
    const preDelete = async () => {
        ctx === null || ctx === void 0 ? void 0 : ctx.step("Checking whether a refund is owed");
        const investment = await db_1.models.aiInvestment.findByPk(params.id, {
            paranoid: false,
        });
        if (!investment) {
            ctx === null || ctx === void 0 ? void 0 : ctx.warn(`Investment not found: ${params.id}`);
            return;
        }
        if (investment.status !== "ACTIVE" || investment.deletedAt) {
            ctx === null || ctx === void 0 ? void 0 : ctx.step(`No refund owed: investment is ${investment.status}` +
                (investment.deletedAt ? " and already cancelled" : ""));
            return;
        }
        ctx === null || ctx === void 0 ? void 0 : ctx.step("Processing wallet refund");
        const transaction = await db_1.models.transaction.findOne({
            where: { referenceId: params.id, type: "AI_INVESTMENT" },
            include: [{ model: db_1.models.wallet, as: "wallet" }],
        });
        if (!transaction) {
            ctx === null || ctx === void 0 ? void 0 : ctx.warn(`Funding transaction not found for id: ${params.id}`);
            return;
        }
        ...
        await wallet_1.walletService.credit({
            ...
            referenceId: `${params.id}_admin_refund`,
    ...
    const postDelete = async () => {
        if (externalData.transactionId) {
            ctx === null || ctx === void 0 ? void 0 : ctx.step("Preserving the funding transaction for audit");
        }
    };
