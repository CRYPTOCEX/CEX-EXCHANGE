...
exports.metadata = {
    ...
    requiresAuth: true,
    permission: "delete.copy_trading",
            ...
            for (const follower of followers) {
                ...
                if (allocations.length > 0) {
                    ...
                    for (const allocation of allocations) {
                        ...
                        if (alloc.marketType === "BINARY") {
                            if (quoteToRefund > 0) {
                                const transferResult = await wallet_2.walletService.transfer({
                                    idempotencyKey: `ct_admin_leader_delete_quote_${id}_${alloc.id}`,
                                    ...
                                    description: `Refund: Leader deleted by admin - ${quoteToRefund} ${quoteCurrency} from ${alloc.symbol} (BINARY)`,
                                    metadata: {
                        ...
                        if (baseToRefund > 0) {
                            const baseWallet = await (0, wallet_1.getWalletByUserIdAndCurrency)(followerData.userId, baseCurrency, "COPY_TRADING");
                            if (baseWallet) {
                                const transferResult = await wallet_2.walletService.transfer({
                                    idempotencyKey: `ct_admin_leader_delete_base_${id}_${alloc.id}`,
                                    fromUserId: followerData.userId,
                                    toUserId: followerData.userId,
                                    fromWalletType: "COPY_TRADING",
                                    toWalletType: "ECO",
                                ...
                                const balanceBefore = transferResult.toResult.previousBalance;
                                await db_1.models.copyTradingTransaction.create({
                                    userId: followerData.userId,
                                    leaderId: id,
                                    followerId: followerData.id,
                        ...
                        if (quoteToRefund > 0) {
                            const quoteWallet = await (0, wallet_1.getWalletByUserIdAndCurrency)(followerData.userId, quoteCurrency, "COPY_TRADING");
                            if (quoteWallet) {
                                const transferResult = await wallet_2.walletService.transfer({
                                    idempotencyKey: `ct_admin_leader_delete_quote_${id}_${alloc.id}`,
                                    fromUserId: followerData.userId,
