...
exports.default = async (data) => {
    try {
        ...
        return {
            ...
            currencyDist: {
                rows: currencyDist.map((c) => ({ currency: c.currency, count: Number(c.count) })),
                limit: 10,
                total: activeOffers,
                other: Math.max(0, activeOffers - currencyDist.reduce((sum, c) => sum + Number(c.count), 0)),
            },
            ...
            disputeQueue: recentDisputes.map((d) => ({
                id: d.id,
                reason: d.reason,
                status: d.status,
                priority: d.priority,
