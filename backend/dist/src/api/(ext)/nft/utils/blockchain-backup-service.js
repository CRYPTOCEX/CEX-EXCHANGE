...
class BlockchainBackupService {
    ...
    async createBackup(includeDisputes = true) {
        ...
        try {
            ...
            const compressed = await this.gzip(backupJson);
    ...
    async createIncrementalBackup() {
        ...
        try {
            ...
            try {
                ...
                console_1.logger.warn("NFT_BACKUP", `Could not read block number for incremental backup: ${error.message}`);
            }
            incrementalData.metadata = {
                ...
                encrypted: !!this.encryptionKey,
                compressed: true,
                dataTypes: Object.keys(incrementalData).filter(k => k !== "metadata"),
                recordCount: {
                    contracts: incrementalData.contracts.length,
                    listings: incrementalData.listings.length,
                    tokens: incrementalData.tokens.length,
                    transactions: incrementalData.transactions.length,
                    disputes: ((_a = incrementalData.disputes) === null || _a === void 0 ? void 0 : _a.length) || 0,
                },
            };
            const recordTotal = incrementalData.contracts.length +
                incrementalData.listings.length +
                incrementalData.tokens.length;
            if (recordTotal === 0) {
                console_1.logger.warn("NFT_BACKUP", `Incremental backup for ${this.chain} captured 0 changed records since ${lastTimestamp.toISOString()}`);
            }
            const filename = this.generateBackupFilename(undefined, "incremental");
            const compressed = await this.prepareBackupData(incrementalData);
            const localPath = await this.saveLocalBackup(filename, compressed);
            if (this.s3Client) {
                await this.uploadToS3(filename, compressed, incrementalData.metadata);
            }
            await this.saveBackupRecord(filename, incrementalData.metadata, localPath);
            await this.cleanOldBackups();
    ...
    async verifyStateConsistency() {
        try {
            ...
            const contracts = await db_1.models.nftCollection.findAll({
                where: { contractAddress: { [sequelize_1.Op.not]: null } },
            });
            const provider = await getProvider(this.chain);
    ...
    async collectBackupData(includeDisputes) {
        ...
        data.contracts = await this.fetchAllPaged(db_1.models.nftCollection, {
            attributes: { exclude: ["createdAt", "updatedAt"] },
        });
        data.tokens = await this.fetchAllPaged(db_1.models.nftToken, {
            attributes: { exclude: ["createdAt", "updatedAt"] },
        });
        data.listings = await this.fetchAllPaged(db_1.models.nftListing, {
            attributes: { exclude: ["createdAt", "updatedAt"] },
        });
        data.transactions = [];
        if (includeDisputes && db_1.models.nftDispute) {
            data.disputes = await this.fetchAllPaged(db_1.models.nftDispute, {
                attributes: { exclude: ["createdAt", "updatedAt"] },
            });
        }
        ...
        if (db_1.models.gasHistory) {
            data.gasHistory = await db_1.models.gasHistory.findAll({
                where: {
                    chain: this.chain,
                    createdAt: { [sequelize_1.Op.gte]: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) },
                },
                raw: true,
            });
        }
    ...
    async fetchAllPaged(model, options = {}, pageSize = 1000) {
        const rows = [];
        let lastId = null;
        for (;;) {
            const where = lastId
                ? { ...(options.where || {}), id: { [sequelize_1.Op.gt]: lastId } }
                : options.where;
    ...
    async prepareBackupData(data) {
        ...
        return this.gzip(jsonString);
    }
    gzip(data) {
        const zlib = require("zlib");
        return new Promise((resolve, reject) => {
            zlib.gzip(data, (error, result) => error ? reject(error) : resolve(result));
        });
    }
    gunzip(data) {
        const zlib = require("zlib");
    ...
    async listBackupFilesNewestFirst() {
        const names = (await fs.promises.readdir(this.backupPath)).filter(f => f.startsWith(`nft-backup-${this.chain}`));
        const files = [];
        for (const filename of names) {
            try {
                const stats = await fs.promises.stat(path.join(this.backupPath, filename));
    ...
    async listLocalBackups(limit) {
        const files = await this.listBackupFilesNewestFirst();
        return files.slice(0, limit).map(({ filename, stats }) => ({
            filename,
            size: stats.size,
    ...
    async getLastBackupMetadata() {
        ...
        if (lastBackup.metadata) {
            return this.usableMetadata(lastBackup.metadata);
        }
        try {
            const data = await this.loadBackup(lastBackup.filename || lastBackup.Key);
            return this.usableMetadata(data.metadata);
        }
        catch (_a) {
            return null;
        }
    }
    usableMetadata(metadata) {
        let parsed = metadata;
    ...
    async cleanOldBackups(keepCount = 30) {
        try {
            ...
            if (files.length > keepCount) {
                const newestFull = files.find(f => f.filename.startsWith(`nft-backup-${this.chain}-full-`));
                const toDelete = files
                    .slice(keepCount)
                    .filter(f => f.filename !== (newestFull === null || newestFull === void 0 ? void 0 : newestFull.filename));
                for (const { filename } of toDelete) {
                    await fs.promises.unlink(path.join(this.backupPath, filename));
                    console_1.logger.debug("NFT_BACKUP", `Deleted old backup: ${filename}`);
                }
            }
            if (this.s3Client) {
            }
