...
const AUCTION_SETTLE_CONFIRMATION_TIMEOUT_MS = 120000;
class NFTAuctionService {
    ...
    async settleAuction(auctionContractAddress) {
        try {
            ...
            let receipt;
            try {
                receipt = await tx.wait(1, AUCTION_SETTLE_CONFIRMATION_TIMEOUT_MS);
            }
            catch (waitError) {
                if ((waitError === null || waitError === void 0 ? void 0 : waitError.code) === "TIMEOUT") {
                    console_1.logger.error("AUCTION_SERVICE", `endAuction tx ${tx.hash} for auction ${auctionContractAddress} did not confirm within ${AUCTION_SETTLE_CONFIRMATION_TIMEOUT_MS}ms; verify it on chain before retrying settlement`);
                }
                throw waitError;
            }
            if (!receipt) {
