...
exports.default = async (data) => {
    ...
    await (0, kyc_1.assertKycFeature)(user.id, kyc_1.KYC_FEATURES.TRANSFER_NFT, "transfer an NFT");
    if (!id) {
    ...
    const senderWalletAddress = await (0, nft_auth_1.requireWallet)(user.id, "transfer an NFT");
    try {
        ...
        if (transactionHash && !executeTransfer) {
            ...
            transferResult = await blockchainService.transferNFT(token.collection.contractAddress, senderWalletAddress, toAddress, token.blockchainTokenId, token.collection.standard);
