...
exports.default = async (data) => {
    ...
    await (0, kyc_1.assertKycFeature)(user.id, kyc_1.KYC_FEATURES.CREATE_NFT, "create a collection");
    ...
    const { CacheManager: RoyaltyCacheManager } = await Promise.resolve().then(() => __importStar(require("@b/utils/cache")));
    const royaltySettings = await RoyaltyCacheManager.getInstance().getSettings();
    const parsedMaxRoyalty = parseFloat((_d = royaltySettings.get("nftMaxRoyaltyPercentage")) !== null && _d !== void 0 ? _d : "");
    const maxRoyaltyPercentage = Number.isFinite(parsedMaxRoyalty) && parsedMaxRoyalty >= 0 ? parsedMaxRoyalty : 10;
    if (royaltyPercentage !== undefined &&
        (typeof royaltyPercentage !== "number" ||
            royaltyPercentage < 0 ||
            royaltyPercentage > maxRoyaltyPercentage)) {
        validationErrors.push(`Royalty percentage must be a number between 0 and ${maxRoyaltyPercentage}`);
    }
    ...
    try {
        ...
        if (!creator) {
            const actor = await (0, nft_auth_1.loadActor)(user.id);
            const creatorUsername = actor.firstName
                ? `${actor.firstName}${actor.lastName || ''}`.toLowerCase().replace(/\s+/g, '')
