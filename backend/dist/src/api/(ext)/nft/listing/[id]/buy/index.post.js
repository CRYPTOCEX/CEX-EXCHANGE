...
exports.default = async (data) => {
    ...
    (0, storefront_1.assertPurchaseAllowedOnNativeApp)(data, "buy items");
    ...
    const buyerWalletAddress = await (0, nft_auth_1.requireWallet)(user.id, "buy an NFT");
    if (!id) {
    ...
    try {
        ...
        const requireKycForHighValue = (0, settlement_1.settingEnabled)(settings.get('nftRequireKycForHighValue'), false);
        const highValueThreshold = (_c = settings.get('nftHighValueThreshold')) !== null && _c !== void 0 ? _c : 1000;
        if (requireKycForHighValue && highValueThreshold > 0) {
            const salePriceUSD = await (0, price_converter_1.convertToUSD)(salePrice, (_d = listing.currency) !== null && _d !== void 0 ? _d : "ETH");
            if (salePriceUSD && salePriceUSD >= highValueThreshold) {
                await (0, kyc_1.assertKycFeatureOrLegacy)(user.id, kyc_1.KYC_FEATURES.BUY_NFT, { requireVerified: true }, "complete this high-value purchase");
            }
        }
        try {
            await verification_1.TransactionVerificationService.validateNFTTransaction(transactionHash, listing.token.collection.chain || "ETH", "purchase", {
                ...
                expectedSender: buyerWalletAddress,
        ...
        ctx === null || ctx === void 0 ? void 0 : ctx.step("Claim listing for purchase");
        const [claimedRows] = await db_1.models.nftListing.update({ status: "SOLD", soldAt: new Date() }, { where: { id: listing.id, status: "ACTIVE" } });
        if (claimedRows === 0) {
            throw (0, error_1.createError)({
                statusCode: 409,
                message: "Listing is no longer available",
        ...
        if (!useMarketplace) {
            try {
                ...
                transferResult = await blockchainService.executePurchase(listing.token.collection.contractAddress, listing.seller.walletAddress, buyerWalletAddress, listing.token.blockchainTokenId, salePrice.toString(), listing.token.collection.standard || "ERC721");
                ...
                try {
                    await db_1.models.nftListing.update({ status: "ACTIVE", soldAt: null }, { where: { id: listing.id, status: "SOLD" } });
                }
                catch (revertError) {
                    console_1.logger.error("NFT_PURCHASE", "Failed to revert listing claim after on-chain failure", revertError);
                }
                throw (0, error_1.createError)({
                    statusCode: 500,
        ...
        try {
            ...
            const sale = await db_1.models.nftSale.create({
                ...
                buyerId: user.id,
                price: salePrice,
                currency: (_e = listing.currency) !== null && _e !== void 0 ? _e : "ETH",
                ...
                transactionHash,
                blockNumber: (_f = transferResult.blockNumber) !== null && _f !== void 0 ? _f : 0,
                status: "COMPLETED",
                metadata: {
            ...
            await db_1.models.nftActivity.create({
                ...
                toUserId: user.id,
                price: salePrice,
                currency: (_g = listing.currency) !== null && _g !== void 0 ? _g : "ETH",
                transactionHash,
                metadata: JSON.stringify({
                    saleId: sale.id,
            ...
            try {
                const suppressed = (0, mobile_forbidden_1.isRewardSuppressedOnNativeApp)(data, "NFT_PURCHASE");
                if (!suppressed)
                    await (0, affiliate_1.processRewards)(user.id, salePrice !== null && salePrice !== void 0 ? salePrice : 0, "NFT_PURCHASE", (_j = listing.currency) !== null && _j !== void 0 ? _j : "ETH", `NFT_PURCHASE:nft_listing:${listing.id}:buyer`);
                if (!suppressed)
                    await (0, affiliate_1.processRewards)(listing.sellerId, sellerAmount, "NFT_SALE", (_k = listing.currency) !== null && _k !== void 0 ? _k : "ETH", `NFT_SALE:nft_listing:${listing.id}:seller`);
            }
            ...
            if ((error === null || error === void 0 ? void 0 : error.name) === "SequelizeUniqueConstraintError") {
                throw (0, error_1.createError)({
                    statusCode: 409,
                    message: "This transaction has already been processed (duplicate sale)",
                });
            }
            throw error;
        }
