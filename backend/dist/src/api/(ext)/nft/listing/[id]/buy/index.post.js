...
exports.default = async (data) => {
    ...
    try {
        ctx === null || ctx === void 0 ? void 0 : ctx.step("Atomically reserving listing");
        // Atomic compare-and-swap: only one buyer can flip status from ACTIVE to PENDING_PURCHASE.
        // This prevents two concurrent buyers from both passing the status check.
        const [updatedRows] = await db_1.models.nftListing.update({ status: "PENDING_PURCHASE" }, { where: { id, status: "ACTIVE" } });
        if (updatedRows === 0) {
            throw (0, error_1.createError)({ statusCode: 409, message: "Listing is no longer available (already reserved or sold)" });
        }
        ...
        if (listing.status !== "PENDING_PURCHASE") {
            // We atomically reserved this listing as PENDING_PURCHASE above; if it's any other state
            // here, something went wrong (concurrent admin cancel, etc.). Refuse to proceed.
            throw (0, error_1.createError)({
                statusCode: 409,
        ...
        try {
            ...
            try {
                // Pass sourceId so reward dedup is sale-scoped, not time-window-scoped.
                // Without it, two legit purchases within 60 s collapse to one reward.
                await (0, affiliate_1.processRewards)(user.id, salePrice !== null && salePrice !== void 0 ? salePrice : 0, "NFT_PURCHASE", (_m = listing.currency) !== null && _m !== void 0 ? _m : "ETH", `NFT_PURCHASE:nft_sale:${sale.id}:buyer`);
                await (0, affiliate_1.processRewards)(listing.sellerId, sellerAmount, "NFT_SALE", (_o = listing.currency) !== null && _o !== void 0 ? _o : "ETH", `NFT_SALE:nft_sale:${sale.id}:seller`);
            }
        ...
        // Revert the PENDING_PURCHASE reservation so the listing becomes available again.
        // Only revert from PENDING_PURCHASE — never from SOLD (that would re-expose a sold listing).
        try {
            await db_1.models.nftListing.update({ status: "ACTIVE" }, { where: { id, status: "PENDING_PURCHASE" } });
        }
        catch (revertError) {
            console_1.logger.error("NFT_PURCHASE_ERROR", `Failed to revert listing ${id} reservation: ${revertError.message}`);
        }
        if (error.statusCode) {
            throw error;
        }
