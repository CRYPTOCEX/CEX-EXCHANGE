...
exports.default = async (data) => {
    ...
    (0, storefront_1.assertPurchaseAllowedOnNativeApp)(data, "list items for sale");
    const { user, body, ctx } = data;
    ctx === null || ctx === void 0 ? void 0 : ctx.step("Validate user authorization");
    if (!(user === null || user === void 0 ? void 0 : user.id)) {
        throw (0, error_1.createError)({ statusCode: 401, message: "Unauthorized" });
    }
    await (0, kyc_1.assertKycFeature)(user.id, kyc_1.KYC_FEATURES.SELL_NFT, "list an NFT for sale");
    ...
    try {
        ...
        if (type === "FIXED_PRICE") {
            const fixedPriceSalesEnabled = (0, settlement_1.settingEnabled)(settings.get('nftEnableFixedPriceSales'), true);
        ...
        if (type === "AUCTION") {
            const auctionsEnabled = (0, settlement_1.settingEnabled)(settings.get('nftEnableAuctions'), true);
            if (!auctionsEnabled) {
                throw (0, error_1.createError)({
                    statusCode: 403,
        ...
        const listingFee = (_b = settings.get('nftListingFee')) !== null && _b !== void 0 ? _b : 0;
        ...
        const requireMetadataValidation = (0, settlement_1.settingEnabled)(settings.get('nftRequireMetadataValidation'), false);
        ...
        if (type === "AUCTION") {
            ...
            const minAuctionDuration = (_c = settings.get('nftMinAuctionDuration')) !== null && _c !== void 0 ? _c : 3600;
            const maxAuctionDuration = (_d = settings.get('nftMaxAuctionDuration')) !== null && _d !== void 0 ? _d : 604800;
            ...
            const bidIncrementPercentage = (_e = settings.get('nftBidIncrementPercentage')) !== null && _e !== void 0 ? _e : 5;
            ...
            const auctionCollection = await db_1.models.nftCollection.findByPk(token.collectionId, { attributes: ["id", "contractAddress"] });
            if (!(auctionCollection === null || auctionCollection === void 0 ? void 0 : auctionCollection.contractAddress)) {
                throw (0, error_1.createError)({
                    statusCode: 400,
                    message: "This collection's contract has not been deployed yet, so an auction on it could never be settled. Deploy the collection contract first, or list at a fixed price.",
                });
            }
            if (!token.blockchainTokenId) {
                throw (0, error_1.createError)({
                    statusCode: 400,
                    message: "This token has not been minted on chain yet, so an auction on it could never be settled. Mint it first, or list at a fixed price.",
                });
            }
        }
        if (type === "BUNDLE") {
        ...
        const marketplaceFeePercentage = (_f = settings.get('nftMarketplaceFeePercentage')) !== null && _f !== void 0 ? _f : 2.5;
        const maxRoyaltyPercentage = (_g = settings.get('nftMaxRoyaltyPercentage')) !== null && _g !== void 0 ? _g : 10;
        ...
        if (error.name === 'SequelizeValidationError') {
            const errorMessages = ((_a = error.errors) === null || _a === void 0 ? void 0 : _a.map(e => `${e.path}: ${e.message}`).join(', ')) || 'Unknown validation error';
            throw (0, error_1.createError)({
                statusCode: 400,
