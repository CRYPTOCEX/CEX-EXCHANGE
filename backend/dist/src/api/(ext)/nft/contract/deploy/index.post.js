...
exports.default = async (data) => {
    try {
        ...
        await (0, kyc_1.assertKycFeature)(user.id, kyc_1.KYC_FEATURES.DEPLOY_NFT_CONTRACT, "deploy a contract");
        const deployerActor = await (0, nft_auth_1.loadActor)(user.id);
        const deployerWalletAddress = deployerActor.walletAddress;
        ...
        const deploymentResult = await deployNftContract(masterWallet, chain, standard, name, symbol, baseURI || "", maxSupply || 0, Math.floor((royaltyPercentage || 0) * 100), deployerWalletAddress || masterWallet.address, mintPrice ? parseFloat(mintPrice.toString()) : 0, isPublicMint || false, deployerWalletAddress || masterWallet.address);
