...
const settlement_1 = require("../../../utils/settlement");
const nft_auth_1 = require("../../../utils/nft-auth");
...
exports.default = async (data) => {
    ...
    try {
            ...
            include: [
                {
                    ...
                    as: "token",
                    attributes: ["id", "name", "blockchainTokenId", "collectionId", "ownerId"],
                    include: [
                        {
        ...
        const isAdmin = await (0, nft_auth_1.isPlatformAdmin)(user.id);
        if (listing.token.ownerId !== user.id &&
            listing.sellerId !== user.id &&
            !isAdmin) {
            throw (0, error_1.createError)({
                statusCode: 403,
                message: "Not authorized to settle this auction"
            });
        }
        ...
        if (highestBid < reservePrice) {
            ...
            try {
                ...
                await (0, transaction_1.rollbackIfActive)(dbTransaction);
                throw dbError;
            }
        }
        ...
        ctx === null || ctx === void 0 ? void 0 : ctx.step("Claiming auction for settlement");
        const [claimedRows] = await db_1.models.nftListing.update({ status: "SOLD", soldAt: new Date() }, { where: { id, status: "ACTIVE" } });
        if (claimedRows === 0) {
            throw (0, error_1.createError)({
                statusCode: 409,
                message: "Auction is already being settled or has been settled"
            });
        }
        ...
        let transferResult;
        try {
            settlementResult = await auctionService.settleAuction(listing.auctionContractAddress);
            ...
            transferResult = await blockchainService.transferNFT(listing.token.collection.contractAddress, listing.seller.walletAddress, winningBid.user.walletAddress, listing.token.blockchainTokenId, listing.token.collection.standard);
            ...
            try {
                await db_1.models.nftListing.update({ status: "ACTIVE", soldAt: null }, { where: { id, status: "SOLD" } });
            }
            catch (revertError) {
                console_1.logger.error("SETTLE_AUCTION", "Failed to revert auction claim after on-chain failure", revertError);
            }
            throw onChainError;
        }
        ...
        const dbTransaction = await db_1.sequelize.transaction();
        try {
            const settleSettings = await cache_1.CacheManager.getInstance().getSettings();
            const feePercent = (0, settlement_1.marketplaceFeePercent)(settleSettings);
            const maxRoyaltyPercent = parseFloat((_c = settleSettings.get("nftMaxRoyaltyPercentage")) !== null && _c !== void 0 ? _c : "10");
            const royaltyPercentage = Math.min(Number(listing.token.collection.royaltyPercentage) || 0, Number.isFinite(maxRoyaltyPercent) ? maxRoyaltyPercent : 10);
            const winningAmount = Number(winningBid.amount);
            const auctionMarketplaceFee = winningAmount * (feePercent / 100);
            const auctionRoyaltyFee = winningAmount * (royaltyPercentage / 100);
            const sale = await db_1.models.nftSale.create({
                ...
                marketplaceFee: auctionMarketplaceFee,
                royaltyFee: auctionRoyaltyFee,
                totalFee: auctionMarketplaceFee + auctionRoyaltyFee,
                netAmount: winningAmount - auctionMarketplaceFee - auctionRoyaltyFee,
                transactionHash: settlementResult.transactionHash,
                blockNumber: (_d = settlementResult.blockNumber) !== null && _d !== void 0 ? _d : 0,
                status: "COMPLETED",
                metadata: {
                    saleType: "AUCTION",
            ...
            await (0, transaction_1.rollbackIfActive)(dbTransaction);
            console_1.logger.error("SETTLE_AUCTION", `CRITICAL: auction ${id} settled ON-CHAIN (settle tx ${settlementResult === null || settlementResult === void 0 ? void 0 : settlementResult.transactionHash}, transfer tx ${transferResult === null || transferResult === void 0 ? void 0 : transferResult.transactionHash}) but the DB write failed — listing left SOLD, ownership/ledger NOT updated. Manual reconciliation required.`, dbError);
            if ((dbError === null || dbError === void 0 ? void 0 : dbError.name) === "SequelizeUniqueConstraintError") {
                throw (0, error_1.createError)({
                    statusCode: 409,
                    message: "This auction has already been settled (duplicate sale)",
                });
            }
            throw dbError;
        }
