...
exports.default = async (data) => {
    ...
    try {
        ...
        const heldAmount = await (0, settlement_1.escrowHeldFor)(offer.id);
        const escrowAmount = heldAmount !== null && heldAmount !== void 0 ? heldAmount : Number(offer.amount) * (1 + feePercent / 100);
        ...
        await db_1.sequelize.transaction(async (transaction) => {
            ...
            try {
                await wallet_1.walletService.release({
                    ...
                    description: `Release escrow ${escrowAmount} ${offer.currency} - NFT offer rejected by owner`,
                    metadata: {
                        offerId: offer.id,
                ...
                if ((releaseError === null || releaseError === void 0 ? void 0 : releaseError.name) === "DuplicateOperationError") {
                    ...
                    console_1.logger.warn("NFT_OFFER", `No escrow to release for offer ${offer.id} (likely legacy); continuing reject`);
                }
                else {
                    throw releaseError;
                }
