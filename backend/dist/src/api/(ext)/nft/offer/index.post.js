...
exports.default = async (data) => {
    (0, storefront_1.assertPurchaseAllowedOnNativeApp)(data, "make offers");
    ...
    try {
        ...
        const offersEnabled = (0, settlement_1.settingEnabled)(settings.get('nftEnableOffers'), true);
        if (!offersEnabled) {
            throw (0, error_1.createError)({
                statusCode: 403,
        ...
        const marketplaceFee = amount * (marketplaceFeePercent / 100);
        const escrowAmount = amount + marketplaceFee;
        const escrowWalletType = "SPOT";
        const offerData = {
            userId: user.id,
        ...
        ctx === null || ctx === void 0 ? void 0 : ctx.step("Create offer and hold escrow in database transaction");
        const offer = await db_1.sequelize.transaction(async (transaction) => {
            ...
            await wallet_1.walletService.hold({
                idempotencyKey: `nft_offer_hold_${newOffer.id}`,
                userId: user.id,
                walletType: escrowWalletType,
                currency,
                amount: escrowAmount,
                operationType: "NFT_OFFER",
                description: `Escrow ${escrowAmount} ${currency} for NFT offer ${newOffer.id}`,
                metadata: {
                    offerId: newOffer.id,
                    offerType,
