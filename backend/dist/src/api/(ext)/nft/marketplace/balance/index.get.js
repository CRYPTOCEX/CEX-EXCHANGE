                                ...
                                oneOf: [
                                    {
                                        ...
                                        properties: {
                                            ...
                                            balanceUSD: {
                                                type: "number",
                                                nullable: true,
                                                description: "USD value of the contract balance, or null when the chain's native coin has no usable rate. Null means UNKNOWN — never treat it as zero."
                                            },
                                    ...
                                    },
                                    {
                                        ...
                                        additionalProperties: {
                                            ...
                                            properties: {
                                                ...
                                                contractAddress: { type: "string" },
                                                balance: { type: "string" },
                                                balanceUSD: { type: "number", nullable: true },
                                                currency: { type: "string" },
                                                lastUpdated: { type: "string" },
                                                accumulatedFees: { type: "string" },
                                                pendingWithdrawals: { type: "string" },
...
async function getMarketplaceBalance(chain) {
    ...
    try {
        ...
        const rates = await (0, utils_1.getUsdRates)([currencySymbol]);
        const rate = rates.get(currencySymbol);
        const WEI_PER_COIN = 1e18;
        const balanceCoins = Number(balance) / WEI_PER_COIN;
        const balanceUSD = !priced(rate) || !Number.isFinite(balanceCoins)
            ? null
            : balanceCoins * rate;
        return {
            chain: chain.toUpperCase(),
