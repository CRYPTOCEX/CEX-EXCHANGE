...
exports.default = async (data) => {
    ...
    if (payment.returnUrl) {
        try {
            new URL(payment.returnUrl);
        }
        catch (_c) {
            ctx === null || ctx === void 0 ? void 0 : ctx.fail("The payment's return URL is not a valid URL");
            throw (0, error_1.createError)({
                statusCode: 400,
                message: "This payment cannot be completed: the merchant's return URL is not a valid URL, so " +
                    "there is nowhere to send you afterwards. No money has moved.",
            });
        }
    }
    ...
    try {
        const result = await db_1.sequelize.transaction(async (t) => {
            ...
            for (const allocation of allocations) {
                ...
                const settledEquivalent = roundAmount(expectedEquivalent);
                if (!isTestMode) {
                    ...
                    const debitResult = await wallet_1.walletService.debit({
                        ...
                        operationType: "GATEWAY_PAYMENT",
                        referenceId: `${payment.id}_${transactionRecords.length}`,
                        description: `Payment to ${((_a = payment.merchant) === null || _a === void 0 ? void 0 : _a.name) || "merchant"}${payment.description ? ` - ${payment.description}` : ""} (${settledEquivalent.toFixed(2)} ${payment.currency})`,
                        metadata: {
                            ...
                            equivalentAmount: settledEquivalent,
                            quotedEquivalentAmount: allocation.equivalentInPaymentCurrency,
            ...
            if (!isTestMode) {
                ...
                for (let i = 0; i < allocations.length; i++) {
                    ...
                    const roundedAmount = roundAmount(allocation.amount);
                    const allocationFee = Math.round(roundedAmount * feePercentage * Math.pow(10, (0, wallet_1.getPrecision)(allocation.currency))) /
                        Math.pow(10, (0, wallet_1.getPrecision)(allocation.currency));
                    if (allocationFee > 0) {
            ...
            await payment.update({
                ...
                completedAt,
                ipAddress: (0, address_parser_1.requestClientIp)(data) || undefined,
        ...
        try {
            ...
            console_1.logger.error("GATEWAY_CHECKOUT", `Payment ${payment.paymentIntentId} COMPLETED but its return URL could not be built ` +
                `(${payment.returnUrl}); answering without a redirect rather than failing a payment ` +
                `that has already moved money`, error);
        }
        ...
        const isRecoverable = status >= 400 && status < 500;
        if (payment.webhookUrl && !isRecoverable) {
            try {
