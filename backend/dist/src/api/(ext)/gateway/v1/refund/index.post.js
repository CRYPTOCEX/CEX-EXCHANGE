...
const refund_math_1 = require("@b/utils/gateway/refund-math");
...
exports.default = async (data) => {
    ...
    try {
        result = await db_1.sequelize.transaction(async (t) => {
            ...
            if (!lockedPayment) {
                throw (0, error_1.createError)({ statusCode: 404, message: "Payment not found" });
            }
            ...
            const totalRefunded = (0, refund_math_1.sumCompletedRefunds)(existingRefunds);
            const plan = (0, refund_math_1.planRefund)({
                paymentAmount: lockedPayment.amount,
                paymentFeeAmount: lockedPayment.feeAmount,
                alreadyRefunded: totalRefunded,
                requestedAmount: (_a = body.amount) !== null && _a !== void 0 ? _a : lockedPayment.amount,
            });
            if (!plan.ok) {
                throw (0, error_1.createError)({ statusCode: 400, message: plan.message });
            }
            const { refundAmount, proportionalFee, newStatus } = plan;
    ...
    if (payment.webhookUrl) {
        try {
            await (0, gateway_1.sendWebhook)(merchant.id, payment.id, result.refund.id, "refund.completed", payment.webhookUrl, {
                id: `evt_${refundId}`,
