...
function resolvePluginsDir() {
    ...
    const candidates = [
        ...
        path.resolve(__dirname.split(`${path.sep}dist${path.sep}`).join(`${path.sep}`), "..", "plugins"),
        path.join(cwd, "src", "api", "(ext)", "gateway", "integration", "plugins"),
        path.join(cwd, "backend", "src", "api", "(ext)", "gateway", "integration", "plugins"),
        path.join(cwd, "backend", "dist", "src", "api", "(ext)", "gateway", "integration", "plugins"),
    ];
...
if (PLUGINS_DIR) {
    console_1.logger.info("GATEWAY", `Integration plugins directory resolved to: ${PLUGINS_DIR}`);
}
else {
    console_1.logger.error("GATEWAY", `Integration plugins directory not found. cwd=${process.cwd()} __dirname=${__dirname}`);
}
...
exports.default = async (data) => {
    ...
    if (!PLUGINS_DIR) {
        throw (0, error_1.createError)({
            statusCode: 500,
            message: "Integration plugins directory is missing from the deployment. Re-deploy the backend ensuring backend/src/api/(ext)/gateway/integration/plugins is included.",
        });
    }
    const pluginDir = path.join(PLUGINS_DIR, plugin.folder);
    if (!fs.existsSync(pluginDir)) {
        throw (0, error_1.createError)({
            statusCode: 404,
            message: `Plugin files not found at ${pluginDir}`,
        });
    }
