...
const wallet_utils_1 = require("@b/api/(ext)/ecosystem/wallet/utils");
...
exports.default = async (data, message) => {
    ...
    try {
        ...
        // Security: for NO_PERMIT, validate the supplied custodial address was actually locked for THIS user.
        // Without this check, an attacker could supply another user's assigned custodial address and receive
        // their deposit credit (the monitor will detect the legit user's on-chain transfer and credit
        // whichever wallet was attached to the monitor).
        if (contractType === "NO_PERMIT") {
            if (!address || typeof address !== "string") {
                ctx === null || ctx === void 0 ? void 0 : ctx.fail("NO_PERMIT requires a custodial address");
                throw (0, error_1.createError)(400, "Custodial address is required for NO_PERMIT deposits");
            }
            if (!wallet_utils_1.isAddressLockedForUser(address, user.id)) {
                ctx === null || ctx === void 0 ? void 0 : ctx.fail("Supplied custodial address is not assigned to this user");
                throw (0, error_1.createError)(403, "Custodial address not assigned to this user. Request a new deposit address.");
            }
        }
