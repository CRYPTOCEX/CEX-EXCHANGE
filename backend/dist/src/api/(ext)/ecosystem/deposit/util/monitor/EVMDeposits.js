...
// Minimum confirmations required to safely credit a deposit (reorg protection).
// Conservative defaults; override via APP_<CHAIN>_MIN_CONFIRMATIONS env var.
const MIN_CONFIRMATIONS_BY_CHAIN = {
    ETH: 12,
    BSC: 15,
    POLYGON: 64,
    ARBITRUM: 20,
...
const getMinConfirmations = (chain) => {
    const envKey = `APP_${chain}_MIN_CONFIRMATIONS`;
    ...
    return MIN_CONFIRMATIONS_BY_CHAIN[chain] || 12;
};
class EVMDeposits {
    constructor(options) {
        this.active = true;
    ...
    async watchNativeDeposits(provider, feeDecimals) {
        ...
        const verifyDeposits = async () => {
            ...
            try {
                ...
                const minConfirmations = getMinConfirmations(this.chain);
                for (const tx of transactions) {
                    if (tx.to &&
                        ...
                        if (txConfirmations < minConfirmations) {
                            console_1.logger.debug("EVM_DEPOSIT", `${tx.hash}: ${txConfirmations}/${minConfirmations} confirmations, waiting`);
                            continue;
                        }
                        consecutiveErrors = 0;
                        ...
                        try {
                            console_1.logger.success("EVM_DEPOSIT", `Found native deposit for ${this.chain}: ${tx.hash} (${txConfirmations} confs)`);
