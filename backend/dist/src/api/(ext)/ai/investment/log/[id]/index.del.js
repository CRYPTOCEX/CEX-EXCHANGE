...
exports.default = async (data) => {
    ...
    await db_1.sequelize.transaction(async (t) => {
        var _a;
        // Re-check status inside the transaction with row lock to prevent a race where the cron
        // matures and pays out the investment between the earlier status check and this transaction.
        ctx === null || ctx === void 0 ? void 0 : ctx.step("Re-locking investment to verify still ACTIVE");
        const lockedInvestment = await db_1.models.aiInvestment.findByPk(id, {
            transaction: t,
            lock: t.LOCK.UPDATE,
        });
        if (!lockedInvestment || lockedInvestment.status !== "ACTIVE") {
            throw (0, error_1.createError)({
                statusCode: 400,
                message: `Investment is no longer cancellable (current status: ${lockedInvestment ? lockedInvestment.status : "missing"})`,
            });
        }
