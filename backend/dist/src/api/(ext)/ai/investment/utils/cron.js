...
function calculateEndDate(investment) {
    ...
    switch (investment.duration.timeframe) {
        ...
        case "WEEK":
            return (0, date_fns_1.addWeeks)(createdAt, investment.duration.duration);
        case "MONTH":
            return (0, date_fns_1.addMonths)(createdAt, investment.duration.duration);
        default:
            return (0, date_fns_1.addHours)(createdAt, investment.duration.duration);
    }
}
async function handleAiInvestmentUpdate(investment, user, roi, investmentResult) {
    ...
    try {
        ...
        if (payoutAmount > 0) {
            // Idempotency key keyed only on investment.id so that admin-changed `result` cannot
            // produce a second payout for the same matured investment.
            const idempotencyKey = `ai_invest_cron_payout_${investment.id}`;
