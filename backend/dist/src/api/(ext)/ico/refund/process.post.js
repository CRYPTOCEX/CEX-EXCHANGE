...
exports.default = async (data) => {
    ...
    try {
        ...
        for (const icoTransaction of pendingTransactions) {
            try {
                // Refund must include any platform fee paid at purchase — the fee was for processing
                // a successful contribution, and offering failure is not the user's fault.
                let storedFee = 0;
                try {
                    const parsedNotes = icoTransaction.notes ? JSON.parse(icoTransaction.notes) : {};
                    storedFee = typeof parsedNotes.feeAmount === "number" ? parsedNotes.feeAmount : 0;
                }
                catch (_e) {
                    storedFee = 0;
                }
                const refundAmount = (icoTransaction.amount * icoTransaction.price) + storedFee;
