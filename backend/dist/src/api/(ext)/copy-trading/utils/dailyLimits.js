...
async function resetDailyLimits() {
    ...
    try {
        ...
        reset++;
        const limitHitLogs = await db_1.models.copyTradingAuditLog.findAll({
            attributes: ["entityId"],
            where: {
                entityType: "copyTradingFollower",
                action: "DAILY_LOSS_LIMIT_REACHED",
        ...
        const limitHitFollowerIds = limitHitLogs
            .map((row) => row.entityId)
            .filter(Boolean);
        const pausedFollowers = limitHitFollowerIds.length
            ? await db_1.models.copyTradingFollower.findAll({
                where: { status: "PAUSED", id: { [sequelize_1.Op.in]: limitHitFollowerIds } },
            })
            : [];
