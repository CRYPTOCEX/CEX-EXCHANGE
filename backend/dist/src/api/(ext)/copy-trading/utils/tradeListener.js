...
class LeaderTradeListener {
    ...
    async onOrderCreated(event) {
        try {
            const { checkCopyTypeAvailability, leaderOffersMarketType } = await Promise.resolve().then(() => __importStar(require("./index")));
            const availability = await checkCopyTypeAvailability("SPOT");
            ...
            if (!leaderOffersMarketType(leaderData.tradingType, "SPOT")) {
                return { success: true };
            }
            ...
            const leaderWallet = await (0, wallet_1.getWalletByUserIdAndCurrency)(event.userId, quoteCurrency, "ECO");
...
async function handleOrderCancelled(orderId, userId, symbol) {
    try {
        ...
        for (const followerTrade of followerTrades) {
            if (await teardownFollowerTrade(followerTrade))
                cancelledCount++;
        }
        await (0, settings_core_1.createAuditLog)({
            entityType: "copyTradingTrade",
