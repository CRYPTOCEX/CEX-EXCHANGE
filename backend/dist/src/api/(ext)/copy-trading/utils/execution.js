...
async function executeOrder(params) {
    ...
    try {
        ...
        if (type === "MARKET") {
            const { asks, bids } = await (0, queries_1.getRealOrderBook)(symbol);
        ...
        const wallet = await (0, wallet_1.getWalletByUserIdAndCurrency)(userId, walletCurrency, "COPY_TRADING");
        if (!wallet) {
...
function toOrderBigInt(value) {
    if (typeof value === "bigint")
        return value;
    if (value === null || value === undefined) {
        throw new TypeError("cannot convert null/undefined order field to bigint");
    }
...
async function cancelCopyOrder(orderId, userId) {
    var _a;
    try {
        const { orders, truncated } = await (0, queries_1.getOpenOrdersByUserId)(userId);
        const order = orders.find((o) => o.id === orderId);
        if (!order) {
            return {
                success: false,
                error: truncated
                    ? "Order not found in the orders searched — the account has more open orders than one pass reads. Cancel it from the orders page."
                    : "Order not found",
            };
        }
        ...
        await (0, queries_1.cancelOrderByUuid)(userId, orderId, createdAtStr, order.symbol, toOrderBigInt(order.price), order.side, toOrderBigInt((_a = order.remaining) !== null && _a !== void 0 ? _a : order.amount));
        return { success: true };
    }
    catch (error) {
...
async function monitorStopLevels() {
    ...
    try {
        ...
        for (const trade of openTrades) {
            ...
            const { asks, bids } = await (0, queries_1.getRealOrderBook)(trade.symbol);
