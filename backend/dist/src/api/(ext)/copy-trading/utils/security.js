...
async function checkMarketConflict(userId, leaderId, markets) {
    ...
    const requested = markets.map((m) => typeof m === "string"
        ? { symbol: m, marketType: "SPOT" }
        : { symbol: m.symbol, marketType: m.marketType === "BINARY" ? "BINARY" : "SPOT" });
    const requestedKeys = new Set(requested.map((m) => `${m.symbol}|${m.marketType}`));
        ...
        include: [
            {
                ...
                as: "allocations",
                where: {
                    symbol: { [Op.in]: requested.map((m) => m.symbol) },
                    isActive: true,
                },
                required: false,
            },
    ...
    for (const sub of activeSubscriptions) {
        const clashing = (sub.allocations || []).filter((a) => requestedKeys.has(`${a.symbol}|${a.marketType || "SPOT"}`));
        if (clashing.length > 0) {
            conflicts.push({
                leaderName: sub.leader.displayName,
                markets: clashing.map((a) => `${a.symbol}${a.marketType === "BINARY" ? " (BINARY)" : ""}`),
            });
        }
    }
