...
exports.default = async (data) => {
    ...
    for (const sub of subscriptions) {
        ...
        for (const alloc of sub.allocations) {
            try {
                ...
                const allocUSDT = baseInUSDT + quoteInUSDT;
                totalAllocated += allocUSDT;
                allocatedByLeader[sub.leaderId] =
                    (allocatedByLeader[sub.leaderId] || 0) + allocUSDT;
            }
            catch (error) {
    ...
    const byLeader = Object.values(byLeaderMap).map((item) => {
        var _a;
        const allocated = allocatedByLeader[(_a = item.leader) === null || _a === void 0 ? void 0 : _a.id] || 0;
        return {
            ...item,
            winRate: item.trades > 0 ? (item.wins / item.trades) * 100 : 0,
            roi: allocated > 0 ? Math.round((item.profit / allocated) * 100 * 100) / 100 : 0,
