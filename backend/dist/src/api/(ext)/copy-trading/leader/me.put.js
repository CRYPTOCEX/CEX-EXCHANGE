...
const leaderTerms_1 = require("@b/api/(ext)/copy-trading/utils/leaderTerms");
...
exports.default = async (data) => {
    ...
    if (updateData.profitSharePercent !== undefined) {
        const openPositionCount = await db_1.models.copyTradingTrade.count({
            where: {
                leaderId: leader.id,
                isLeaderTrade: false,
        ...
        const decision = (0, leaderTerms_1.evaluateProfitShareChange)({
            current: leader.profitSharePercent,
            next: updateData.profitSharePercent,
            openPositionCount,
        });
        if (!decision.allowed) {
            ctx === null || ctx === void 0 ? void 0 : ctx.fail("Profit share raise refused while positions are open");
            throw (0, error_1.createError)({
                statusCode: 400,
                message: decision.reason ||
                    "You cannot raise your profit share while copied positions are open.",
            });
        }
    }
