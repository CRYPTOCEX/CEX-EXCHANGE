...
exports.default = async (data) => {
    ...
    const countMap = new Map(followerCounts.map((c) => [
        `${c.symbol}|${c.marketType || "SPOT"}`,
        parseInt(c.count, 10),
    ]));
    const marketsWithCounts = markets.map((m) => {
        const market = m.toJSON();
        market.followerCount =
            countMap.get(`${market.symbol}|${market.marketType || "SPOT"}`) || 0;
        return (0, mobile_shape_1.shapeForClient)((0, native_binary_1.stripBinaryForNative)(market, data), data);
    });
