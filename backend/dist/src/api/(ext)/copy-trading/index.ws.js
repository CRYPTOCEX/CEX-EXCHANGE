...
class CopyTradingDataHandler {
    ...
    async addSubscription(userId, channel, leaderId, isNative = false) {
        ...
        switch (channel) {
            case "my_trades":
                initialData = await this.getMyTrades(userId, isNative);
                break;
            case "leader_updates":
                if (leaderId) {
                    initialData = await this.getLeaderData(leaderId, isNative);
                }
                break;
            case "all_leaders":
    ...
    async getMyTrades(userId, isNative = false) {
        try {
            ...
            return (0, native_binary_1.stripBinaryForNative)(trades.map((t) => t.toJSON()), { headers: { platform: isNative ? "mobile" : "web" } });
        }
        catch (error) {
    ...
    async getLeaderData(leaderId, isNative = false) {
        try {
            ...
            if (!leader)
                return null;
            return (0, native_binary_1.stripBinaryForNative)(leader.toJSON(), {
                headers: { platform: isNative ? "mobile" : "web" },
            });
        }
        catch (error) {
...
exports.default = async (ws, message) => {
    ...
    if (action === "SUBSCRIBE") {
        const result = await handler.addSubscription(userId, channel, leaderId, ws.isNativeApp === true);
        return {
            subscribed: channel,
            leaderId,
            data: result.data,
        ...
        if (!(0, Websocket_1.hasClientOnRoute)(ws.path, userId)) {
