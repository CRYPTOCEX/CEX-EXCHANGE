...
function validatePriceConfig(data, marketPrice) {
    ...
    if (!Number.isFinite(data.value) || (data.model === "FIXED" && data.value <= 0)) {
        throw (0, error_1.createError)({
            statusCode: 400,
    ...
    if (!Number.isFinite(finalPrice) || finalPrice <= 0 || Number(finalPrice.toFixed(8)) <= 0) {
        throw (0, error_1.createError)({ statusCode: 400, message: "Calculated price must be finite and positive" });
    }
    return {
        model: data.model,
        value: data.value,
