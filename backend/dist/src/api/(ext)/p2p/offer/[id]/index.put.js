...
exports.default = async (data) => {
    ...
    try {
        ...
        if (offer.type === "SELL" && updateData.status && updateData.status !== previousOfferStatus) {
            ...
            if (offerAmount > 0) {
                if (updateData.status === "PAUSED") {
                    ...
                    if (wallet && ((_a = wallet.inOrder) !== null && _a !== void 0 ? _a : 0) >= offerAmount) {
                        await wallet_1.walletService.release({
                            idempotencyKey: `p2p_offer_pause_release_${offer.id}_${Date.now()}`,
                            userId: user.id,
                            walletId: wallet.id,
                    ...
                    const wallet = await (0, utils_1.getWalletSafe)(user.id, offer.walletType, offer.currency, false, ctx);
                    if (!wallet || ((_b = wallet.balance) !== null && _b !== void 0 ? _b : 0) < offerAmount) {
                        await transaction.rollback();
                        throw (0, error_1.createError)({
                            statusCode: 400,
                            message: `Insufficient balance to resume offer. Available: ${(_c = wallet === null || wallet === void 0 ? void 0 : wallet.balance) !== null && _c !== void 0 ? _c : 0} ${offer.currency}, Required: ${offerAmount} ${offer.currency}`,
                        });
                    }
                    await wallet_1.walletService.hold({
                        idempotencyKey: `p2p_offer_resume_lock_${offer.id}_${Date.now()}`,
                        userId: user.id,
                        walletId: wallet.id,
                        walletType: offer.walletType,
                        currency: offer.currency,
        ...
        if (error.code === 'ECONNRESET' || ((_d = error.message) === null || _d === void 0 ? void 0 : _d.includes('ECONNRESET'))) {
            throw (0, error_1.createError)({
                statusCode: 500,
