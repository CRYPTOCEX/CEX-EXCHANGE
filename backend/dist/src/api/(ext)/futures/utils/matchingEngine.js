...
function cancelledKey(id, symbol) { return `${symbol}:${String(id)}`; }
function sweepCancelledOrders() {
    const now = Date.now();
    for (const [key, expires] of cancelledOrders) {
        if (expires <= now) cancelledOrders.delete(key);
    }
    while (cancelledOrders.size > 50000) cancelledOrders.delete(cancelledOrders.keys().next().value);
}
const { withFuturesEngineLock } = require("./engine-lock");
const { models } = require("@b/db");
class FuturesMatchingEngine {
    constructor() {
        FuturesMatchingEngine.currentInstance = this;
        this.orderQueue = {};
    ...
    static recordCancellation(orderId, symbol) {
        cancelledOrders.set(cancelledKey(orderId, symbol), Date.now() + 60000);
        sweepCancelledOrders();
        FuturesMatchingEngine.currentInstance?.removeFromQueue(orderId, symbol);
    }
    removeFromQueue(orderId, symbol) {
        this.orderQueue[symbol] = (this.orderQueue[symbol] || []).filter((order) => String(order.id) !== String(orderId));
    }
    static getInstance() {
    ...
    async initializeOrders() {
        try {
            ...
            openOrders.forEach((order) => {
                ...
                sweepCancelledOrders();
                if (cancelledOrders.has(cancelledKey(normalizedOrder.id, normalizedOrder.symbol))) return;
                if (!this.orderQueue[normalizedOrder.symbol]) {
                    this.orderQueue[normalizedOrder.symbol] = [];
                }
                if (!this.orderQueue[normalizedOrder.symbol].some((queued) => String(queued.id) === String(normalizedOrder.id))) {
    ...
    async processQueue() {
        return withFuturesEngineLock(async () => {
            this.cycleStarted = false;
            const result = await this.processFundedQueue();
            if (this.cycleStarted) await models.settings.destroy({ where: { key: "futuresEnginePendingCycle", value: this.cycleToken } });
            return result;
        });
    }
    async processFundedQueue() {
        ...
        // Claim before reading the snapshot. A unique key also prevents a
        // second worker from matching if an advisory-lock connection is lost.
        this.cycleToken = JSON.stringify({ startedAt: new Date().toISOString(), pid: process.pid, token: require("crypto").randomUUID() });
        await models.settings.create({ key: "futuresEnginePendingCycle", value: this.cycleToken });
        this.cycleStarted = true;
        // Refresh under the shared writer lock: another worker may have
        // cancelled or filled a row since this worker last loaded its queue.
        const freshOrders = await (0, order_1.getAllOpenOrders)();
        const freshQueue = {};
        for (const raw of freshOrders) {
            const id = String(raw.id);
            const hold = await models.transaction.findOne({ where: { idempotencyKey: `futures_hold_${id}`, userId: String(raw.userId), status: "COMPLETED" } });
            if (!hold) {
                console_1.logger.warn("FUTURES", `Order ${id} has no verified funding hold; excluded from matching pending reconciliation`);
                continue;
            }
            const order = { ...raw, id, userId: String(raw.userId), createdAt: new Date(raw.createdAt), updatedAt: new Date(raw.updatedAt) };
            for (const field of ["amount", "price", "cost", "fee", "remaining", "filled", "leverage"]) order[field] = BigInt(String(raw[field] ?? 0));
            for (const field of ["stopLossPrice", "takeProfitPrice"]) order[field] = raw[field] == null ? null : BigInt(String(raw[field]));
            (freshQueue[order.symbol] ||= []).push(order);
        }
        this.orderQueue = freshQueue;
        ...
        if (!Object.values(this.orderQueue).some((orders) => orders.length)) return;
    ...
    async performUpdates(ordersToUpdate, orderBookUpdates) {
        ...
        if (updateQueries.length > 0) {
            try {
                await client.batch(updateQueries, { prepare: true, consistency: 6 /* LOCAL_QUORUM */ });
            }
            catch (error) {
    ...
    async handleOrderCancellation(orderId, symbol) {
        this.removeFromQueue(orderId, symbol);
