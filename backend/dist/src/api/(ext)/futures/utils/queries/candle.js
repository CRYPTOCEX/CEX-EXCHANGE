...
async function getLastCandles() {
    ...
    `;
        const result = await client.execute(query, [], { prepare: true, consistency: 6 /* LOCAL_QUORUM */ });
        const latestByKey = {};
...
async function getLatestCandleForSymbol(symbol, interval) {
    ...
    try {
      ...
      LIMIT 1;
    `;
        const result = await client.execute(query, [symbol, interval], { prepare: true, consistency: 6 /* LOCAL_QUORUM */ });
...
async function getYesterdayCandles() {
    ...
    `;
        const result = await client.execute(query, [startOfYesterday.toISOString(), endOfYesterday.toISOString()], { prepare: true, consistency: 6 /* LOCAL_QUORUM */ });
