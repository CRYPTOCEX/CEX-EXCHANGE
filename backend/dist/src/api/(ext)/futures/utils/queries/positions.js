...
async function getPosition(userId, symbol, side) {
    ...
    WHERE symbol = ? AND "userId" = ? AND side = ? ALLOW FILTERING;
  `;
    const params = [symbol, userId, side];
    try {
        const result = { rows: await (0, order_1.readAllPages)(query, params) };
        result.rows = result.rows.filter(row => ["OPEN", "PARTIALLY_LIQUIDATED"].includes(row.status));
        if (result.rows.length > 1) throw (0, error_1.createError)({ statusCode: 409, message: "Multiple active positions require reconciliation" });
...
async function getAllOpenPositions() {
  ...
  `;
    try {
        const result = { rows: (await (0, order_1.readAllPages)(query, [])).filter(row => ["OPEN", "PARTIALLY_LIQUIDATED"].includes(row.status)) };
        return result.rows.map((row) => ({
            id: (0, order_1.uuidToString)(row.id),
            userId: (0, order_1.uuidToString)(row.userId),
            symbol: row.symbol,
...
async function createPosition(userId, symbol, side, entryPrice, amount, leverage, unrealizedPnl, stopLossPrice, takeProfitPrice) {
  ...
  `;
    ...
    try {
        await client.execute(query, params, { prepare: true, consistency: 6 /* LOCAL_QUORUM */ });
    }
    catch (error) {
