...
exports.default = async (data) => {
    ...
    try {
        ...
        // Pass `remaining` (not `amount`) to the orderbook subtractor so we don't decrement filled
        // quantity that's already left the book. Cancel must only refund the unfilled portion.
        const remainingBig = BigInt(order.remaining);
        await (0, order_1.cancelOrderByUuid)(user.id, id, timestamp, order.symbol, BigInt(order.price), order.side, remainingBig);
        const [currency, pair] = order.symbol.split("/");
        // Refund only the cost of the unfilled portion plus the unspent fee on that portion.
        // Previous behavior refunded the full original cost+fee, double-paying any filled portion
        // (the user kept the filled-position AND got the cost refunded — fund loss to platform).
        const originalAmount = fromBigInt(BigInt(order.amount));
        const remainingAmount = fromBigInt(remainingBig);
        const fillRatio = originalAmount > 0 ? (remainingAmount / originalAmount) : 0;
        const refundAmount = (fromBigInt(order.cost) + fromBigInt(order.fee)) * fillRatio;
