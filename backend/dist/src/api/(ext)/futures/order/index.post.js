...
exports.default = async (data) => {
    ...
    // Validate amount/price/leverage are positive finite numbers. Without these guards a user could
    // submit NaN/Infinity/negative/zero and bypass downstream balance and margin checks.
    ...
    if (price !== undefined && price !== null && (!Number.isFinite(price) || price <= 0)) {
        throw (0, error_1.createError)({ statusCode: 400, message: "Invalid price" });
    }
    const MAX_LEVERAGE = Number(process.env.APP_FUTURES_MAX_LEVERAGE) || 125;
    if (!Number.isFinite(leverage) || leverage < 1 || leverage > MAX_LEVERAGE) {
        throw (0, error_1.createError)({ statusCode: 400, message: `Leverage must be between 1 and ${MAX_LEVERAGE}` });
    }
    ...
    try {
        ...
        // Both market BUY and market SELL are takers.
        const feeRate = (type || "").toLowerCase() === "market"
