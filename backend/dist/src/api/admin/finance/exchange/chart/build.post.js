...
exports.metadata = {
    ...
    logModule: "ADMIN_FIN",
    logTitle: "Rebuild chart data",
};
const DEFAULT_INTERVALS = utils_1.SUPPORTED_INTERVALS.filter((i) => i !== "1M");
...
exports.default = async (data) => {
    ...
    if (symbols && symbols.length > 0) {
        const invalid = symbols.filter((s) => !(0, utils_1.isValidSymbol)(s));
        if (invalid.length > 0) {
            throw (0, error_1.createError)({
                statusCode: 400,
                message: `Invalid symbol format: ${invalid.slice(0, 5).join(", ")}`,
            });
        }
    ...
    const unknown = intervalsToProcess.filter((i) => !(0, utils_1.isSupportedInterval)(i));
    if (unknown.length > 0) {
        throw (0, error_1.createError)({
            statusCode: 400,
            message: `Unsupported interval: ${unknown.join(", ")}`,
        });
    }
        ...
        for (const market of markets) {
            ...
            for (const interval of intervals) {
                try {
                    ...
                    const existingCandles = await (0, utils_1.readSeriesFile)(symbol, interval);
                    let fetchFrom = from;
                    if (existingCandles.length > 0) {
                        const newestExisting = existingCandles[existingCandles.length - 1][0];
                        if (newestExisting > from) {
                            fetchFrom = (0, utils_1.advanceInterval)(newestExisting, interval);
                        }
                    }
                    const batchSize = 500;
                    let storedCandles = 0;
                    while (fetchFrom <= lastClosed) {
                        let candles = null;
                        try {
                            candles = await exchange.fetchOHLCV(symbol, interval, fetchFrom, batchSize);
                        }
                        catch (fetchError) {
                        ...
                        if (!candles || candles.length === 0)
                            break;
                        const valid = (0, utils_1.validateAndCleanCandles)(candles, interval);
                        if (valid.length === 0)
                            break;
                        await (0, utils_1.saveOHLCVToCache)(symbol, interval, valid);
                        storedCandles += valid.length;
                        const next = (0, utils_1.advanceInterval)(valid[valid.length - 1][0], interval);
                        if (!(next > fetchFrom))
                            break;
                        fetchFrom = next;
                        await new Promise((resolve) => setTimeout(resolve, rateLimit));
                    }
                    console_1.logger.debug("Chart Build", `Job ${jobId} - ${symbol}:${interval} stored ${storedCandles} bars`);
                    completedTasks++;
