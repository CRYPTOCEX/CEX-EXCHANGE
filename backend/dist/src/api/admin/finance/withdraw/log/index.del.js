...
exports.default = async (data) => {
    ...
    const { ids } = body;
    ctx === null || ctx === void 0 ? void 0 : ctx.step("Checking for completed withdrawals");
    const completed = await db_1.models.transaction.findAll({
        where: { id: ids, status: "COMPLETED" },
        attributes: ["id"],
    });
    if (completed.length) {
        throw (0, error_1.createError)({
            statusCode: 400,
            message: `Cannot delete ${completed.length} completed withdraw log(s). Deselect them and try again.`,
        });
    }
