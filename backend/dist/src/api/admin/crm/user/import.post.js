...
exports.default = async (data) => {
    ...
    try {
        ...
        const roleVerdicts = new Map();
        const resolveRole = async (rawRoleId) => {
            const key = String(rawRoleId);
            const cached = roleVerdicts.get(key);
            if (cached)
                return cached;
            let verdict;
            const role = await db_1.models.role.findByPk(rawRoleId);
            if (!role) {
                verdict = { role: null, refusal: "Invalid roleId" };
            }
            else if (role.name === "Super Admin") {
                verdict = { role: null, refusal: "Cannot import a Super Admin user" };
            }
            else {
                let refusal = null;
                try {
                    await (0, utils_1.assertCanAssignRole)(user.id, role.id);
                }
                catch (err) {
                    refusal = (err === null || err === void 0 ? void 0 : err.message) || "You cannot import users into this role";
                }
                verdict = { role, refusal };
            }
            roleVerdicts.set(key, verdict);
            return verdict;
        };
        ...
        for (let i = 0; i < records.length; i++) {
            ...
            try {
                ...
                if ((0, system_accounts_1.isReservedEmail)(record.email)) {
                    errors.push({
                        row: rowNumber,
                        email: record.email,
                        error: "This email address is reserved for the platform's own accounts",
                    });
                    failedCount++;
                    continue;
                }
                ...
                if (existingUser) {
                    errors.push({
                        ...
                        error: existingUser.deletedAt
                            ? "A deleted account still holds this email address"
                            : "User with this email already exists",
                    });
                    failedCount++;
                    continue;
                }
                let resolvedRoleId = defaultRole.id;
                if (record.roleId !== undefined && record.roleId !== null && String(record.roleId).trim() !== "") {
                    if (superAdminRole && Number(record.roleId) === Number(superAdminRole.id)) {
                        errors.push({
                            row: rowNumber,
                            email: record.email,
                            error: "Cannot import a Super Admin user",
                        });
                        failedCount++;
                        continue;
                    }
                    const { role, refusal } = await resolveRole(record.roleId);
                    if (refusal || !role) {
                        errors.push({
                            row: rowNumber,
                            email: record.email,
                            error: refusal || "Invalid roleId",
                        });
                        failedCount++;
                        continue;
                    }
                ...
                const userData = {
                    ...
                    phone: (() => {
                        const d = record.phone
                            ? String(record.phone).replace(/\D/g, "").slice(0, 15)
                            : "";
                        const p = d ? `+${d}` : null;
                        return p && /^\+\d{7,15}$/.test(p) ? p : null;
                    })(),
                    status: VALID_STATUSES.has(String(record.status || "").toUpperCase())
                ...
                importedCount++;
                if (record.twoFactor === true) {
                    errors.push({
                        row: rowNumber,
                        email: record.email,
                        error: "Imported, but `twoFactor` was ignored: two-factor authentication " +
                            "cannot be enabled by import — the user must enrol it themselves.",
                    });
                }
                if (sendWelcomeEmail && newUser.email) {
                    ...
                    const welcomeUserId = newUser.id;
                    const welcomeEmail = newUser.email;
            ...
            <p>You can now sign in using your email address (<strong>${newUser.email}</strong>) and the password you were provided.</p>
            <p>We recommend completing your profile and changing your password after your first sign-in.</p>
          `;
                    void (0, emails_1.sendEmailToTargetWithTemplate)(welcomeEmail, welcomeSubject, welcomeHtml).catch((emailError) => {
                        console_1.logger.error("USER_IMPORT", `Failed to send welcome email to user ${welcomeUserId} (${welcomeEmail})`, emailError instanceof Error ? emailError : new Error(String(emailError)));
                    });
                }
            }
            catch (error) {
