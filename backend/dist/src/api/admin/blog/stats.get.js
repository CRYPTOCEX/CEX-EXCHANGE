...
exports.default = async (data) => {
    ...
    try {
        ctx === null || ctx === void 0 ? void 0 : ctx.step("Fetching blog dashboard statistics");
        const [publishedCount, draftCount, recentPosts, approvedCount, pendingCount, recentPendingAuthors, totalCategories, topCategories, totalTags, topTags, totalComments,] = await Promise.all([
            db_1.models.post.count({
                where: { status: "PUBLISHED" },
            }),
            db_1.models.post.count({ where: { status: "DRAFT" } }),
            db_1.models.post.findAll({
            ...
            db_1.models.author.count({
                where: { status: "APPROVED" },
            }),
            db_1.models.author.count({
                where: { status: "PENDING" },
            }),
            db_1.models.author.findAll({
                where: { status: "PENDING" },
                order: [["createdAt", "DESC"]],
                limit: 3,
                include: [
            ...
            db_1.models.category.count(),
            db_1.models.category.findAll({
                subQuery: false,
                attributes: [
                    "id",
                    "name",
            ...
            db_1.models.tag.count(),
            db_1.models.tag.findAll({
                subQuery: false,
                attributes: [
                    "id",
                    "name",
