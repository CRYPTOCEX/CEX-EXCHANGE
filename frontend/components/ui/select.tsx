...
>(
  (
    ...
    const triggerClasses = cn(
      ...
      error && "border-2 border-destructive focus:ring-destructive focus:ring-2",
      className
    );
          ...
          <div className="p-2 border-b sticky top-0 bg-popover">
            <div className="relative">
              ...
              <input
                ...
                onKeyDown={(e) => {
                  // This used to `stopPropagation()` on EVERY key. Radix
                  // attaches its list handling to Select.Content, so stopping
                  // the synthetic event stopped ArrowDown/ArrowUp/Enter/Escape
                  // from ever reaching it — a searchable Select became
                  // mouse-only. That hit every admin CRUD form, because the
                  // shared form generator turns search on for any list over
                  // 6 options.
                  //
                  // Only the keys the INPUT must own are withheld: printable
                  // characters (which Radix would otherwise swallow as
                  // typeahead) and the text-editing keys. Navigation bubbles.
                  const NAVIGATION = [
                    "ArrowDown",
                    "ArrowUp",
                    "Enter",
