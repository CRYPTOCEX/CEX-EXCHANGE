...
/* Function-valued exports of lucide-react that are not icons. `default` and
   `createLucideIcon` have always been here; `Icon`, `LucideProvider` and
   `useLucideContext` arrived with lucide-react 1.x. */
...
export function IconPicker({ selectedIcon, onSelectIcon }: IconPickerProps) {
  ...
  useEffect(() => {
    ...
    const iconNames = Object.keys(LucideIcons)
      .filter(
        (key) =>
          /* Filter out non-icon exports. These are all FUNCTIONS, so the
             typeof test below does not exclude them: lucide-react 1.x added
             `LucideProvider`, `useLucideContext` and a generic `Icon` to the
             barrel, and picking one of those renders a hook as a component
             and throws. */
      ...
      <PopoverContent className="w-72 p-0" align="start">
        <div className="p-2 border-b">
          <div className="relative">
            ...
            <Input
              placeholder={`${tCommon("search_icons")}…`}
              className="pl-8"
              value={searchTerm}
