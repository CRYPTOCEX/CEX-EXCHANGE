...
/**
 * A fallback may be a plain node, or a render prop receiving the caught error and a
 * reset callback. The render-prop form is what domain-scoped boundaries need in order
 * to show the failure and offer a retry without remounting the whole page.
 * `ReactNode` never includes functions, so the two forms are unambiguous at runtime.
 */
...
class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  handleGoHome = () => {
    window.location.href = withCurrentLocale('/');
  };
    ...
    if (hasError) {
      ...
      if (fallback) {
        return typeof fallback === 'function'
          ? fallback(error as Error, this.resetError)
          : fallback;
      }
