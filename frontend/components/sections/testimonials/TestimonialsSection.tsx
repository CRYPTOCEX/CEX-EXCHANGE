...
export default function TestimonialsSection({
  ...
  const renderTrustBadge = () => {
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-muted shadow-lg">
          {trustBadge.icon && (
            <trustBadge.icon className="w-5 h-5" style={{ color: primaryColor }} />
          )}
          <span className="font-medium text-foreground">{trustBadge.text}</span>
          {trustBadge.rating && (
    ...
    return (
      <div className="relative">
        ...
        <div className="flex items-center justify-center gap-4 mt-8">
          <button
            onClick={prevSlide}
            className="p-2 rounded-full bg-muted shadow-lg hover:shadow-xl transition-all hover:-translate-x-0.5"
      ...
      <div className={`grid ${columnsClasses[columns]} ${gapClasses[gap]}`}>
        {testimonials.map((testimonial, index) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <div
                className="w-6 h-6 rounded-full flex items-center justify-center text-xs text-overlay-foreground"
