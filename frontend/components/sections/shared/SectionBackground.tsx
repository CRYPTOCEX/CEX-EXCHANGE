...
export default function SectionBackground({
  ...
  const getGradientStyle = (): React.CSSProperties => {
    ...
    const from = gradientFrom || `${withAlpha(primaryColor, 0.03)}`;
    const via = gradientVia || "transparent";
    const to = gradientTo || `${withAlpha(secondaryColor, 0.03)}`;
  ...
  const renderGridPattern = () => {
    ...
    const color = gridPattern.color || "hsl(var(--muted-foreground))";
          ...
          <>
            ...
            <div
              ...
              style={{
                ...
                }, ${withAlpha(primaryColor, 0.07)}, transparent, ${withAlpha(secondaryColor, 0.07)})`,
              }}
            />
          </>
        );
      ...
      case "mesh":
        return (
          <div
            ...
            style={{
              backgroundImage: `
                radial-gradient(at 40% 20%, ${withAlpha(primaryColor, 0.13)} 0px, transparent 50%),
                radial-gradient(at 80% 0%, ${withAlpha(secondaryColor, 0.08)} 0px, transparent 50%),
                radial-gradient(at 0% 50%, ${withAlpha(primaryColor, 0.06)} 0px, transparent 50%),
                radial-gradient(at 80% 50%, ${withAlpha(secondaryColor, 0.06)} 0px, transparent 50%),
                radial-gradient(at 0% 100%, ${withAlpha(primaryColor, 0.08)} 0px, transparent 50%),
                radial-gradient(at 80% 100%, ${withAlpha(secondaryColor, 0.13)} 0px, transparent 50%)
              `,
            }}
          />
        );
      ...
      {topFade && (
        <div
          className={`absolute top-0 left-0 right-0 h-24 pointer-events-none ${
            fadeColor || "bg-linear-to-b from-card to-transparent"
          }`}
          style={{ zIndex: 6 }}
        />
      )}
      ...
      {bottomFade && (
        <div
          className={`absolute bottom-0 left-0 right-0 h-24 pointer-events-none ${
            fadeColor || "bg-linear-to-t from-card to-transparent"
          }`}
          style={{ zIndex: 6 }}
        />
      )}
