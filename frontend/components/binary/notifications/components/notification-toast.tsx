/**
 * Notification Toast Component
 *
 * Individual toast notification with animations.
 * Supports different notification types with appropriate icons and colors.
 */

"use client";

import React, { memo, useEffect, useState } from "react";
import { m } from "framer-motion";
import {
  X,
  CheckCircle,
  AlertTriangle,
  XCircle,
  Info,
  TrendingUp,
  TrendingDown,
  Bell,
  BellRing,
  Zap,
  Check,
} from "lucide-react";
import type { TradingNotification, NotificationType } from "../types";

// ============================================================================
// TYPES
// ============================================================================

export interface NotificationToastProps {
  notification: TradingNotification;
  onDismiss: (id: string) => void;
  darkMode?: boolean;
  position?: "top-right" | "top-left" | "bottom-right" | "bottom-left" | "top-center" | "bottom-center";
}

// ============================================================================
// ICON MAPPING
// ============================================================================

const NOTIFICATION_ICONS: Record<NotificationType, React.ElementType> = {
  success: CheckCircle,
  warning: AlertTriangle,
  error: XCircle,
  info: Info,
  trade_win: TrendingUp,
  trade_loss: TrendingDown,
  order_placed: Check,
  order_cancelled: X,
  alert_triggered: BellRing,
  price_alert: Bell,
  signal: Zap,
};

/**
 * The primary action's fill used to be built at RUNTIME as
 * `colors.icon.replace("text-", "bg-")`. Tailwind discovers classes by scanning
 * source TEXT, so an assembled name is never emitted — and one of the five
 * variants produced `bg-subtle-foreground`, which appears nowhere else in the
 * codebase, so the "system" toast's primary action rendered with NO fill at all.
 * Literal strings are the only form the compiler can see.
 */
const ACTION_FILL: Record<NotificationType, string> = {
  success: "bg-success text-success-foreground",
  warning: "bg-warning text-warning-foreground",
  error: "bg-destructive text-destructive-foreground",
  info: "bg-info text-info-foreground",
  trade_win: "bg-success text-success-foreground",
  trade_loss: "bg-destructive text-destructive-foreground",
  order_placed: "bg-primary text-primary-foreground",
  order_cancelled: "bg-muted text-foreground",
  alert_triggered: "bg-warning text-warning-foreground",
  price_alert: "bg-info text-info-foreground",
  signal: "bg-primary text-primary-foreground",
};

const NOTIFICATION_COLORS: Record<NotificationType, {
  bg: string;
  bgLight: string;
  border: string;
  borderLight: string;
  icon: string;
  text: string;
}> = {
  success: {
    bg: "bg-success/10",
    bgLight: "bg-success/10",
    border: "border-success/30",
    borderLight: "border-success",
    icon: "text-success",
    text: "text-success",
  },
  warning: {
    bg: "bg-warning/10",
    bgLight: "bg-warning/10",
    border: "border-warning/30",
    borderLight: "border-warning/30",
    icon: "text-warning",
    text: "text-warning",
  },
  error: {
    bg: "bg-destructive/10",
    bgLight: "bg-destructive/10",
    border: "border-destructive/30",
    borderLight: "border-destructive",
    icon: "text-destructive",
    text: "text-destructive",
  },
  info: {
    bg: "bg-primary/10",
    bgLight: "bg-primary/10",
    border: "border-primary/30",
    borderLight: "border-primary/30",
    icon: "text-primary",
    text: "text-primary",
  },
  trade_win: {
    bg: "bg-success/10",
    bgLight: "bg-success/10",
    border: "border-success/30",
    borderLight: "border-success",
    icon: "text-success",
    text: "text-success",
  },
  trade_loss: {
    bg: "bg-destructive/10",
    bgLight: "bg-destructive/10",
    border: "border-destructive/30",
    borderLight: "border-destructive",
    icon: "text-destructive",
    text: "text-destructive",
  },
  order_placed: {
    bg: "bg-primary/10",
    bgLight: "bg-primary/10",
    border: "border-primary/30",
    borderLight: "border-primary/30",
    icon: "text-primary",
    text: "text-primary",
  },
  order_cancelled: {
    bg: "bg-muted/10",
    bgLight: "bg-muted",
    border: "border-border/30",
    borderLight: "border-border",
    icon: "text-subtle-foreground",
    text: "text-subtle-foreground",
  },
  alert_triggered: {
    bg: "bg-warning/10",
    bgLight: "bg-warning/10",
    border: "border-warning/30",
    borderLight: "border-warning/30",
    icon: "text-warning",
    text: "text-warning",
  },
  price_alert: {
    bg: "bg-warning/10",
    bgLight: "bg-warning/10",
    border: "border-warning/30",
    borderLight: "border-warning/30",
    icon: "text-warning",
    text: "text-warning",
  },
  signal: {
    bg: "bg-primary/10",
    bgLight: "bg-primary",
    border: "border-primary/30",
    borderLight: "border-primary",
    icon: "text-primary",
    text: "text-primary",
  },
};

// ============================================================================
// COMPONENT
// ============================================================================

export const NotificationToast = memo(function NotificationToast({
  notification,
  onDismiss,
  darkMode = true,
  position = "top-right",
}: NotificationToastProps) {
  const [progress, setProgress] = useState(100);
  const Icon = NOTIFICATION_ICONS[notification.type] || Info;
  const colors = NOTIFICATION_COLORS[notification.type] || NOTIFICATION_COLORS.info;

  // Progress bar animation
  useEffect(() => {
    if (!notification.duration || notification.duration === 0) return;

    const startTime = Date.now();
    const duration = notification.duration;

    const interval = setInterval(() => {
      const elapsed = Date.now() - startTime;
      const remaining = Math.max(0, 100 - (elapsed / duration) * 100);
      setProgress(remaining);

      if (remaining === 0) {
        clearInterval(interval);
      }
    }, 50);

    return () => clearInterval(interval);
  }, [notification.duration]);

  // Theme classes
  const theme = {
    bg: darkMode ? "bg-surface-2" : "bg-card",
    border: darkMode ? colors.border : colors.borderLight,
    text: darkMode ? "text-white" : "text-foreground",
    textMuted: darkMode ? "text-muted-foreground" : "text-muted-foreground",
    iconBg: darkMode ? colors.bg : colors.bgLight,
  };

  // Animation variants based on position
  const getAnimationVariants = () => {
    const isRight = position.includes("right");
    const isTop = position.includes("top");
    const isCenter = position.includes("center");

    return {
      initial: {
        opacity: 0,
        x: isCenter ? 0 : isRight ? 100 : -100,
        y: isTop ? -20 : 20,
        scale: 0.9,
      },
      animate: {
        opacity: 1,
        x: 0,
        y: 0,
        scale: 1,
      },
      exit: {
        opacity: 0,
        x: isCenter ? 0 : isRight ? 100 : -100,
        scale: 0.9,
        transition: { duration: 0.2 },
      },
    };
  };

  // Format profit/loss for trade notifications
  const formatTradeData = () => {
    if (!notification.data) return null;
    const { profit, profitPercentage, amount, symbol, side } = notification.data;

    if (notification.type === "trade_win" || notification.type === "trade_loss") {
      return (
        <div className="mt-1.5 flex items-center gap-2 text-xs">
          {symbol && (
            <span className={theme.textMuted}>{symbol}</span>
          )}
          {side && (
            <span className={`px-1.5 py-0.5 rounded text-chart-xs font-medium ${
              side === "RISE"
                ? "bg-success/10 text-success-ink"
                : "bg-destructive/10 text-destructive-ink"
            }`}>
              {side}
            </span>
          )}
          {profit !== undefined && (
            <span className={`font-mono font-medium ${
              profit >= 0 ? "text-up" : "text-down"
            }`}>
              {profit >= 0 ? "+" : ""}{profit.toFixed(2)}
              {profitPercentage !== undefined && (
                <span className="ml-1 opacity-70">
                  ({profitPercentage >= 0 ? "+" : ""}{profitPercentage.toFixed(1)}%)
                </span>
              )}
            </span>
          )}
        </div>
      );
    }

    return null;
  };

  return (
    <m.div
      layout
      {...getAnimationVariants()}
      className={`relative w-80 rounded-lg border shadow-lg overflow-hidden ${theme.bg} ${theme.border}`}
    >
      {/* Progress bar */}
      {notification.duration && notification.duration > 0 && (
        <div className="absolute top-0 left-0 right-0 h-0.5 bg-foreground/10">
          <m.div
            className={`h-full ${colors.icon.replace("text-", "bg-")}`}
            style={{ width: `${progress}%` }}
            transition={{ duration: 0.05 }}
          />
        </div>
      )}

      <div className="flex items-start gap-3 p-3 pt-3.5">
        {/* Icon */}
        <div className={`flex-shrink-0 p-2 rounded-lg ${theme.iconBg}`}>
          <Icon size={18} className={colors.icon} />
        </div>

        {/* Content */}
        <div className="flex-1 min-w-0">
          <h4 className={`text-sm font-medium ${theme.text}`}>
            {notification.title}
          </h4>
          <p className={`text-xs mt-0.5 ${theme.textMuted} line-clamp-2`}>
            {notification.message}
          </p>
          {formatTradeData()}
        </div>

        {/* Dismiss button */}
        <button
          onClick={() => onDismiss(notification.id)}
          className={`flex-shrink-0 p-1 rounded-md transition-colors ${
            darkMode
              ? "text-subtle-foreground hover:text-white hover:bg-card/10"
              : "text-muted-foreground hover:text-foreground hover:bg-foreground/5"
          }`}
        >
          <X size={16} />
        </button>
      </div>

      {/* Actions */}
      {notification.actions && notification.actions.length > 0 && (
        <div className={`flex items-center gap-2 px-3 pb-3 pt-1`}>
          {notification.actions.map((action) => (
            <button
              key={action.id}
              onClick={action.onClick}
              className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${
                action.variant === "primary"
                  ? ACTION_FILL[notification.type] ?? "bg-primary text-primary-foreground"
                  : action.variant === "danger"
                  ? "bg-destructive text-destructive-foreground"
                  : darkMode
                  ? "bg-card/10 text-destructive-foreground hover:bg-card/20"
                  : "bg-foreground/5 text-foreground hover:bg-foreground/10"
              }`}
            >
              {action.label}
            </button>
          ))}
        </div>
      )}
    </m.div>
  );
});

export default NotificationToast;
