...
import { GalleryFormControl } from "./gallery";
...
export function FormControls({
  ...
  switch (column.type) {
    ...
    case "image": {
      /**
       * The box the upload is downscaled into.
       *
       * `processImageUploads` -> `imageUploader` sends these as the sharp
       * `resize({ fit: "inside" })` bounds, defaulting to 1024x728
       * (`data-table/utils/image.ts`). The picker never said so, so operators
       * had no way to know what an image would come back as — the question
       * "what dimensions does this field accept?" had no answer anywhere in
       * the UI. It accepts any dimensions; what it STORES is bounded, and that
       * is the number worth printing.
       */
      const maxWidth = (column as any).maxWidth ?? 1024;
      const maxHeight = (column as any).maxHeight ?? 728;
      const sizeHint = t("images_are_resized_to_fit_within_dimensions", {
        width: maxWidth,
        height: maxHeight,
      });
      return (
    ...
    case "gallery":
      /**
       * Its own box is bigger than an image field's — a gallery holds up to
       * `maxImages` slots — so the defaults here are deliberately larger than
       * the 1024x728 a single thumbnail is stored at. A gallery image is the
       * one a customer zooms into.
       */
      return (
        <GalleryFormControl
          field={field}
          ...
          title={displayTitle}
          description={description}
          maxImages={(column as any).maxImages ?? 12}
          maxWidth={(column as any).maxWidth ?? 1600}
          maxHeight={(column as any).maxHeight ?? 1600}
        />
      );
    case "rating":
      return (
