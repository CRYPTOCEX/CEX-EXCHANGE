      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            role="combobox"
            aria-expanded={open}
            /* Disabled, not absent — the same idiom `select.tsx` in this folder
               settled on. The trigger keeps its box, its badges, its label, its
               description and its error line while the options resolve; only
               OPENING it waits, because there is nothing to choose from yet.

               Note what is NOT skeletoned: the badges in `displayContent`. They
               come from `field.value`, which is the record's own saved value
               and is in hand before this control mounts — unlike `select.tsx`,
               whose trigger has to look the chosen id up in the FETCHED option
               list and therefore does need a `Loadable`. Skeletoning a value
               that is already known would be inventing a wait. */
            disabled={loading}
            className={cn(
        ...
        >
          {/*
            The `loading` arm that used to head this chain is gone, and the
            trigger's `disabled={loading}` is what replaced it: a popover whose
            trigger cannot be pressed cannot be open, so the state that spinner
            painted is now unreachable rather than merely unattractive. That is
            the same shape `select.tsx` in this folder arrived at — disable the
            control, keep every part of it, delete the second tree.

            It also removes a real defect rather than a cosmetic one. The
            spinner was a `p-4` row about 56px tall inside a popover that opens
            to 320px of options, so the panel opened at a sixth of its height
            and then grew — and Radix repositions a popper when its content
            resizes, so the whole panel JUMPED, sometimes flipping from below
            the trigger to above it.

            `fetchErr` keeps its own arm: an error is a resolved state with
            nothing to search or select, not a pending one.
          */}
          {fetchErr ? (
            <div className="p-3 text-sm text-destructive-ink bg-destructive/10">
              {t("error")}: {fetchErr}
            </div>
          ) : (
            <>
