...
export const TableHeaderComponent = React.memo(
  ({ columns, showActions }: TableHeaderProps) => {
    ...
    /* Subscribing to `visibleColumns`/`columns` is what makes hiding a column
       actually hide its HEADING. `getVisibleColumns` is a stable store action,
       so it is inert both as a subscription and as a memo dep: without these
       two the header never re-rendered on a toggle, and even when some other
       store change forced a render the memo below returned its stale list —
       the cells disappeared while the <th> stayed. */
    ...
    // (custom bulk actions count as an action too).
    ...
    return (
      <TableHeader
        className={cn(
          ...
          "bg-linear-to-r from-muted/60 via-muted/40 to-muted/60",
          "backdrop-blur-sm",
      ...
      >
        <TableRow className="border-none hover:bg-transparent">
          ...
          {showSelectColumn && (
            <TableHead
              className={cn(
                "h-[var(--table-header-height)] px-[calc(var(--table-cell-padding-x)/2)] sm:px-[var(--table-cell-padding-x)]",
          ...
          {filteredColumns
            ...
            return (
              <TableHead
                key={column.key}
                className={cn(
                  "h-[var(--table-header-height)] whitespace-nowrap px-[var(--table-cell-padding-x)] border-none",
                ...
                /* The weight of a header label is a token, and the ACTIVE
                   label is one step above whatever that token says — see the
                   `[data-dt-frame]` block in globals.css. Marked here rather
                   than classed, because `font-medium`/`font-semibold` are
                   utilities and utilities outrank the components layer: left
                   in place they would have pinned both states and made the
                   control dead. */
              ...
              >
                <Button
                  variant="ghost"
                  className={cn(
                    "flex h-8 items-center gap-2 p-0 hover:bg-transparent",
          ...
          {showActions && (
            <TableHead className={cn(
              "h-[var(--table-header-height)] w-[80px] border-none rounded-r-xl",
