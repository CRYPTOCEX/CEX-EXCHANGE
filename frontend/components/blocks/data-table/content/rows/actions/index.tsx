...
export function RowActions({ row }: RowActionsProps) {
  ...
  // 7) If there's no view, edit, delete or custom action, hide the entire menu.
  //    `extraRowActions` has to count: a table whose operations are all custom
  //    (approve, disable, re-issue…) sets canView/canEdit/canDelete to false,
  //    and without this the menu that holds those operations never rendered —
  //    the actions were configured, mounted and unreachable.
  const hasAnyAction =
    hasViewAction ||
    hasEditAction ||
    hasDeleteAction ||
      ...
      >
        ...
        {tableConfig.extraRowActions &&
          tableConfig.extraRowActions(row, {
            ...
            permissions,
            refresh: () => void useTableStore.getState().fetchData(),
          })}
      </DropdownMenuContent>
    </DropdownMenu>
  );
