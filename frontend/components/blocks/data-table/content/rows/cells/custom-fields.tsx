...
export function CustomFieldsCell({
  ...
  if (!value) {
    return <span className="text-muted-foreground text-xs">{t("no_custom_fields")}</span>;
  }
  ...
  try {
    if (typeof value === "string") {
      ...
      return <span className="text-muted-foreground text-xs">{t("invalid_format")}</span>;
    }
  } catch (error) {
    return <span className="text-destructive text-xs">{t("error_parsing_fields")}</span>;
  }
