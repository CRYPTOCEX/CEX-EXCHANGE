...
//
// The fallback branch used to re-arm the SAME `imgError` flag it was already
// rendering under, so `setImgError(true)` was a no-op and a fallback asset that
// also 404'd left the browser's broken-image glyph on screen permanently. The
// two flags below make the degradation actually terminate: real logo -> shared
// placeholder -> the "No Logo" chip.
const GatewayImage: React.FC<{
  ...
  return (
    <div className="w-30 h-18 flex items-center justify-center bg-card rounded border">
      <img
        src={src}
        alt={alt}
...
const colorVariants: Record<BadgeVariant, string> = {
  ...
  // Was `text-primary`: the variant NAMED info painted the brand colour, so an
  // admin retheming `--info` moved nothing here. Same fix badge.tsx already
  // carries.
  info: "text-info",
  primary: "text-primary",
  // Was `text-secondary`. `--secondary` is a near-BACKGROUND surface token, so
  // that painted text in a background colour — near-invisible on its own
  // ground, exactly like the `bg-clip-text` headings. Ink is the foreground.
  secondary: "text-foreground",
  muted: "text-muted-foreground",
  default: "text-foreground",
};

/** The tone a value resolves to centrally, as an ink class. */
        ...
        <div className="shrink-0">
          {config.image.size === "gateway" ? (
            <GatewayImage
              ...
              alt={primaryValue || t("gateway_logo")}
              // Gateway logos are always a URL asset; the per-row function form
              // of `fallback` is for text placeholders and has no meaning here.
              fallback={
                typeof config.image.fallback === "function"
                  ? undefined
          ...
          <div className="flex flex-wrap gap-2 mt-1">
            {config.metadata.map((item: MetadataItem, index: number) => {
              ...
              if (item.options) {
                ...
                // With no explicit `color`, fall through to the central status
                // table rather than flat grey. `statusTone()` returns `neutral`
                // for anything it does not recognise, so non-status compound
                // options (roles, categories, names) render exactly as before —
                // only real statuses gain their tone.
                const colorClass = option?.color
                  ? colorVariants[option.color]
                  : TONE_TEXT[statusTone(String(roleId))];
