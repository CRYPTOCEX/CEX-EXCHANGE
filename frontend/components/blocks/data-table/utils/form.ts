...
/**
 * A date field is "blank" when it carries no date at all.
 *
 * `""` is the value the picker writes back when its clear button is pressed
 * and the value `getDefaultValueForType` seeds for an untouched field, so it —
 * not `undefined` — is what an empty date actually looks like in form state.
 */
function isBlankDate(val: unknown): boolean {
...
function createBaseSchemaForType(
  ...
  switch (type) {
    ...
    case "date": {
      /**
       * An empty date is legal unless the field is required.
       *
       * `new Date("")` is an Invalid Date, so the bare refine this replaces
       * failed EVERY blank date with "Invalid date" — including ones the form
       * itself documents as leave-empty ("leave empty to apply immediately").
       * Marking the column optional was no escape either: `.optional()` admits
       * `undefined`, never the empty string the picker actually holds, so the
       * message survived and the record could not be saved at all.
       *
       * The required case is handled here rather than by the generic
       * min-length branch below, which only fires for a ZodString — `.refine()`
       * has already turned this into a ZodEffects, so that branch has never
       * reached a date and an operator who skipped a mandatory date read
       * "Invalid date" for a date they never typed.
       */
      validator = z
        .union([z.string(), z.null(), z.undefined()])
        .refine(
          (val) => isBlankDate(val) || !isNaN(new Date(val as string).getTime()),
          { message: "Invalid date" }
        );
      if (required) {
        validator = validator.refine((val) => !isBlankDate(val), {
    ...
    case "gallery": {
      // Each slot is either a file the operator just picked or a path already
      // on the server, and the two coexist for the whole life of the form —
      // an edit that adds one image to four stored ones submits both shapes.
      validator = z.array(
        z.union([
          z.instanceof(File),
          z.string().refine(isValidImageUrl, { message: "Invalid url" }),
        ])
      );
      if (required) {
        validator = (validator as z.ZodArray<z.ZodTypeAny>).min(1, {
          message: title ? `${title} is required` : "Required",
        });
      }
      break;
  ...
  // "date" is deliberately absent: its validator is a ZodEffects by this point,
  // so the instanceof guard below could never match it. It carries its own
  ...
  if (Array.isArray(config.metadata)) {
    ...
    if (metaItem) {
      ...
      switch (metaItem.type) {
        ...
        case "date": {
          // Blank is absent, not invalid — see the "date" case in
          // createBaseSchemaForType. Returned directly because the shared
          // `required` pipe below is a ZodString and would report a blank date
          // as a type mismatch rather than as a missing value.
          const dateSchema = z
            .union([z.string(), z.null(), z.undefined()])
            .refine(
              (val) =>
                isBlankDate(val) || !isNaN(new Date(val as string).getTime()),
              { message: "Invalid date" }
            );
          return required
            ? dateSchema.refine((val) => !isBlankDate(val), {
                message: `${metaItem.title || fieldKey} is required`,
              })
            : dateSchema.optional();
        }
        case "select":
...
export const formatDataForForm = (data: any, columns: ColumnDefinition[]) => {
  ...
  columns.forEach((column) => {
     ...
     * Handle gallery.
     *
     * Without a branch of its own this fell to the `value ?? ""` fallback, and
     * `""` is the one thing the control and the array schema both reject — a
     * product that has never had extra images would arrive at the form as an
     * empty string and fail validation on a field the operator never touched.
     * `parseJsonField` is here because the column is `DataTypes.JSON`: prod
     * MySQL hands it back as an array, local MariaDB as the raw string.
     */
      ...
      formattedData[column.key] = Array.isArray(parsed)
        ? parsed.filter((entry: unknown) => typeof entry === "string" && entry)
        : [];
    }
    ...
    // Fallback.
      ...
      const value = column.key.includes(".")
        ? getNestedValue(data, column.key)
        : formattedData[column.key];
      // Textareas backed by JSON columns receive the parsed array/object —
      // stringify it so the field round-trips as editable JSON text
      // (previously this rendered "[object Object]" and failed validation).
      if (
        ...
        try {
          formattedData[column.key] = JSON.stringify(value, null, 2);
        } catch {
          formattedData[column.key] = "";
        }
        return;
...
export const processFormValues = (
  ...
  /**
   * Which keys the operator can actually see.
   *
   * The edit view seeds form state from the WHOLE row, so `values` carries
   * `createdAt`, `updatedAt` and `deletedAt` alongside the fields on screen.
   * Any rewriting below has to leave those alone — turning a live row's blank
   * `deletedAt` into an explicit `null`, for instance, is a write the operator
   * never asked for.
   */
  const editableKeys = formConfig
    ? getFormConfigFieldKeys(formConfig, isEdit)
    : null;
  ...
  columns.forEach((column) => {
    ...
    if (column.optional) {
      ...
      if (column.type === "rating") {
        ...
        // Dates keep their blank so the pass below can send it as an explicit
        // `null`. Dropping the key would make a stored date unclearable: the
        // API would never hear that the operator emptied it.
        if (val === undefined) delete processedValues[column.key];
      } else {
  ...
  /**
   * An untouched image field is `""`, and `""` is not a valid image.
   *
   * `getDefaultValues` seeds image fields with an empty string and nothing
   * strips it: the `column.optional` cleanup above is keyed on a different
   * flag than `formConfig`'s `required: false`, so leaving an image blank sent
   * `image: ""` to the API. Several models validate that column with an
   * `is: ^/(uploads|img)/` regex, and Sequelize skips built-in validators only
   * for null/undefined — an empty string is checked, and fails. The record was
   * then rejected for the one field the operator had deliberately left empty.
   *
   * `null` is left alone: that is the value `ImageUpload` sends when an image
   * is REMOVED, and it has to reach the API to clear the column.
   */
  for (const key of imageFieldKeys(columns)) {
    if (processedValues[key] === "") delete processedValues[key];
  }

  /**
   * A blank date has to travel as `null`, not `""`.
   *
   * Route schemas declare these columns `format: "date-time"`, and `""` is not
   * a date-time — so a record saved with a deliberately empty date was refused
   * by the request validator ("must match format date-time") even once the form
   * itself accepted the blank. `null` passes those schemas, because they are
   * declared `nullable`, and it is also the only value that CLEARS a date
   * already stored: dropping the key, the way blank images are handled above,
   * would leave the old date in place on edit.
   */
  for (const [configKey, valueKey] of dateFieldKeys(columns)) {
    if (
      editableKeys &&
      !editableKeys.has(configKey) &&
      !editableKeys.has(valueKey)
    ) {
      continue;
    }
    if (processedValues[valueKey] === "") processedValues[valueKey] = null;
  }
...
/**
 * Every date picker, compound ones included, as
 * `[key the form config names, key the value ends up under]`.
 *
 * The two differ whenever a column declares `baseKey`: the baseKey remap above
 * has already moved the value across by the time this is read, but the form
 * config still refers to the column by its own key.
 */
function dateFieldKeys(columns: ColumnDefinition[]): Array<[string, string]> {
  const keys: Array<[string, string]> = [];
  for (const column of columns) {
    if (column.type === "date") {
      keys.push([column.key, column.baseKey || column.key]);
    } else if (column.type === "compound" && column.render?.config) {
      const { metadata } = column.render.config;
      if (Array.isArray(metadata)) {
        for (const item of metadata) {
          if (item.type === "date") {
            keys.push([item.key, item.baseKey || item.key]);
          }
        }
      }
    }
...
/** Every value key that renders as an image picker, compound ones included. */
function imageFieldKeys(columns: ColumnDefinition[]): string[] {
  const keys: string[] = [];
  for (const column of columns) {
    if (column.type === "image") {
      keys.push(column.key);
    } else if (column.type === "compound" && column.render?.config?.image?.key) {
      keys.push(column.render.config.image.key);
    }
  }
  return keys;
}
