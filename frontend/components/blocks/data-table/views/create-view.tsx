...
export function CreateView({ columns, title, formConfig, hasHero = false }: CreateViewProps) {
  ...
  const handleSubmit = useCallback(
    async (values: any) => {
      try {
        ...
        const processedValues = columns
          ? processFormValues(values, columns, formConfig, false)
          : values;
         ...
         * Field errors FIRST.
         *
         * `$fetch` returns `validationErrors` alongside `error`, not instead
         * of it, so testing `error` first meant this branch never ran and no
         * per-field message ever reached the form — the operator got a toast
         * naming a problem but nothing marking the field that caused it. A
         * failed image upload arrives the same way now, tagged to the image
         * field by `handleSubmit`.
         *
         * The general toast is `$fetch`'s job and it already fires; repeating
         * it here would just stack two identical toasts.
         */
        if (validationErrors) {
          ...
          Object.entries(validationErrors).forEach(([fieldName, errorObj]) => {
            ...
            form.setError(fieldName, {
              type: "server",
              message: errorMessage || t("invalid_input"),
            });
          });
        } else if (error) {
    ...
    [columns, formConfig, apiEndpoint, form, defaultValues, goToOverview]
  );
  ...
  const viewDescription = configDescription || `${"Add"} ${t("new").toLowerCase()} ${title.toLowerCase()}`;
