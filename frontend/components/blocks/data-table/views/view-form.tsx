...
interface ProcessedGroup {
  // Always set - falls back to the source index when the config omits an id
  id: string;
  title: string;
...
function processFormConfigGroups(
  ...
  const mapped = config.groups
    .map((group, groupIndex): ProcessedGroup | null => {
      ...
      return {
        // id is optional in FormGroupConfig, so fall back to the source index:
        // it is unique and stable across renders for a given config.
...
function getFieldSpan(column: ColumnDefinition): string {
  /* `gallery` belongs here for the same reason `image` does, only more so: its
     control is a grid of up to twelve tiles, and in a third of the form width
     that grid is two tiles wide. */
    ...
    return (
      <Form {...form}>
        ...
        >
          ...
          >
            {/* The non-hero form's sticky bar. Only shown when the page has no
                hero — with one, `header/hero.tsx` becomes the fixed form header
                and carries Cancel/Save itself.

                It carried a fifth layer, `OrbsAnimation`, in a hue pair that
                defaulted to amber/orange on edit and emerald/cyan on create:
                the two STATUS palettes, on a bar whose only state is "is this
                form dirty" — which the unsaved chip beside it already says in
                words. The surface stays (opaque base plus the neutral wash);
                only the colour left. */}
            {!hasHero && (
              <div className="sticky top-0 z-50 -mx-4 md:-mx-6 lg:-mx-8 border-b overflow-hidden mt-16">
                {/* Solid background layer */}
                <div className="absolute inset-0 bg-background" />
                {/* Gradient overlay */}
                <div className="absolute inset-0 bg-linear-to-r from-transparent via-muted/30 to-transparent dark:via-surface-2/50" />
                  ...
                  <div className="flex items-center justify-between py-4">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      {form.formState.isDirty && (
                        <span className="hidden sm:flex items-center gap-1.5 text-xs text-warning-ink shrink-0 px-2 py-1 rounded-md bg-warning/10 border border-warning/20">
                      ...
                      >
                        <Card className="overflow-hidden">
                          ...
                          <CardContent className="p-4 pt-4">
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-[var(--field-gap)]">
