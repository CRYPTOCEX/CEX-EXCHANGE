...
import { onDesignTokensChanged } from "@/lib/live-style";
import {
  buildChartOverrides,
  buildChromeOptions,
  buildStudiesOverrides,
  repairPersistedChartColors,
} from "./chart-theme";
...
interface TradingViewChartProps {
  ...
  orders?: any[];
  // Mirrors the chart-engine MarketType union. `dex` is accepted so
  // ChartSwitcher can forward whatever it was given, but see the datafeed
  // below: TradingView resolves its own symbol/provider and has no DEX
  // datafeed, so a `dex` value here selects the default feed rather than a
  // DEX one. The Swap terminal uses the chart-engine renderer, not this one.
...
const resolutionMap: Record<string, string> = {
  ...
  "30m": "30",
  "1h": "60",
  /* 2h was the one interval missing from all three of these tables while the
     chart-engine toolbar and every backend already offered it, so switching to
     TradingView on a 2h chart fell through to the default resolution. */
  "2h": "120",
  "4h": "240",
  "6h": "360",
  "8h": "480",
...
const TradingViewChartBase = ({
  ...
  const [provider, setProvider] = useState<string>(resolvePublicExchange(process.env.NEXT_PUBLIC_EXCHANGE));
  ...
  useEffect(() => {
    setProvider(resolvePublicExchange(process.env.NEXT_PUBLIC_EXCHANGE));
  }, []);
           ...
           try {
             const unsubscribe = marketDataWs.subscribe(
               ...
               (data: any) => {
                 ...
                 /*
                  * THE NEWEST BAR, BY TIMESTAMP — NOT `data[0]`.
                  *
                  * This read `data.data[0]` under a comment calling it "the
                  * latest", and it is not: this stream carries TWO shapes, and
                  * the common one is a SNAPSHOT of the last ~100 bars sent
                  * every couple of seconds, in CHRONOLOGICAL order. So `[0]`
                  * was the OLDEST bar in the window — up to a hundred
                  * intervals stale — and `onRealtimeCallback` ignores any bar
                  * older than the one it is drawing. Every realtime frame was
                  * therefore discarded and the chart only ever moved when
                  * something forced a `getBars` refetch: "the chart does not
                  * reflect the price changes", with a live Trades tab beside it.
                  *
                  * Chosen by MAX TIMESTAMP rather than `at(-1)` so it is
                  * correct for both shapes and cannot be broken again by a
                  * producer that reorders its window. The single-bar per-fill
                  * frame is the one-element case of the same rule.
                  */
                 const ohlcvData = (data.data as any[]).reduce(
                   (newest: any, candle: any) =>
                     Array.isArray(candle) &&
                     typeof candle[0] === "number" &&
                     (!Array.isArray(newest) || candle[0] > newest[0])
                       ? candle
                       : newest,
                   data.data[0]
                 );
  ...
  useEffect(() => {
    ...
    const initTradingView = async () => {
      ...
      /* The saved property tree outranks the overrides below, so a colour an
         earlier build persisted would keep throwing out of the pane constructor
         on every load. Must run BEFORE the widget reads localStorage. */
      repairPersistedChartColors();
      ...
      const widgetOptions = {
        ...
        overrides: {
          ...
          /* Colour comes last so the palette wins any collision with the
             booleans above, and so every colour in this object has been through
             the library's own dialect — see `chart-theme.ts`. */
          ...buildChartOverrides(),
        },
        studies_overrides: buildStudiesOverrides(),
      };
  ...
  /**
   * Re-read the palette and push it into the live widget.
   *
   * The overrides are plain strings sent across an iframe boundary, so they are
   * a SNAPSHOT of the tokens at the moment they were read — nothing inside the
   * widget re-resolves when the stylesheet changes. Every path that can move a
   * token therefore has to call this.
   */
  const applyPalette = useCallback(() => {
    if (!tvWidget?._ready || typeof tvWidget.applyOverrides !== "function") return;
    try {
      tvWidget.applyOverrides(buildChartOverrides());
      if (typeof tvWidget.applyStudiesOverrides === "function") {
        tvWidget.applyStudiesOverrides(buildStudiesOverrides());
      }
    } catch (error) {
      console.error("TradingView palette override failed:", error);
    }
  }, [tvWidget]);
  ...
  useEffect(() => {
    if (
      ...
      /* `changeTheme` resets the chart to TradingView's OWN theme colours —
         `applyTheme` merges the stock `paneProperties` (background, grid,
         crosshair, the whole candle palette) over whatever was overridden — so
         the palette has to be re-applied after it.

         And it has to be AFTER, not next to it: `changeTheme` returns a promise
         and resolves at least two microtasks later, so the previous
         side-by-side call was unconditionally overwritten by the theme it was
         meant to correct. That was invisible on a theme TOGGLE, where the
         `<html>` class change re-fires `applyPalette` a frame later, and
         permanent at chart MOUNT, where nothing else re-applies. */
      Promise.resolve(tvWidget.changeTheme(isDark ? "Dark" : "Light"))
        .then(applyPalette)
        .catch((error: unknown) => {
          console.error("TradingView theme change failed:", error);
        });
    }
  }, [isDark, chartReady, tvWidget, applyPalette]);

  /**
   * Follow the site designer while the chart is open.
   *
   * Saving a palette rewrites the theme <style> tag in place, which repaints
   * everything that reads `hsl(var(--token))` but cannot reach a value already
   * copied into the widget. Without this the owner had to reload to see their
   * own colours on the chart.
   */
  useEffect(() => {
    if (!chartReady || !tvWidget) return;
    return onDesignTokensChanged(applyPalette);
  }, [chartReady, tvWidget, applyPalette]);

  // Show loading state while TradingView is loading.
  //
  // The pane these placeholders stand in for is `--card` (see `paneBackground`),
  // so they are that surface — `bg-black` painted a black rectangle inside a
  // white card in light mode. The ink follows: `--overlay-foreground` is white
  // in BOTH themes and is only for a real photo/scrim ground.
  if (isTradingViewLoading) {
    return (
      <div className="w-full h-full flex-1 min-h-0 bg-card flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin h-8 w-8 border-2 border-primary border-t-transparent rounded-full mx-auto mb-4"></div>
          <p className="text-muted-foreground text-sm">{t("loading_tradingview")}…</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="w-full h-full flex-1 min-h-0 bg-card flex items-center justify-center">
        <div className="text-center text-destructive">
          ...
          <p className="text-xs text-subtle-foreground">{t("falling_back_to_native_chart")}…</p>
        </div>
      </div>
    );
  }
