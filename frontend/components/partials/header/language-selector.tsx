...
const getCurrentLanguageData = (locale: string) => {
  const languageMap: Record<string, { name: string; flag: string }> = {
    ...
    en: { name: "English", flag: "/img/flag/gb.webp" },
    ...
    <>
      {variant === "compact" ? (
        ...
        <m.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={() => setIsDrawerOpen(true)}
          className="flex items-center gap-2 px-3 py-2 bg-transparent hover:bg-muted rounded-lg transition-colors cursor-pointer"
        >
