...
export function useMenuTranslations(
  ...
  const getTitle = (item: any): string => {
    // If the item has a custom title override (set by addon aliases), use it directly
    if (item?._customTitle && item?.title) {
      return item.title;
    }
  ...
  const getDescription = (item: any): string => {
    // An admin-authored description (menu overrides, /admin/menus) has no
    // translation key. Same contract as `_customTitle` above — without this the
    // lookup below runs against a key that will never exist.
    if (item?._customDescription && item?.description) {
