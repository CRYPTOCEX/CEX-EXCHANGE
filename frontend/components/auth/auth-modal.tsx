...
/**
 * `@reown/appkit`, and the @walletconnect stack under it, is the heaviest
 * dependency in the app — and this modal is what put it in the server graph of
 * *every* route. The root layout renders GlobalAuthDetector → UnauthorizedAccess
 * → AuthModal, and the site header renders it as well, so a static import here
 * made Node evaluate appkit while rendering pages with no wallet UI at all.
 * That is where the terminal's "Lit is in dev mode" and the localStorage
 * ExperimentalWarning were coming from on /admin.
 *
 * The form only ever renders inside LazyWalletProvider, which is already
 * ssr:false, so it could never have rendered on the server — only its module
 * body ran, for nothing. Loading it the same way keeps appkit out of the server
 * graph and out of the bundle every page shares.
 */
const WalletLoginForm = dynamic(
  () => import("@/components/auth/wallet-login-form"),
  { ssr: false }
);
...
export function AuthModal({
  ...
  // Wallet login is a wallet_connect extension feature; /api/settings only
  // lists enabled extensions, and the backend endpoints enforce it server-side
  const walletConnectEnabled = useConfigStore((state) =>
    state.extensions.includes("wallet_connect")
  );
  ...
  useEffect(() => {
    try {
      ...
      setError(t("failed_to_process_reset_token_from_url"));
    }
  ...
  useEffect(() => {
    try {
      ...
      setError(t("failed_to_update_view"));
    }
  }, [initialView]);
  ...
  const handleViewChange = (newView: string) => {
    try {
      ...
      setError(t("failed_to_change_view"));
    }
  };
  ...
  const handleSuccess = () => {
    try {
      ...
      setError(t("failed_to_process_successful_action"));
  ...
  const handleLoginClick = () => {
    try {
      ...
      setError(t("failed_to_return_to_login"));
    }
  };
  ...
  const handleTokenSubmit = (token: string) => {
    try {
      ...
      setError(t("failed_to_process_token"));
    }
  };
  ...
  const handleRegistrationSuccess = (email: string, needsEmailVerification: boolean) => {
    try {
      ...
      setError(t("failed_to_process_registration_success"));
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      ...
      <DialogPrimitive.Overlay className="fixed inset-0 z-50 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 bg-primary/20 via-surface-2/40" />
      <DialogContent className="sm:max-w-xl p-0 border-none shadow-2xl rounded-xl w-[95vw] sm:w-full max-h-[90vh] overflow-hidden bg-linear-to-b from-background to-background/95 backdrop-blur-sm">
        ...
        {error && (
          <div
            className="bg-destructive/10 border border-destructive/20 text-destructive-ink px-4 py-3 rounded-lg mx-6 mt-6 relative z-10"
            role="alert"
          >
        ...
        >
          {view === "forgot-password" ? (
          ...
          ) : view === "wallet-login" && walletConnectEnabled ? (
            <div className="p-8">
              <LazyWalletProvider cookies="" fallbackVariant="modal">
                <WalletLoginForm
                  onSuccess={handleSuccess}
            ...
            <div className="p-8">
              {view === "register" ? (
                ...
                // default to login, including when wallet-login is requested
                // but the wallet_connect extension is disabled
                <LoginForm
                  onSuccess={handleSuccess}
