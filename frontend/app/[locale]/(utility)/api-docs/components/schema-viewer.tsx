...
/**
 * JSON types are categorical — a boolean is not "more successful" than a
 * number — so they take `--chart-*` slots. They used to be green / amber / red-
 * adjacent palette shades, which under a class-for-class rename would have made
 * `boolean` a success and `number` a warning.
 *
 * Slots match the syntax theme in code-block.tsx so the same value reads the
 * same colour in the schema tree and in the example payload beside it. Measured
 * as ink on `--card`, light / dark: chart-1 5.01/5.17, chart-2 4.99/5.19,
 * chart-3 4.55/5.59, chart-4 5.23/4.90, chart-6 4.78/5.35.
 */
  ...
  if (isReferenceObject(schema)) {
    ...
    return (
      <div className={cn("flex items-center gap-2 py-1", className)}>
        ...
        {/* A $ref always resolves to a named object, so it wears the object slot. */}
        <span className="text-chart-4 font-mono text-sm">${refName}</span>
      </div>
    );
  }
                ...
                <td className="py-3 px-4 text-muted-foreground">
                  ...
                  {param.example !== undefined && (
                    <span className="block text-xs mt-1">
                      {tCommon("example")}:  <code>{JSON.stringify(param.example)}</code>
                    </span>
                  )}
                </td>
              </tr>
