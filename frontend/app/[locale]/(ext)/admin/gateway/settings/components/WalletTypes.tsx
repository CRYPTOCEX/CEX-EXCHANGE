...
export default function WalletTypesField({
  ...
  /*
    THREE `h-24` BARS, AND THE EXPLANATORY ALERT THROWN AWAY WITH THEM.
    ==========================================================================
    The `<Alert>` at the top of this panel is static copy — it explains how
    merchant settlement currencies work — and the pending branch dropped it
    along with the accordion, so the panel was 288px pending and ~460px
    resolved. The bars themselves were `h-24` against accordion rows whose real
    height is a `py-4` row around a 40px switch/icon/label group.

    The alert now renders in both states, and the accordion's pending rows are
    built from the row's own boxes below.
  */
  return (
    <div className="space-y-6">
      ...
      {/* Three states. `walletTypes.length > 0` alone meant this panel showed
          its "no wallet types" fallback for the duration of the fetch. */}
      {loading ? (
                ...
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <div className="text-left">
                    <p className="font-medium">
                      <SkeletonText placeholder={tExt("spot_wallet")} />
                    </p>
                    <p className="text-sm text-muted-foreground">
                      <SkeletonText placeholder="00 currencies available" />
                    </p>
                  </div>
                </div>
              </div>
