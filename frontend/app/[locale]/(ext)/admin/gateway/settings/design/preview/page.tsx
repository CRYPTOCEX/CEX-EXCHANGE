  ...
  switch (previewState) {
    case "loading":
      return {
        ...
        /* The preview always has an ANSWER for auth, in every case including
           `not_authenticated`. `authResolved` distinguishes "signed out" from
           "not asked yet", and the designs render their pending treatment on
           the latter — so leaving it false here would preview every layout in
           its auth-pending state and the signed-out design could never be
           inspected at all. */
        authResolved: true,
        multiWallet: null,
      };
