...
export default function AdminPayoutsPage() {
  ...
  const handleApprove = async (payoutId: string) => {
    ...
    if (error) {
      toast.error(typeof error === 'string' ? error : t("failed_to_approve_payout"));
    } else {
  ...
  const handleReject = async (payoutId: string) => {
    ...
    if (error) {
      toast.error(typeof error === 'string' ? error : t("failed_to_reject_payout"));
    } else {
      toast.success(t("payout_rejected"));
      setRefreshKey((prev) => prev + 1);
    }
  };
