...
/**
 * Which venue a maker's market lives on.
 *
 * A market id names a row in `ecosystem_market` OR `futures_market` and the two
 * tables share a UUID space, so nothing may be keyed on the id alone. Absent on
 * a row written before futures support, which is by definition ECO.
 */
export type MarketMakerVenue = "ECO" | "FUTURES";

export interface MarketMaker {
  id: string;
  ecosystemMarketId: string;
  /** The venue. Treat an absent value as `"ECO"`, never as unknown. */
  marketType?: MarketMakerVenue;
  /**
   * Leverage the maker posts FUTURES orders at. Meaningless on ECO, where
   * orders are funded from the pool rather than margined.
   */
  ...
  /** The column, the model and the handler all call this `pauseOnHighVolatility`. */
