...
export function ActivityItem({ activity, onView, onDelete, index = 0 }: ActivityItemProps) {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case "DELIST":
        return <ListX className="h-5 w-5 text-subtle-foreground" />;
      case "BID":
        return <Gavel className="h-5 w-5 text-warning" />;
      case "OFFER":
        return <HandCoins className="h-5 w-5 text-primary" />;
      case "BURN":
        return <Flame className="h-5 w-5 text-destructive" />;
      default:
        return <Coins className="h-5 w-5 text-subtle-foreground" />;
    }
  };
  ...
  const getActivityDescription = () => {
    ...
    switch (activity.type) {
      case "MINT":
        return (
          <span>
            <span className="font-medium">{tokenName || tCommon("token")}</span> {t("minted_in")}{' '}
      ...
      case "TRANSFER":
        return (
          <span>
            <span className="font-medium">{tokenName || tCommon("token")}</span> transferred
            {fromUser && (
      ...
      case "SALE":
        return (
          <span>
            <span className="font-medium">{tokenName || tCommon("token")}</span> sold
            {activity.price && activity.currency && (
              <> for <span className="font-medium text-success">{formatPrice(activity.price, activity.currency)}</span></>
            )}
          </span>
        );
      case "LIST":
        return (
          <span>
            <span className="font-medium">{tokenName || tCommon("token")}</span> {t("listed_for_sale")}
      ...
      case "DELIST":
        return (
          <span>
            <span className="font-medium">{tokenName || tCommon("token")}</span> {t("removed_from_marketplace")}
          </span>
        );
      case "BID":
        return (
          <span>
            {t("bid_placed_on")} <span className="font-medium">{tokenName || tCommon("token")}</span>
            {activity.price && activity.currency && (
              <> for <span className="font-medium text-warning">{formatPrice(activity.price, activity.currency)}</span></>
            )}
          </span>
        );
      case "OFFER":
        return (
          <span>
            {t("offer_made_on")} <span className="font-medium">{tokenName || tCommon("token")}</span>
            {activity.price && activity.currency && (
      ...
      case "BURN":
        return (
          <span>
            <span className="font-medium">{tokenName || tCommon("token")}</span> {t("burned_from")}{' '}
            <span className="font-medium">{collection?.name}</span>
          </span>
        );
      default:
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute left-[30px] top-[60px] bottom-[-24px] w-[2px] bg-linear-to-b from-border via-border/50 to-transparent" />
