...
import { useAdminDashboardStore, type CurrencyAmount } from "@/store/p2p/admin-dashboard-store";
...
function moneyByCurrency(rows?: CurrencyAmount[]) {
  return rows?.length ? rows.map(row => `${row.amount.toLocaleString()} ${row.currency}`).join(" · ") : "—";
}
...
function TradeTimelineChart({
  ...
  data: { date: string; trades: number; volume: number | null; revenue: number | null }[];
}) {
      ...
      {trades.map((trade) => (
        ...
        >
          ...
          <div className="text-right">
            <p className="font-semibold">{trade.total?.toLocaleString() ?? "—"} {trade.priceCurrency ?? "(currency not recorded)"}</p>
        ...
        >
          ...
          <div className="text-right">
            <p className="font-semibold text-emerald-500">
              {moneyByCurrency(trader.volumeByCurrency)}
            </p>
          </div>
        </div>
      ))}
        ...
        bottomSlot={
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
            <StatsCard
              ...
              icon={Users}
              change={stats?.offerGrowth ?? undefined}
            ...
            <StatsCard
              ...
              icon={BarChart3}
              change={stats?.tradeGrowth ?? undefined}
            ...
            <StatsCard
              ...
              icon={Shield}
              change={stats?.disputeChange ?? undefined}
              changeLabel="from yesterday"
              index={2}
            ...
            <StatsCard
              ...
              value={moneyByCurrency(stats?.platformRevenueByCurrency)}
              icon={DollarSign}
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Trade count for the last 7 days</CardDescription>
                  </div>
                  <div className="flex items-center gap-4 text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="h-2.5 w-2.5 rounded-full bg-emerald-500" />
                      <span className="text-muted-foreground">Money totals are listed by currency below</span>
                    </div>
                  </div>
                </div>
              </CardHeader>
            ...
            <motion.div variants={containerVariants} className="grid gap-6 md:grid-cols-3">
              ...
              <GlassCard>
                <CardContent className="pt-6">
                    ...
                    <div>
                      ...
                      <div className="text-2xl font-bold mt-1">
                        {isLoadingStats ? (
                          ...
                          moneyByCurrency(stats?.totalVolumeByCurrency)
                        )}
                      </div>
                    </div>
                  ...
                  <div className="mt-4">
                    <p className="text-xs text-muted-foreground">
                      This week: {moneyByCurrency(stats?.weekVolumeByCurrency)}
                    </p>
                  </div>
                </CardContent>
              </GlassCard>
                    ...
                    <div>
                      ...
                      <div className="text-2xl font-bold mt-1">
                        {isLoadingStats ? (
                          ...
                          moneyByCurrency(stats?.avgTradeValueByCurrency)
                        )}
                      </div>
                    </div>
              ...
              <GlassCard>
                ...
                <CardContent>
                  {isLoadingStats ? (
                    ...
                    <CurrencyDistChart data={stats?.currencyDist ? [...stats.currencyDist.rows, ...(stats.currencyDist.other > 0 ? [{ currency: "Other", count: stats.currencyDist.other }] : [])] : []} />
                  )}
                </CardContent>
              </GlassCard>
          ...
          <motion.div variants={containerVariants} className="space-y-6">
            ...
            <GlassCard gradient="bg-gradient-to-br from-emerald-500/10 to-blue-500/10">
              ...
              <CardContent>
                {isLoadingStats ? (
                  ...
                  <div className="space-y-2"><p>Open disputes: {stats?.disputeStats?.open ?? "—"}</p><p>Overdue disputes: {stats?.disputeStats?.breached ?? "—"}</p><p>Overdue offer reviews: {stats?.offerReview?.breached ?? "—"}</p></div>
                )}
                <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                  <div className="p-2 rounded-lg bg-muted/50">
                    <p className="text-muted-foreground">Pending</p>
                    <p className="font-semibold">{stats?.offerReview?.pending ?? stats?.pendingOffers ?? "—"}</p>
                  </div>
                  <div className="p-2 rounded-lg bg-muted/50">
            ...
            <GlassCard>
              <CardHeader>
                ...
                <CardDescription>Trade activity and volume by currency</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <GlassCard gradient="bg-gradient-to-br from-rose-500/10 to-amber-500/10">
              ...
              <CardContent>
                <RecentDisputesSection disputes={stats?.disputeQueue || []} loading={isLoadingStats} />
