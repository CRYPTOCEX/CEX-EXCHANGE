...
export default function AdminTradeDetailsClient() {
  ...
  const handleSendMessage = async () => {
    ...
    await sendAdminMessage(tradeId, adminMessage);

    if (!useAdminTradesStore.getState().sendingMessageError) {
      setActionMessage({
        type: "success",
      ...
      setActionMessage({
        type: "error",
        message: useAdminTradesStore.getState().sendingMessageError || "Failed to send message",
      });
    }
  };
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
            ...
            <div className="flex flex-col items-center gap-2 px-6">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground">{`${trade.total.toLocaleString()} ${trade.priceCurrency ?? "(currency not recorded)"}`}</p>
              </div>
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <Clock className="h-3 w-3" />
                <span>{trade.createdAt ? new Date(trade.createdAt).toLocaleString() : "—"}</span>
              </div>
            </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="flex items-center gap-3 p-4 rounded-lg border bg-muted/30">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="font-semibold truncate">{`${trade.total.toLocaleString()} ${trade.priceCurrency ?? "(currency not recorded)"}`}</p>
                      </div>
                    </div>
                    ...
                    {/* Time Remaining */}
                    {trade.status === "PENDING" && trade.paymentWindowEnforced && (
                      <div className="flex items-center gap-3 p-4 rounded-lg border bg-muted/30">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="font-semibold text-red-600 dark:text-red-400">{trade.paymentWindow} minutes from trade creation</p>
                        </div>
                      </div>
                    )}
                  </div>
