...
export default function CheckoutClient() {
  ...
  const { cart, clearCart, removeFromCart, isProcessingOrder } = useEcommerceStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // The API commits one product per request. Remove each confirmed purchase
      // so a later failure does not leave already-paid products in the retry cart.
      const results: any[] = [];
      ...
      for (const item of cart) {
        ...
        if (error) {
          ...
          // Keep failed and unsubmitted products in the cart.
          break;
        }
