    ...
    <div className={`${styles.container} p-4`}>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className={`font-semibold ${styles.header}`}>{t("payment_sources")}</h3>
        </div>
        <div className="flex items-center gap-2">
          {hasAllocations && (
            <button
      ...
      <div className="mb-4">
        <div className="flex justify-between text-sm mb-1">
          <span className={styles.subtext}>{t("payment_progress")}</span>
        ...
        {!isFullyAllocated && (
          <div className="flex items-center gap-1 mt-1">
            ...
            <span className="text-xs text-yellow-500">
              {tCommon("remaining")}: {formatCurrency(multiWallet.remainingAmount, paymentCurrency)}
            </span>
          </div>
        )}
        {isFullyAllocated && (
          <div className="flex items-center gap-1 mt-1">
            ...
            <span className="text-xs text-green-500">{t("payment_fully_allocated")}</span>
          </div>
        )}
      </div>
      ...
      {hasAllocations && (
        <div className="space-y-2 mb-4">
          <div className={`text-xs ${styles.subtext} uppercase tracking-wide`}>{t("selected_wallets")}</div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <div className={`text-xs ${styles.subtext} mt-0.5`}>
                      {tCommon("balance")}: {wallet.balance.toFixed(wallet.type === "SPOT" ? 8 : 2)} {allocation.currency}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
      ...
      {unallocatedWallets.length > 0 && (
        <div>
          ...
          >
            <span>
              {expanded ? tCommon("hide_detail") : tCommon("show")} available wallets ({unallocatedWallets.length})
            </span>
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className={`text-xs ${styles.subtext} mt-0.5`}>
                        {tCommon("balance")}: {wallet.balance.toFixed(wallet.type === "SPOT" ? 8 : 2)} {wallet.currency}
      ...
      {!multiWallet.canPayFull && (
        <div className={`mt-4 p-3 rounded-lg flex items-start gap-2 ${styles.badgeWarning}`}>
          ...
          <div className="text-sm">
            <div className="font-medium">{t("insufficient_funds_across_all_wallets")}</div>
      ...
      {hasAllocations && paymentAmount > 0 && (
        <div className="flex items-center gap-2 text-xs">
          ...
          <span className={isFullyAllocated ? "text-green-400" : "text-white/60"}>
            {isFullyAllocated ? tExt("ready") : formatCurrency(multiWallet.remainingAmount, paymentCurrency) + " left"}
          </span>
        </div>
      )}
