...
export interface CheckoutState {
  ...
  /**
   * Whether `isAuthenticated` is yet an ANSWER rather than a default.
   *
   * `isAuthenticated` is `!!user`, and the user store is empty until something
   * populates it, so `false` means "guest" and "not asked yet" identically.
   * Never render a signed-out state on `!isAuthenticated` alone — require this
   * too, or a signed-in customer is told to sign in.
   */
