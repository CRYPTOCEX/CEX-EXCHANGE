...
export default function CreateApiKeyClient() {
  ...
  const gate = useKycGate("use_gateway");
  ...
  const handleCreate = async (data: any) => {
    ...
    if (error) {
      toast.error(typeof error === "string" ? error : (error as any)?.message || t("failed_to_create_api_key"));
    } else {
      setCreatedKeys({
        ...
        <Card className="border-2">
          <CardHeader className="pb-4">
            <div className="flex items-center gap-3">
              <div className="p-2.5 rounded-lg border border-primary/20 bg-primary/10">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  readOnly
                  className="font-mono text-sm bg-surface-3"
                />
                <Button
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  <Input
                    ...
                    readOnly
                    className="font-mono text-sm bg-surface-3 pr-10"
                  />
                  <Button
                    variant="ghost"
                    size="icon"
  ...
  // API keys sign gateway requests as the merchant, so key creation carries the
  // same gate as merchant onboarding. It sits after the created-keys screen
  // above so a key that was just issued is never hidden from its owner. The page
  // has no sign-in prompt of its own, so "anonymous" renders nothing.
