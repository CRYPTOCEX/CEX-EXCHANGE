...
const WALLET_TYPE_CONFIG: Record<string, { label: string; icon: typeof Banknote; description: string; color: string }> = {
  FIAT: {
    ...
    color: "bg-success/10 text-success-ink border-success/20",
  },
  SPOT: {
    label: "Spot",
    icon: Coins,
    description: "Cryptocurrency payments (BTC, ETH, etc.)",
    color: "bg-warning/10 text-warning-ink border-warning/20",
  },
  ECO: {
    label: "Ecosystem",
    icon: CircleDollarSign,
    description: "Platform ecosystem tokens",
    color: "bg-primary/10 text-primary-ink border-primary/20",
  },
};
...
export default function ApiKeyForm({
  ...
  /*
    "No payment methods configured — contact admin" is a claim about the
    platform's settings, and it was unreachable during the settings fetch only
    because a spinner stood in front of it. With the rows rendering through the
    load, `enabledGlobalTypes` is empty for the whole request, so the guard
    names `loadingSettings` explicitly.
  */
  const showNoPaymentMethods =
    !loadingSettings && enabledGlobalTypes.length === 0;

  /*
    The rows to paint — and this one is unusual, in a good way: the stand-ins
    are the REAL wallet types.

    `WALLET_TYPE_CONFIG` is a compile-time table in this file, so every row's
    label ("Fiat"), icon and description are already in hand before the settings
    request is made. The only unknowns are which of the three the platform has
    turned on and how many currencies each carries. Reserving with the actual
    entries therefore reserves the actual widths, and the three keys are also a
    hard upper bound — the response can only ever be a subset — so the list can
    lose a row on settle but never gain one.

    A `null` `globalConfig` IS the pending marker for a row; the same body
    renders both.
  */
  const walletTypeRows: [string, any | null][] = loadingSettings
    ? Object.keys(WALLET_TYPE_CONFIG).map((type) => [type, null])
    : enabledGlobalTypes;
  ...
  return (
    <div className="space-y-6">
      {/* Header Info */}
      {/* Was a three-stop horizontal accent gradient used as a panel fill (R3).
          The banner is a surface, so it takes a rung and a hairline; the
          LIVE/TEST distinction stays where it is legible — on the icon tile. */}
      <div className="flex items-center gap-3 p-4 rounded-lg border border-border bg-surface-2">
        <div
        ...
        >
          <Key className={`h-6 w-6 ${keyMode === "LIVE" ? "text-success" : "text-warning"}`} />
        </div>
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold">
              {mode === "create" ? t("new_api_key") : tCommon("edit_api_key")}
            </h3>
          ...
          <p className="text-sm text-muted-foreground">
            {mode === "create"
              ? t("configure_your_api_key_settings_both")
              : t("update_your_api_key_configuration_changes")
            }
          </p>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label htmlFor="name">{tExt("key_name")}</Label>
                  {isFieldLocked("name") && (
              ...
              {mode === "create" && (
                <div className="p-4 rounded-lg border border-dashed border-border bg-surface-2">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Badge variant={hasFullAccess ? "default" : "secondary"}>
                  {hasFullAccess ? t("full_access") : t("selected", { selectedPermissionCount: String(selectedPermissionCount) })}
                </Badge>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex gap-2">
                <Input
                  placeholder={`${t("enter_ip_address_or_cidr")} (${"e.g., 192.168.1.1 or 10.0.0.0/24"})`}
                  value={newIp}
              ...
              {ipWhitelist.length > 0 ? (
                ...
                <div className="flex items-center gap-3 p-4 rounded-lg bg-warning/5 border border-warning/20">
                  ...
                  <div>
                    <p className="text-sm font-medium text-warning">{t("no_ip_restrictions")}</p>
                    <p className="text-xs text-muted-foreground">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Badge variant={selectedWalletCount === 0 ? "default" : "secondary"}>
                  {selectedWalletCount === 0 ? tExt("all_enabled") : t("selected_1", { selectedWalletCount: String(selectedWalletCount) })}
                </Badge>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              {/*
                The old pending state was a 24px spinner in a `py-8` box —
                about 96px — where the settled section is a caption plus up to
                three ~86px bordered rows, so this card grew by roughly 200px
                and pushed the tab's Save footer down with it.

                What made that avoidable is that the row LABELS are constants in
                this file, not response data: see `walletTypeRows` above. The
                rows render immediately with their real names, icons and
                descriptions, and only the toggle state and the currency count
                wait.
              */}
              {showNoPaymentMethods ? (
                ...
                <>
                  ...
                  <div className="grid gap-4">
                    {walletTypeRows.map(([type, globalConfig]) => {
                      const pending = !globalConfig;
                      // `allowedWalletTypes` is this form's own state, so it is
                      // truthful during load — but a row cannot be toggled
                      // before the platform's settings say it exists, so the
                      // control is inert while pending rather than wrong.
                      const isTypeEnabled = !pending && !!allowedWalletTypes[type];
                      ...
                      return (
                        <div key={type} className="overflow-hidden rounded-lg border border-border">
                          ...
                          <div
                            ...
                            onClick={() =>
                              !pending && toggleWalletType(type, !isTypeEnabled)
                            }
                          >
                            <div className="flex items-center gap-3">
                              {/* Icon, label and description are all local
                                  constants — nothing here is awaiting the
                                  server, so nothing here is skeletoned. */}
                              <div className={`p-2.5 rounded-lg border ${walletConfig.color}`}>
                            ...
                            {/* The Switch keeps its track in both states; `off`
                                is the safe rest position for a control that
                                cannot be operated yet. */}
                            <Switch checked={isTypeEnabled} disabled={pending} />
                          </div>

                          {/* Currencies — only ever open for an ENABLED type,
                              which cannot be true while pending, so this whole
                              expansion is naturally unreachable during load. */}
                          {isTypeEnabled && globalConfig.currencies && globalConfig.currencies.length > 0 && (
                            <div className="border-t border-border bg-surface-2 px-4 pb-4 pt-2">
        ...
        <TabsContent value="webhooks" className="mt-6 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="successUrl">{tExt("success_url")}</Label>
                <Input
                  id="successUrl"
                  type="url"
                  value={successUrl}
              ...
              <div className="space-y-2">
                <Label htmlFor="cancelUrl">{tExt("cancel_url")}</Label>
                <Input
                  id="cancelUrl"
                  type="url"
                  value={cancelUrl}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">{tExt("webhook_url")}</CardTitle>
              <CardDescription>
      ...
      <div className="flex items-center justify-between pt-4 border-t">
        ...
        {/* IN-FLIGHT STATE — the user pressed this button, so the spinner is
            the action's, and `<Button loading>` is the house spelling: it draws
            the design system's own `size-4` mark in the button's flex row (8px
            `gap-2`, where the hand-rolled `mr-2` made it 16px), sets
            `aria-busy`, and ORs `loading` into `disabled` — which is why
            `isSubmitting` is gone from the `disabled` expression rather than
            merely redundant there. `min-w-[140px]` already pinned the width, so
            this changes no box; it changes the mark, the gap and the
            announcement. */}
        <Button
        ...
        >
          {submitLabel || (mode === "create" ? tCommon("create_api_key") : tCommon("save_changes"))}
        </Button>
      </div>
    </div>
  );
