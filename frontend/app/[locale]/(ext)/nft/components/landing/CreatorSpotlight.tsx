...
import { publicName, publicHandle } from "@/utils/display-name";
...
export default function CreatorSpotlight() {
  ...
  const creators: Creator[] = (Array.isArray(topCreators) ? topCreators : []).map(creator => ({
    id: creator.id,
    /* This read `creator.displayName || creator.username`, which put the
       nftCreator row's own column first — and that column is seeded at profile
       creation from `firstName + lastName` lowercased. So the headline on a
       page that answers signed-out callers was the KYC name with the space
       taken out, while the ladder's answer sat unused in the line below it. */
    name: publicName(creator.user, creator.username || tCommon("creator")),
    /* The old fallback INVENTED a handle from that same column when the
       account had none — `@ogbeimole`, the surname included. An account with
       no handle has no handle, and gets the generic label rather than a
       manufactured one built out of the name it was hiding. */
    username: publicHandle(creator.user)
      ? `@${publicHandle(creator.user)}`
      : tCommon("creator"),
  ...
  // Tier is ordered, so it steps through one hue. `GOLD` and `BRONZE` both
  // returned `from-warning to-warning` — two tiers rendering identically, which
  // is what a status token used as decoration always eventually collapses into.
    ...
    <section ref={ref} className="py-20">
      <div className="container">
                ...
                <div className="px-6 -mt-12 relative z-10">
                  <div className="relative w-20 h-20 rounded-xl overflow-hidden border-4 border-card shadow-xl">
                    {creator.avatar ? (
                      ...
                      <div
                        className={cn(
                          "w-full h-full flex items-center justify-center text-primary-foreground font-bold text-2xl",
                          `bg-linear-to-br ${getTierColor(creator.verificationTier)}`
                        )}
                      >
                ...
                <div className="p-6 pt-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      ...
                      {creator.isFollowing ? tCommon("following") : t("follow")}
                    </Button>
        ...
        <m.div
          ...
          className={`mt-16 p-8 rounded-2xl bg-primary/10 border border-primary/20`}
        >
