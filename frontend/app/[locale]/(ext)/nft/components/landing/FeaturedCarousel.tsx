...
export default function FeaturedCarousel() {
  ...
  const featuredNFTs: FeaturedNFT[] = featuredTokens.slice(0, 6).map(token => ({
    ...
    currency: token.currentListing?.currency,
            ...
            <div className="flex gap-6 min-w-max pt-2">
              ...
              <Link key={`first-${nft.id}`} href={`/nft/token/${nft.id}`}>
                <m.div
                  ...
                  className="group w-64 bg-card border border-border rounded-lg overflow-hidden hover:border-primary/50 transition-all cursor-pointer"
                >
                  {/* NFT Image */}
                  <div className={`relative aspect-square bg-primary/20 flex items-center justify-center overflow-hidden`}>
                    <Image
                    ...
                    <div className="absolute inset-0 bg-linear-to-t from-overlay/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <span className="text-sm font-bold flex items-center gap-1.5">
                          {nft.currency && <CurrencyIcon currency={nft.currency} size={14} />}
                          {nft.price}
                        </span>
                      </div>
                      <Button
