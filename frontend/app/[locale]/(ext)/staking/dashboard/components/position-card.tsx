...
export default function PositionCard({ position }: PositionCardProps) {
  ...
  const totalDuration = endDate ? endDate.getTime() - startDate.getTime() : 0;
  const elapsedDuration = currentDate.getTime() - startDate.getTime();
  const progressPercentage = Math.min(
    100,
    Math.max(0, totalDuration > 0 ? (elapsedDuration / totalDuration) * 100 : 0)
  );

  // Calculate days remaining until end date.
  const daysRemaining = endDate ? Math.max(
    0,
    Math.ceil(
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center text-sm">
                ...
                <span className="ml-auto">{position.endDate ? formatDate(position.endDate) : "—"}</span>
              </div>
            </div>
