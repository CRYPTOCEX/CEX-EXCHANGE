...
/**
 * Pending state for /forex/account/[id]/withdraw.
 *
 * Also rendered directly by `client.tsx` (`if (isPending) return
 * <WithdrawLoading/>`), so it is a component on two paths.
 *
 * WHAT DRIFTED — no container at all, and the wrong ground
 * -------------------------------------------------------
 * WAS `min-h-screen bg-background text-foreground pt-20 p-6` — byte-identical
 * to the sibling deposit file, which is how one of them came to be describing
 * the other's page. This page is
 * `min-h-screen bg-linear-to-b from-background via-muted/10 to-background
 * dark:via-surface-2/30` → `<main className="container mx-auto px-4 pt-20
 * pb-24">`.
 *
 *  - NO `container`: the pending state ran FULL BLEED, so the stepper and the
 *    option cards spanned the whole viewport and snapped inwards on resolve.
 *    On a wide screen that is several hundred pixels of sideways movement.
 *  - No `px-4` either, which this route adds on top of `container`'s gutter
 *    (the deposit route does not — the two pages genuinely differ here, which
 *    a shared copy could never have captured).
 *  - GROUND: flat `bg-background` against the forex wash.
 *  - `p-6` put 24px on all four sides, including a second helping on top of
 *    the `pt-20`, against the page's `pb-24` (96px) at the foot.
 *  - `mb-4` under the header against the page's `mb-8`.
 *  - The heading and its subtitle were grey bars: the heading is a `t()` call
 *    and only the broker name and account id come from the fetch — which is
 *    what `client.tsx` already expresses with two `Loadable`s.
 *
 ...
 * here removes today's jump; changing it belongs in `client.tsx`.
 */
  ...
  return (
    <div className="min-h-screen bg-linear-to-b from-background via-muted/10 to-background dark:via-surface-2/30">
      <main className="container mx-auto px-4 pt-20 pb-24">
        {/* Header — the title is static; only the broker and account id wait. */}
