...
export function calculateEquityCurve(
  ...
  for (const order of sortedOrders) {
    /*
      ─────────────────────────────────────────────────────────────────────────
      A LOSS COSTS THE STAKE. `profit` IS ZERO ON EVERY LOSING ROW.

      `BinaryOrderService.ts:2474` writes `updateData.profit = 0` on a LOSS
      ("No profit on loss"), so `-Math.abs(profit)` was `-0` and every losing
      trade moved the equity curve by NOTHING. The consequences all point the
      same way, and all flatter:

        • max drawdown was permanently 0.0% (`advanced-metrics.tsx:167,229`,
          `summary-cards.tsx:152-153`)
        • Recovery Factor and Sortino divided by that zero and rendered ∞
        • the equity line rose monotonically on an account that was losing money

      `calculateTradingStats` in THIS FILE gets it right at line 40 — *"for
      losses, if profit is 0 or not set, use the bet amount as the loss"* —
      which is what proves this was an oversight rather than a different
      convention. A DRAW is a genuine zero: the stake is returned.
      ─────────────────────────────────────────────────────────────────────────
    */
    ...
    const pnl =
      order.status === "WIN"
        ? profit
        : order.status === "LOSS"
          ? -(profit ? Math.abs(profit) : Math.abs(order.amount) || 0)
          : 0;
    balance += pnl;
 ...
 *
 * This is P&L, i.e. direction of money, so it takes the price-direction tokens
 * rather than success/destructive (DESIGN-SYSTEM.md R1 and §4).
 */
 ...
 *
 * Three steps, not four. The 60/50/40 thresholds are a SEQUENTIAL scale — one
 * quantity read at different heights — and green/yellow/orange/red reads as
 * four unrelated statuses. Orange and red were also adjacent enough on a small
 * label to be unreliable to tell apart, so "poor" and "very poor" are one step:
 * good / marginal / poor. The number itself is always rendered beside this
 * colour, which is what carries the precision.
 */
export function getWinRateColor(winRate: number): string {
  if (winRate >= 60) return "text-up";
