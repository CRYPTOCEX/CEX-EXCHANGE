...
import { FieldLabel, NoticeStrip, Panel, SkeletonBox } from "./order-ui";
...
import { AlertCircle, AlertTriangle, Timer, Zap } from "lucide-react";
...
export function OrderPanel({
  ...
  /*
   * Resolved through `getOrderTypeConfig`, never indexed directly.
   *
   * `selectedOrderType` is persisted in `binary-trading-store`, so its value
   * comes from the user's localStorage and can name a type this build no
   * longer has. `ORDER_TYPE_CONFIGS[stale]` is `undefined`, and the four reads
   * below (`.label`, `.requiresBarrier`, `.requiresStrikePrice`,
   * `.requiresPayoutPerPoint`) each threw — taking the whole order panel into
   * its error boundary on the page where someone is placing money. The type
   * system cannot see it: the map is a `Record<BinaryOrderType, …>`, so every
   * index looks total.
   *
   * Named `active…` to stay clear of the unrelated `orderTypeConfig` inside the
   * profit-ratio memo below, which reads `binarySettings.orderTypes` — a
   * different map, already guarded.
   */
  const activeOrderTypeConfig = getOrderTypeConfig(selectedOrderType);
  ...
  /**
   * THE PAYOUT PER SIDE — because TOUCH and NO_TOUCH are not priced the same.
   *
   * `profitPercentage` above is one figure, and it was printed on both trading
   * buttons. For every order type but one that is correct. For TOUCH_NO_TOUCH
   * it is the product's central asymmetry being erased:
   * `BinaryOrderService.ts:494-502` multiplies the barrier rate by
   * `touchProfitMultiplier` (1.2 by default) or `noTouchProfitMultiplier` (0.7),
   * so on a 95% barrier with a 1,000 stake the NO_TOUCH button promised
   * **+950.00** against a contract that freezes 66.5% and credits **665.00**.
   *
   * The multipliers were already arriving — `use-binary-store.ts` declares both
   * on the TOUCH_NO_TOUCH config and the settings endpoint sends them. They were
   * received and never read.
   */
  const profitPercentageBySide = useMemo(() => {
    if (selectedOrderType !== "TOUCH_NO_TOUCH") return undefined;
    const config = binarySettings?.orderTypes?.TOUCH_NO_TOUCH;
    return {
      TOUCH: applySideProfitMultiplier(profitPercentage, "TOUCH_NO_TOUCH", "TOUCH", config),
      NO_TOUCH: applySideProfitMultiplier(profitPercentage, "TOUCH_NO_TOUCH", "NO_TOUCH", config),
    };
  }, [selectedOrderType, binarySettings, profitPercentage]);
      ...
      if (isLimited) {
        toast.warning(
          limitReason === "max_increases_reached"
            ? t("martingale_max_reached") || t("martingale_max_level_reached_resetting_to_base")
            : limitReason === "stop_loss_limit"
              ? t("martingale_stop_loss") || t("stop_loss_limit_reached")
              : t("martingale_balance_limit") || t("balance_limit_reached"),
          { duration: 3000 }
        );
      }
    }
    ...
    if (oneClickTrading.canOneClick(tradeAmount)) {
      ...
      try {
        ...
        if (success) {
          ...
          // Show success toast
          toast.success(
            tCommon("order_placed_successfully") || t("order_placed_for_usdt", { side: String(side), tradeAmount: String(tradeAmount) }),
            {
              duration: 2000,
  ...
  const renderConfirmationPortal = () => {
    /*
     * WHY THIS IS NOT A MOUNT GATE, THOUGH IT USED TO READ LIKE ONE
     * =============================================================
     * The old first condition was `!isMounted`, which made this look like the
     * defect the scanner's `mount-gate` rule hunts — a subtree withheld until
     * hydration, so the server renders a different page. It is not one, and the
     * rewrite says so rather than relying on the reader to work it out.
     *
     * This modal only exists after a click: `showConfirmation` starts `false`
     * and is only ever set from `placeOrder`. So the server, the browser's first
     * render and every render before the trader commits an order all return
     * `null` here from `showConfirmation` alone — no subtree is being withheld
     * and there is nothing for hydration to disagree about.
     *
     * What genuinely cannot happen on the server is the LAST line of this
     * function: `createPortal(..., document.body)` needs a real `document`.
     * That is a portal-target precondition, not a mount one, so it is now
     * stated as itself. `isMounted` stays in the component — `AmountSelector`
     * takes it for its own dropdown portal.
     */
    if (!showConfirmation || !pendingSide) return null;
   ...
   * Skeleton loader.
   *
   * This used to be a 207-line hand-painted copy of every child component,
   * with its own `dark:` fork on each of the ~60 elements — the single largest
   * block of theme-forked colour in the cluster, and a second place to keep in
   * step every time a field changed. Tokens are already theme-correct, so it
   * collapses to the same shapes at the same heights on the surface ramp.
   */
    ...
    <div className="space-y-2">
      ...
      <Panel className="px-2.5 py-2">
        <div className="flex items-center gap-2">
          <SkeletonBox className="w-[22px] h-[22px] bg-primary/15" />
          <SkeletonBox className="h-3.5 w-14" />
        </div>
      </Panel>

      {/* Amount and Expiry row — heights match AmountSelector / ExpirySelector */}
      <div className="flex gap-2">
        {[0, 1].map((i) => (
          <Panel key={i} className="flex-1">
            <div className="p-2">
              <div className="flex justify-between items-center mb-0.5">
                <FieldLabel>&nbsp;</FieldLabel>
                <div className="flex items-center gap-0.5">
                  <SkeletonBox className="w-[18px] h-[18px]" />
                  <SkeletonBox className="w-[18px] h-[18px]" />
                </div>
              </div>
              <div className="flex items-baseline gap-1">
                <span className="text-lg font-bold text-transparent">00:00 PM</span>
                <SkeletonBox className="w-3 h-3 ml-auto" />
              </div>
              <div className="flex items-center justify-between mt-1.5">
                <SkeletonBox className="h-[10px] w-10" />
                <SkeletonBox className="h-[18px] w-12 bg-up/10" />
              </div>
            </div>
          </Panel>
        ))}
      ...
      <Panel>
        <div className="px-2.5 py-2 bg-up/5">
          <div className="flex items-center justify-between">
            ...
            <SkeletonBox className="h-[18px] w-10 bg-up/10" />
          </div>
          <div className="flex items-baseline justify-between mt-1">
            <SkeletonBox className="h-3 w-6" />
            <SkeletonBox className="h-5 w-20 bg-up/20" />
          </div>
          <div className="h-1 rounded-full overflow-hidden mt-1.5 bg-surface-3">
            <div className="h-full w-[90%] rounded-full bg-up" />
          </div>
        </div>
      ...
      <Panel>
        ...
        <div className="grid grid-cols-3 gap-1.5 p-2 pt-1.5">
          {[0, 1, 2].map((i) => (
            <div
              key={i}
              className="rounded-lg p-1.5 bg-card border border-border flex items-center gap-1.5"
            >
              <SkeletonBox className="w-7 h-7 shrink-0" />
      ...
      <div className="grid grid-cols-2 gap-2">
        {[0, 1].map((i) => (
          <div
            key={i}
            className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl bg-surface-2 border border-border"
          >
            <SkeletonBox className="w-5 h-5 bg-primary/15" />
            <SkeletonBox className="h-2.5 w-8" />
            <SkeletonBox className="h-[15px] w-5" />
          </div>
        ))}
      </div>
  ...
  return (
    <div
      ...
      className={cn(
        "flex flex-col h-full relative z-10 overflow-hidden transition-colors duration-300",
        "bg-card text-foreground",
        isMobile ? "w-full" : "border-l border-border w-[300px]"
      )}
    >

      {/* Main content area */}
      <div className="flex-1 overflow-y-auto h-0 min-h-0 scrollbar-thin">
        <div className="p-2">
            ...
            <div className="space-y-2">
                  ...
                  {(() => {
                    ...
                    if (enabledTypes.length === 0) {
                      // No types enabled - show nothing
                      return null;
                    }

                    // The type badge. Its border used to be built as
                    // `${typeColors.border}/30`, a class name assembled at
                    // runtime — Tailwind v4 never emitted it, so this control
                    // has been borderless the whole time.
                    const body = (
                      <>
                        <div className="flex items-center gap-2">
                          <div
                            className={cn(
                              "p-1 rounded",
                              ORDER_TYPE_ACCENT.tint,
                              ORDER_TYPE_ACCENT.ink
                            )}
                          >
                            <OrderTypeIcon orderType={selectedOrderType} size={14} />
                          </div>
                          <div className="text-xs font-medium">
                            {activeOrderTypeConfig.label}
                          </div>
                        </div>
                        {hasMultipleTypes && (
                          <svg
                    ...
                    if (hasMultipleTypes) {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "w-full px-2.5 py-2 rounded-lg border transition-all flex items-center justify-between",
                            "bg-surface-2 text-foreground",
                            ORDER_TYPE_ACCENT.border,
                    ...
                    return (
                      <div
                        className={cn(
                          "w-full px-2.5 py-2 rounded-lg border flex items-center gap-2",
                  ...
                  {activeOrderTypeConfig.requiresBarrier && (
                    <BarrierInput
                  ...
                  {activeOrderTypeConfig.requiresStrikePrice && (
                    <StrikePriceInput
                      orderType={selectedOrderType}
                      currentPrice={currentPrice}
                  ...
                  {activeOrderTypeConfig.requiresPayoutPerPoint && (
                    <PayoutPerPointInput
                      orderType={selectedOrderType}
                      currentPrice={currentPrice}
                  ...
                  <Panel className="flex items-center justify-between px-2.5 py-2">
                    <div className="flex items-center gap-1.5">
                      <Timer size={12} className="text-subtle-foreground" />
                      <FieldLabel className="tracking-normal">
                        {t("next_expiry_in")}
                      </FieldLabel>
                    </div>
                    <div className="font-mono text-sm font-semibold px-2 py-0.5 rounded bg-surface-3 text-foreground">
                      {timeToNextExpiry}
                    </div>
                  </Panel>

              {/* Error message — the platform refused the order */}
              {orderPlacementError && (
                <NoticeStrip tone="destructive" icon={AlertCircle}>
                  {orderPlacementError}
                </NoticeStrip>
              )}

              {/* Safe zone warning — a pause, not a failure */}
              {isInSafeZone && (
      ...
      <div className="space-y-2" data-tutorial="trade-buttons">
        ...
        {oneClickTrading.enabled && (
          <div className="flex items-center justify-center gap-1.5 px-2 py-1 text-warning">
            <Zap size={10} className="animate-pulse" />
            <span className="text-[10px] font-medium">
              {t("one_click_active") || t("one_click_active")}
            </span>
          </div>
        )}
        <DynamicTradingButtons
          ...
          profitPercentage={profitPercentage}
          profitPercentageBySide={profitPercentageBySide}
          disabled={isInSafeZone || isLoadingDurationsLocal}
          isMobile={isMobile}
          darkMode={darkMode}
