...
import { AnchoredPopover, FieldLabel, StepperButton } from "./order-ui";
...
const AmountSelector = memo(function AmountSelector({
  ...
  /*
    A ZERO BALANCE IS NOT A DIVISOR. `amount / 0` is `Infinity`, and `.toFixed(1)`
    on it renders the literal string "Infinity" — so an empty wallet displayed
    "Infinity% of balance" beside the stake field. An em dash says the same thing
    honestly: there is no percentage of nothing.
  */
  ...
  // A suggestion is advice, not a status: the win-streak nudge is the only one
  // that points at price direction, and the loss-streak nudge is a caution.
  const suggestionIcon = {
    history: <History size={10} className="text-subtle-foreground" />,
    trend_up: <TrendingUp size={10} className="text-up" />,
    trend_down: <TrendingDown size={10} className="text-warning" />,
    sparkles: <Sparkles size={10} className="text-subtle-foreground" />,
  } as const;
    ...
    <div className="relative flex-1">
      <div
        ...
        className={cn(
          ...
          showAmountDropdown ? "border-primary" : "border-border"
        )}
          ...
          <div className="mt-1.5">
            <div className="h-0.5 rounded-full overflow-hidden bg-surface-3">
              <div
                ...
                style={{ width: `${Math.min(Number(balancePercentage) || 0, 100)}%` }}
              />
            </div>
            <div className="text-[9px] mt-0.5 text-subtle-foreground">
              <span className="text-primary font-medium">
                {balancePercentage === null ? "—" : `${balancePercentage}%`}
              </span>
              <span className="ml-0.5">{tCommon("of_balance")}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {showAmountDropdown &&
        ...
        createPortal(
          ...
          >
            {/* Quick Amounts */}
            <div className="p-2">
              <div className="text-[9px] font-medium uppercase tracking-wide mb-1.5 text-subtle-foreground">
                {t("quick_amounts")}
              </div>
                ...
                {presetAmounts.map((presetAmount) => {
                  ...
                  return (
                    <button
                      key={presetAmount}
                      className={cn(
                        "py-1.5 px-1 rounded-lg text-[11px] font-semibold transition-all cursor-pointer",
                        isSelected
                          ? "bg-primary text-primary-foreground"
                          : "bg-surface-3 hover:bg-primary/20 text-foreground"
                      )}
                      onClick={() => {
              ...
              <div className="grid grid-cols-4 gap-1">
                {[1, 5, 10, 25].map((percent) => {
                  ...
                  return (
                    <button
                      key={percent}
                      className="py-1.5 rounded-lg text-[10px] font-semibold transition-all cursor-pointer bg-surface-3 hover:bg-primary/20 text-foreground"
                      onClick={() => {
            ...
            {smartSuggestions.length > 0 && (
              <div className="p-2 border-t border-border">
                <div className="text-[9px] font-medium uppercase tracking-wide mb-1.5 flex items-center gap-1 text-subtle-foreground">
                  <Sparkles size={9} />
                ...
                <div className="flex gap-1 flex-wrap">
                  {smartSuggestions.map(({ amount: suggestedAmount, reason, icon }, idx) => (
                    <button
                      ...
                      className="flex items-center gap-1 py-1.5 px-2 rounded-lg text-[10px] transition-all cursor-pointer bg-surface-3 hover:bg-primary/20 text-foreground"
                    >
                      {suggestionIcon[icon]}
            ...
            {/* Custom input */}
            <div className="p-2 border-t border-border">
              <div className="flex items-center rounded-lg overflow-hidden bg-surface-3 border border-border focus-within:border-primary">
                <span className="pl-2 text-[10px] text-subtle-foreground">
                  {currency}
                </span>
                <input
                  type="number"
                  className="flex-1 py-1.5 px-2 text-xs font-medium outline-none bg-transparent text-foreground"
                  value={inputValue}
                  onChange={(e) => {
                ...
                />
                <button
                  className="px-2 py-1 text-[10px] font-semibold text-primary hover:text-primary/80 cursor-pointer"
