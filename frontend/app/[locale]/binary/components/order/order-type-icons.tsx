...
/**
 * One accent for every order type.
 *
 * This used to be a five-hue registry (blue / purple / orange / green / red)
 * keyed by order type — colour standing in for identity that the icon and the
 * label already carry. Two of those hues were also status tokens, so "Call/Put"
 * read as a success and "Turbo" as an error. Worse, the only thing the colour
 * ever expressed at the call sites was SELECTION, which R2 assigns to the brand
 * accent. The five keys collapse to one accent; the icons stay distinct.
 */
export const ORDER_TYPE_ACCENT = {
  /** Tinted ground behind the type's icon. */
  tint: "bg-primary/15",
  /** Ink for that icon. */
  ink: "text-primary",
  /** Border for the selected row / trigger. */
