  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative">
        <input
          ...
          className={cn(
            ...
            isOutOfRange
              ? "bg-destructive/10 border-destructive text-foreground placeholder-destructive/60"
              : "bg-surface-3 border-border text-foreground placeholder-subtle-foreground"
          )}
        />

        {/* Currency indicator */}
        <div className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-medium text-subtle-foreground">
      ...
      {suggestedPayouts.length > 0 && (
        <div>
          <div className="text-[10px] font-medium mb-1 flex items-center justify-between text-subtle-foreground">
            <span>{tCommon("quick_select")}</span>
            <span>
            ...
            {suggestedPayouts.map((suggestedPayout, idx) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "px-1.5 py-1.5 rounded text-xs font-medium border",
                    "transition-all cursor-pointer text-foreground",
                    "disabled:opacity-50 disabled:cursor-not-allowed",
                    isSelected
                      ? "bg-primary/15 border-primary"
                      : "bg-surface-3 border-border hover:bg-primary/10"
                  )}
                >
                  <div className="flex flex-col items-center">
                    <span className="text-[11px]">{formatPrice(suggestedPayout)}</span>
                    {potential && (
                      <span className="text-[9px] text-up">
      ...
      {/* Info */}
      <InfoNote>{t("profit_distance_to_barrier_payout")}</InfoNote>
    </div>
  );
}
