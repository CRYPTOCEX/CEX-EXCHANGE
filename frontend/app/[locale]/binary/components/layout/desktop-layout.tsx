...
export default function DesktopLayout({
  ...
  const [settingsInitialTab, setSettingsInitialTab] = useState<"trading" | "protection" | "sizing" | "sounds" | "notifications" | "limits">("trading");
  ...
  return (
    <div
      className="binary-workspace flex flex-col"
      style={{
        height: `calc(var(--vh, 1vh) * 100 - ${bottomSpacing}px)`,
        // Scaled + eased through the motion tokens rather than a literal
        // `0.3s ease-in-out`, which the design panel cannot reach and which
        // survives `prefers-reduced-motion`.
        transition:
          "height calc(300ms * var(--motion-scale, 1)) var(--motion-ease, ease-in-out)",
        ["--binary-font-scale" as any]: fontScale,
      }}
    >
      <Header
        balance={balance}
