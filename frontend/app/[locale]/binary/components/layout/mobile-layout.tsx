...
import { PositionsErrorBoundary } from "../error-boundaries";
...
export default function MobileLayout({
  ...
  // User-adjustable text size from the trading settings dialog
  ...
  //
  // Grounds here were `darkMode ? "bg-black" : "bg-white"` — a colour decided in
  // JS, which no theme token can reach and which forks to pure black / pure
  // white rather than to the page surface. `bg-background` is already
  // theme-aware, so the branch was redundant as well as off-token.
  return (
    <div
      className="binary-workspace flex flex-col w-full h-full overflow-hidden relative bg-background"
      style={{ ["--binary-font-scale" as any]: fontScale }}
    >
      ...
      >
        ...
        <div className="flex-1 relative min-h-0">
          ...
          <div
            className={`absolute inset-0 transform transition-transform duration-300 ease-in-out z-10 bg-background ${
          ...
          {/*
            ...
            "positions" was already in activePanel's union and in the swipe tabOrder,
            and `orders`/`handlePositionsChange` were already threaded down here for it,
            but no branch ever rendered it: swiping to that tab showed the chart.
          */}
          <div
            className={`absolute inset-0 transform transition-transform duration-300 ease-in-out z-10 bg-background ${
