...
import { EmptyState, ToneMark } from "../binary-ui";
...
/** One `label value` pair in a position row's 2x2 detail grid. */
...
interface ActivePositionsProps {
  ...
  /**
   * Retained so existing call sites keep type-checking; the panel no longer
   * branches on it, because every colour here is a theme-aware token.
   */
  theme?: "dark" | "light";
}
...
export default function ActivePositions({
  ...
  if (activeOrders.length === 0) {
    return (
      <div className={cn(className, "bg-background flex items-center justify-center")}>
        <EmptyState icon={Clock} message={t("no_active_positions")} className="py-0" />
      </div>
    );
  }
  ...
  return (
    <div
      ...
      className={cn(
        className,
        "bg-background border-border border-r flex flex-col transition-all duration-200",
    ...
    >
      ...
      <div
        className={cn(
          "border-border border-b flex items-center transition-all duration-200",
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-2 space-y-2">
            {activeOrders.map((order) => {
              ...
              const side = isBullishSide(order.side) ? "up" : "down";
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    /*
                     * Selection used to render `'hover:' + hoverBgClass.replace('hover:','')`
                     * — which evaluates to the SAME `hover:bg-…` string as the
                     * unselected arm, so the ternary was a no-op and a picked
                     * position never looked picked. Selection is `primary`, and
                     * it now persists without hovering.
                     */
                    isSelected
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center space-x-2">
                      ...
                      {/* Tint + the written side; the ink stays on `foreground`
                          because `text-up` on `bg-up/20` is under 4.5:1 in light. */}
                      <span
                        className={cn(
                          "text-xs px-2 py-1 rounded text-foreground",
                          side === "up" ? "bg-up/20" : "bg-down/20"
                        )}
                      >
                        {order.side}
                      </span>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <Metric
                      label={`${t("entry")}:`}
                      value={<MoneyFigure value={`${order.entryPrice.toFixed(2)} ${getCurrency(order.symbol)}`} />}
                    />
                    <Metric
                      label={`${t("current")}:`}
                      value={<MoneyFigure value={`${symbolPrice.toFixed(2)} ${getCurrency(order.symbol)}`} />}
                    />
                    <Metric
                      label={`${t("amount")}:`}
                      value={<MoneyFigure value={`${order.amount.toFixed(2)} ${getCurrency(order.symbol)}`} />}
                    />
                    <Metric
                      label="P/L:"
                      value={
                        <span className="inline-flex items-center gap-1 font-medium">
                          <ToneMark tone={isProfitable ? "up" : "down"} size={6} />
                          <MoneyFigure
                            value={`${isProfitable ? "+" : ""}${profitLoss.toFixed(2)} ${getCurrency(order.symbol)}`}
                          />
                        </span>
                      }
                    />
                      ...
                      >
                        {
                          (() => {
                            ...
                            /* SVG paint attributes can read the same tokens the
                               classes do, so the sparkline follows the theme
                               instead of two hardcoded hexes. */
                            const lineColor =
                              currentValue > 0 ? "hsl(var(--up))" : "hsl(var(--down))";
                  ...
                  {/* Action Buttons — tinted grounds with `foreground` ink; the
                      hue rides the icon, which is what keeps them legible on
                      the light theme's much paler status tokens. */}
                      ...
                      className={cn(
                        ...
                        canCashOutOrder(order)
                          ? cn(
                              ...
                              isProfitable
                                ? "bg-up/20 hover:bg-up/30"
                                : "bg-warning/20 hover:bg-warning/30"
                            )
                          : "bg-surface-3 text-subtle-foreground cursor-not-allowed"
                      )}
                    >
                      <DollarSign
                        className={cn(
