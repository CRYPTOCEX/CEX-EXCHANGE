  ...
  if (initError) {
    return (
      <div className="min-h-screen-mobile h-screen-mobile bg-background flex items-center justify-center">
        <div className="flex flex-col items-center gap-4 p-8 bg-card rounded-lg max-w-md text-center border border-border">
          {/* Initialisation failed — the platform refused to start, which is
              `destructive`. The copy stays on `foreground`: `text-destructive`
              on its own tint measures 3.9:1 in light mode. */}
          <div className="w-12 h-12 rounded-lg bg-destructive/20 flex items-center justify-center">
            <svg className="w-6 h-6 text-destructive" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
            </svg>
          </div>
          <h2 className="text-xl font-semibold tracking-tight text-foreground">{t("initialization_error")}</h2>
          <p className="text-foreground text-sm">{initError}</p>
          <button
            type="button"
