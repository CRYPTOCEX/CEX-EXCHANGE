...
export default function BlogNav() {
  ...
  // `Boolean("false")` is true — this used to keep the Apply link visible on a
  // blog whose operator had closed the author programme.
  ...
  //
  // Every item sets `_customTitle` because this nav resolves its own labels
  // through `blog_blog` / `common` above. SiteHeader otherwise discards `title`
  // and looks up `menu.<key>.title` instead, which meant two things here: the
  // blog and articles tabs logged "[i18n] Missing translation" on every render
  // (no such key exists, and creating one would duplicate blog_blog across all
  // 90 locales), while categories and authors silently rendered the blog-ADMIN
  // menu's wording because `menu.categories.title` happens to exist.
