  ...
  const formattedSpotMarkets = useMemo((): Market[] => {
    return spotMarkets.map((market) => {
      ...
      if (isActiveMarket && activeMarketData) {
        ...
        /*
          "tickersWs returns 'change' as the percentage value" is true of the
          CEX stream only (`exchange/ticker/index.ws.ts:247` aliases it). The
          ECO and FUTURES engines return BOTH `change` (the absolute price
          delta, `close - open`) and `percentage`, and `ecoData` is preferred
          right here — so a market moving 60,000 -> 61,200 (+2%) rendered
          "+1200.00%". The active branch above already prefers `percentage`;
          every other row did not.
        */
        const raw = ecoData[symbol] || spotData[symbol];
        tickerData = raw
          ? { ...raw, change: raw.percentage ?? raw.change ?? 0 }
          : { last: 0, change: 0, quoteVolume: 0 };
      }
