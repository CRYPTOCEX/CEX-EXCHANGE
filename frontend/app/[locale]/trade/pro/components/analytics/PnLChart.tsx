...
export const PnLChart = memo(function PnLChart({
  ...
  useEffect(() => {
    ...
    // Ink + structure. Read fresh on every draw so a theme toggle is picked up
    // without a remount.
    const mutedColor = themeColor("--tp-text-muted", "--subtle-foreground");

    // Draw grid
    ctx.strokeStyle = themeColor("--tp-border", "--border");
    ctx.lineWidth = 0.5;
    ...
    if (zeroY > padding.top && zeroY < dimensions.height - padding.bottom) {
      // The break-even reference, so it has to read ABOVE the grid but below the
      // series. This was the one hardcoded `rgba(255,255,255,0.3)` in the file —
      // no theme branch at all, so on the light Pro surface (--surface-3, near
      // white) the zero line was invisible. Label ink at 0.6 lands at the same
      // weight on the dark ground and finally paints on the light one.
      ctx.strokeStyle = withAlpha(ctx, mutedColor, 0.6);
      ctx.lineWidth = 1;
      ctx.setLineDash([4, 4]);
      ctx.beginPath();
    ...
    gradient.addColorStop(0, withAlpha(ctx, color, 0.25));
    gradient.addColorStop(1, withAlpha(ctx, color, 0));
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-xs text-[var(--tp-text-muted)]">
            {chartMode === "cumulative" ? t("cumulative") : tCommon("daily")} profit/loss
          </p>
        </div>
      ...
      >
        ...
        {tooltip && (
          <div
            className="absolute pointer-events-none z-10 bg-[var(--tp-bg-secondary)] border border-[var(--tp-border)] rounded-lg px-3 py-2 shadow-lg"
            style={{
              left: tooltip.x + 10,
              top: tooltip.y - 60,
