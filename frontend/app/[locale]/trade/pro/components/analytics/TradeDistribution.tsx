    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2 mb-4 p-2 bg-[var(--tp-bg-secondary)] rounded-lg">
        <div className="text-center">
          <div className="text-[10px] text-[var(--tp-text-muted)]">{tCommon("total_volume")}</div>
          <div className="text-sm font-mono text-[var(--tp-text-primary)]">
            {/* NO `$`. Each `bySymbol` bucket is `amount × price` in ITS OWN
                quote asset (`exchange/trading/analytics/index.get.ts:218`), so
                this sum adds USDT notionals to BTC ones. The magnitude is
                still useful; the dollar sign was a claim about a unit the
        ...
        <div className="text-center">
          <div className="text-[10px] text-[var(--tp-text-muted)]">{tCommon("total_p_l")}</div>
          <div
            className={cn(
              "text-sm font-mono",
