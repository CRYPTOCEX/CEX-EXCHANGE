...
/**
 * A signed money figure, or an em dash when the platform could not measure it.
 *
 * These four read `stats.avgWin || 0` upstream and rendered "+$0.00" in green
 * on every account, because no order source carries a per-order P&L. A zero in
 * green is a claim; a dash is not.
 */
function money(value: number | null | undefined, sign: "+" | "-"): string {
...
export const WinRateCard = memo(function WinRateCard({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      ctx.strokeStyle = withAlpha(ctx, redColor, 0.25);
    ...
    >
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="flex justify-between items-center">
            <span className="text-xs text-[var(--tp-text-muted)]">{tCommon("avg_win")}</span>
          ...
          <div className="flex justify-between items-center">
            <span className="text-xs text-[var(--tp-text-muted)]">{tCommon("avg_loss")}</span>
