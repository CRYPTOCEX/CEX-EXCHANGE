...
function Spotlight({ targetRect, padding = 8, radius = 8 }: SpotlightProps) {
  if (!targetRect) {
    return (
      <div className="fixed inset-0 bg-overlay/70 backdrop-blur-sm z-[var(--z-tour-scrim)] transition-opacity duration-300" />
    );
  }
    ...
    <div className="fixed inset-0 z-[var(--z-tour-scrim)] pointer-events-none">
      ...
      <svg className="w-full h-full">
        ...
        {/* `--overlay` is dark in both themes, which is the point of a scrim.
            The alpha matches the no-target arm above (`bg-overlay/70`) so the
            two states of this component are the same darkness. */}
        <rect
          x="0"
          y="0"
          width="100%"
          height="100%"
          fill="hsl(var(--overlay) / 0.7)"
      ...
      <m.div
        ...
        className="absolute border-2 border-[var(--tp-blue)] shadow-[0_0_20px_hsl(var(--primary)/0.5)]"
        style={{
...
function Tooltip({
  ...
  return (
    <m.div
      ...
      className="fixed z-[var(--z-tour-content)] w-[calc(100vw-40px)] max-w-[360px] rounded-xl shadow-2xl border bg-[var(--tp-bg-primary)] border-[var(--tp-border)]"
      style={{
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={onNext}
            className="flex items-center gap-1 px-4 py-1.5 text-sm rounded-lg bg-[var(--tp-blue)] hover:bg-[var(--tp-blue)]/90 text-[var(--tp-blue-fg)] font-medium transition-colors"
          >
            {isLastStep ? (
              <>
...
function CompletionScreen({ onClose, onRestart }: CompletionScreenProps) {
  ...
  return (
    <m.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[var(--z-tour-content)] flex items-center justify-center bg-overlay/80 backdrop-blur-sm"
    >
      <m.div
      ...
      >
        ...
        <m.div
          ...
          className="w-20 h-20 mx-auto mb-6 rounded-full bg-linear-to-br from-[var(--tp-blue)] to-[var(--tp-green)] flex items-center justify-center"
        >
          {/* Glyph sits on the blue→green tile above; both accents pair with
              the same foreground token, so this reads in either theme. */}
          <CheckCircle2 className="w-10 h-10 text-[var(--tp-blue-fg)]" />
        </m.div>
        ...
        <div className="flex gap-3 justify-center">
          ...
          <button
            onClick={onClose}
            className="flex items-center gap-2 px-6 py-2 rounded-lg bg-[var(--tp-blue)] hover:bg-[var(--tp-blue)]/90 text-[var(--tp-blue-fg)] font-medium transition-colors"
          >
            {t("start_trading")}
          </button>
        </div>
