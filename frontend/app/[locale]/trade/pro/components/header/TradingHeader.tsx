      ...
      >
        ...
        {ticker && !isMobile && (
          <div className="hidden lg:flex items-center h-full border-r border-[var(--tp-border)] px-3">
            <MarketStats ticker={ticker} pricePrecision={metadata?.precision?.price} quoteCurrency={symbol.includes("/") ? symbol.split("/")[1] : undefined} />
          </div>
        )}
        ...
        <QuickActions compact={isMobile} symbol={symbol} />
      </header>
    </>
  );
});
