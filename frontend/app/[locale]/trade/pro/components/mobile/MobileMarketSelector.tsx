...
/**
 * How many pending rows to paint while the market lists are being fetched.
 *
 * A market list has no knowable length, so this reserves the SCROLLER and lets
 * the count settle. A row is ~62px (a two-line left column inside `py-3`), so
 * eight rows is ~496px — taller than any phone viewport this sheet opens on,
 * which is the point: the list is already scrollable in the pending state, as
 * it certainly will be with several hundred markets in it.
 */
const PENDING_ROW_COUNT = 8;

/**
 * What a row reads from before the market lists arrive.
 *
 * The spinner this replaced was also the guard that guaranteed a row HAD a
 * market, so one frozen constant stands in rather than `?.` down the row body.
 * `volume24h` is 0 rather than `undefined` on purpose — see the volume line in
 * the row, which is the only element here whose PRESENCE changes the row
 * height, and therefore the only one that has to be reserved rather than
 * skipped.
 */
const PENDING_MARKET: Market = {
  symbol: "",
  currency: "",
  pair: "",
  price: 0,
...
export const MobileMarketSelector = memo(function MobileMarketSelector({
  ...
  /**
   * ONE list renders both states — a `null` entry is a row whose market has not
   * arrived. The alternative, a second column of grey bars, is the shape
   * SKELETONS.md rules out: a duplicate has nothing keeping it in sync with the
   * row it stands in for.
   */
  const rows: Array<Market | null> = isLoading
    ? Array.from({ length: PENDING_ROW_COUNT }, () => null)
    : formattedMarkets;

  /**
   * "No markets found" / "No favorites yet" is a RESULT and needs a finished
   * request to be one. `formattedMarkets` is `[]` before the subscriptions
   * deliver, so the `!isLoading` half is what stops this sheet opening on "Try
   * a different search term" at a trader whose search is fine.
   */
  const showEmptyState = !isLoading && formattedMarkets.length === 0;

  if (!isOpen) return null;

  const tabs: { id: TabType; label: string }[] = [
    { id: "watchlist", label: "★" },
    { id: "spot", label: tCommon("spot") },
    { id: "futures", label: tCommon("futures") },
  ];
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[var(--tp-bg-primary)] flex flex-col safe-area-inset">
      ...
      <div className="shrink-0 px-4 py-3 bg-[var(--tp-bg-secondary)]">
        <div className="relative">
          ...
          <input
            ...
            placeholder={`${t("search_by_name_or_symbol")}…`}
            className={cn(
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={cn(
              ...
              activeTab === tab.id
                ? "bg-[var(--tp-blue)] text-[var(--tp-blue-fg)] shadow-lg shadow-[var(--tp-blue)]/20"
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain">
        {/* The spinner this replaced was a 40px ring plus a caption in a `py-16`
            box — 168px standing in for a list that is 62px PER ROW and hundreds
            of rows long, on a phone. The three column headings above it stayed
            put while the thing they labelled was withheld entirely.

            Note the primitives: `Loadable`/`SkeletonText` from the core design
            system rather than trade/pro's local `Skeleton`. The local one is a
            box the CALLER sizes, and every fixed pixel height in this workspace
            is wrong at seven of the eight Text Size settings — the font scale
            here rescales `text-[9px]`..`text-[13px]` under the user's control.
            SkeletonText measures itself from the element it sits inside, so it
            follows the scale for free. The local primitive is untouched and
            still right for the panels that use it for genuinely box-shaped
            things. */}
        {showEmptyState ? (
          <div className="flex flex-col items-center justify-center py-16 text-[var(--tp-text-muted)]">
            ...
            <p className="text-base font-medium">
              {activeTab === "watchlist" ? tCommon("no_favorites_yet") : tCommon("no_markets_found")}
            </p>
            <p className="text-sm mt-1">
              {activeTab === "watchlist" ? t("tap_the_star_to_add_favorites") : tCommon("try_a_different_search_term")}
            </p>
          </div>
        ) : (
          <div
            className="divide-y divide-[var(--tp-border)]"
          ...
          >
            {rows.map((row, index) => {
              ...
              /* All four of these compare against PENDING_MARKET's empty
                 strings and zeroes, so every one would be a confident claim
                 about a market nobody has fetched: `"" === currentSymbol` is
                 false but `0 >= 0` is TRUE, which would paint every pending row
                 with a green up-chip. `!pending &&` is what stops that. */
              const isSelected =
                !pending &&
                market.symbol === currentSymbol &&
                market.marketType === currentMarketType;
              const isPositive = market.change24h >= 0;
              const isPopular = !pending && POPULAR_PAIRS.includes(market.symbol);
              const isFavorite = !pending && favorites.includes(market.symbol);

              return (
                <button
                  key={pending ? `pending-${index}` : `${market.marketType}-${market.symbol}`}
                  onClick={pending ? undefined : () => handleSelect(market)}
                  disabled={pending}
                  className={cn(
                    "w-full flex items-center px-4 py-3",
                    "transition-colors",
                    !pending && "active:bg-[var(--tp-bg-tertiary)]",
                    isSelected && "bg-[var(--tp-blue)]/8"
                  )}
                >
                  {/* Favorite star. The 32px slot and the button inside it are
                      chrome and render in both states; the STAR's FILL is the
                      claim — an unfilled star says "not favourited" about a
                      market with no symbol to look one up by — so while pending
                      the control is inert and the glyph is laid out but hidden,
                      with the pulse behind it. One element, one root, exact
                      box; a separate placeholder here would be a second
                      spelling of a 16px square. */}
                  <div className="w-8 flex-shrink-0">
                    <button
                      onClick={pending ? undefined : (e) => handleToggleFavorite(e, market)}
                      disabled={pending}
                      className={cn(
                        "p-1 -m-1 rounded-full transition-colors",
                        pending
                          ? "animate-pulse bg-[var(--tp-bg-tertiary)]"
                          : isFavorite
                            ? "text-[var(--tp-yellow)]"
                  ...
                  <div className="flex-1 text-left">
                    <div className="flex items-center gap-2">
                      <span className={cn(
                        ...
                        <Loadable loading={pending} placeholder="BTC">
                          {market.currency}
                        </Loadable>
                      </span>
                      <span className="text-xs text-[var(--tp-text-muted)]">
                        {/* The slash is chrome and stays; only the quote
                            currency after it waits. */}
                        /
                        <Loadable loading={pending} placeholder="USDT">
                          {market.pair}
                        </Loadable>
                      </span>
                      {/* HOT and ECO are genuinely optional on a real market, so
                          neither is reserved. Both are inline chips in an
                          `items-center` row, so their arrival settles the pair
                          sideways and cannot change the row height. */}
                      {isPopular && (
                    ...
                    {/* THE one element in this row whose presence is a height:
                        a `text-xs` line plus `mt-0.5` is ~18px on a ~62px row,
                        so a row without it and a row with it are different
                        sizes. That is why PENDING_MARKET carries
                        `volume24h: 0` and this branch checks `pending` FIRST —
                        the falsy 0 would otherwise take the `null` arm and every
                        pending row would grow 18px when the real volume landed.
                        Markets without volume still collapse it, which is the
                        rarer and smaller of the two movements. */}
                    {pending ? (
                      <div className="text-xs text-[var(--tp-text-muted)] mt-0.5">
                        {tCommon("vol")}: <SkeletonText placeholder="123.4M" />
                      </div>
                    ) : market.volume24h ? (
                      <div className="text-xs text-[var(--tp-text-muted)] mt-0.5">
                        {tCommon("vol")}: {formatVolume(market.volume24h)}
                      </div>
                    ) : null}
                  </div>
                  ...
                  <div className="w-20 text-right">
                    {/* The chip keeps its padding, radius and type in both
                        states; only its TONE is withheld, because `0 >= 0` is
                        true and a green chip on every row is a claim the feed
                        has not made. The neutral fill is the workspace's own
                        tertiary surface, so the box is identical in size to the
                        tinted one it becomes. */}
                    <span
                      className={cn(
                        "inline-block px-2 py-1 rounded text-xs font-semibold font-mono",
                        pending
                          ? "bg-[var(--tp-bg-tertiary)] text-[var(--tp-text-muted)]"
                          : isPositive
