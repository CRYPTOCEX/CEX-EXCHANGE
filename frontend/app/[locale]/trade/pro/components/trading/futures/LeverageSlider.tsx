...
export const LeverageSlider = memo(function LeverageSlider({
  ...
  return (
    // Padding and the section rule live on the parent now — the trading form is
    // one padded column of controls, so a control that pads itself sits out of
    // alignment with every field above and below it.
    <div>
      ...
      <div className="flex items-center justify-between mt-2 text-[10px]">
        <span className="text-[var(--tp-text-muted)]">{tCommon("risk_level")}</span>
        <span
          className={cn(
        ...
        >
          {value <= 5 && tCommon("low")}
          {value > 5 && value <= 20 && tCommon("medium")}
          {value > 20 && value <= 50 && tCommon("high")}
          {value > 50 && t("very_high")}
        </span>
      </div>
    </div>
  );
