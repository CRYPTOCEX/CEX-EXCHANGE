...
interface OrderParams {
  ...
  leverage?: number;
  /**
   * Post-only travels as timeInForce "PO", not as its own flag.
   *
   * No route on the platform reads a `postOnly` field; the ecosystem and spot
   * order routes both read `timeInForce` and nothing else. Sending both invited
   * a form where FOK and Post Only could be selected at once with only one of
   * them having any effect.
   */
  reduceOnly?: boolean;
  /**
   * Typed to the four the backend accepts; anything else is a 422.
   *
   * WHICH of the four a given order may carry is not decided here — the spot
   * route refuses a value its connected venue cannot express, and refuses
   * fill-or-kill and post-only on a market order at all. `timeInForceChoices`
   * in `advanced/AdvancedOptions` is the caller-side rule.
   */
