...
interface ClosePositionModalProps {
  ...
  /**
   * Whether partial size and limit-price closes may be offered.
   * The futures close route is full-market-close only, so callers wired to it pass
   * false rather than presenting controls the backend would ignore.
   */
  ...
  return (
    ...
    >
      <DialogContent
        ...
        className={cn(
          // The `size` scale emits `sm:max-w-sm`, so the 24rem cap only applies
          // at >=640px; below that only DialogContent's base
          // `max-w-[calc(100%-2rem)]` survives. The old panel was an
          // unconditional `max-w-sm`, so between ~416px and 639px this modal
          // had silently become full-bleed. An unprefixed cap restores it.
          "max-w-sm",
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-[var(--tp-border)]">
          {/* `text-base leading-normal` cancels the dialog-title defaults so the
              heading keeps the size and line-height it had before. */}
          <DialogTitle asChild className="text-base leading-normal">
            <h3 className="text-base font-semibold text-[var(--tp-text-primary)]">
              {tCommon("close_position")}
            </h3>
          </DialogTitle>
          <button
            onClick={onClose}
        ...
        <div className="p-4 space-y-4">
          ...
          {/* Close type — only when the caller's endpoint supports a limit close */}
          {allowPartial && (
          <div>
          ...
          {/* Amount — full size only when partial closes are unsupported */}
          {allowPartial ? (
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              >
                {tCommon("max")}: {position.size.toFixed(4)}
              </button>
            </div>
            <input
              type="text"
            ...
            <div className="flex items-center justify-between py-2 px-3 bg-[var(--tp-bg-tertiary)] rounded-lg">
              <span className="text-xs text-[var(--tp-text-muted)]">Closing</span>
              <span className="text-sm font-mono text-[var(--tp-text-primary)]">
                {position.size.toFixed(4)} (full position, market)
              </span>
            </div>
          )}
          ...
          {/* Estimated P&L */}
          <div className="flex items-center justify-between py-2 px-3 bg-[var(--tp-bg-tertiary)] rounded-lg">
            <span className="text-xs text-[var(--tp-text-muted)]">{t("est_p_l")}</span>
            {/* Signed figure in the mono face, ticker in the interface face. */}
            <span
              className={cn(
        ...
        <div className="flex gap-2 p-4 border-t border-[var(--tp-border)]">
          ...
          <button
            ...
            className={cn(
              "flex-1 py-2.5",
              "text-sm font-semibold text-[var(--tp-red-fg)]",
              "rounded-lg",
              "transition-colors",
          ...
          >
            {isSubmitting ? `${tCommon("closing")}…` : tCommon("close_position")}
          </button>
        </div>
      </DialogContent>
    </Dialog>
