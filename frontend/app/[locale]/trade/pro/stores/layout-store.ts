...
/**
 * Width for a side panel: the user's dragged pixel width when there is one,
 * otherwise the default constant.
 *
 * Deliberately keyed on `sizePx`, NOT on `size`. `size` holds a PERCENTAGE
 * (markets 15, chart 45, orderbook 20 …), so rendering it as a length would
 * collapse the markets panel to a 15px sliver. Panels only take a pixel width
 * once the user has actually dragged one, which keeps the default layout byte
 * -identical to before.
 */
function panelWidth(
  panel: TPPanelConfig | undefined,
  fallback: string
): string {
  if (panel?.collapsed) return COLLAPSED_WIDTH;
  const px = panel?.sizePx;
...
function computeGridColumns(panels: Record<string, TPPanelConfig>): string {
  const marketsWidth = panelWidth(panels.markets, PANEL_WIDTHS.markets);
  // The chart is the flexible track; it absorbs whatever the side panels leave.
  const chartWidth = PANEL_WIDTHS.chart;
  const orderbookWidth = panelWidth(panels.orderbook, PANEL_WIDTHS.orderbook);
  const tradingWidth = panelWidth(panels.trading, PANEL_WIDTHS.trading);
...
interface LayoutStore {
  ...
  /** Set an explicit pixel width from a resizer drag, clamped to sane bounds. */
  setPanelWidthPx: (panelId: string, px: number) => void;
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        setPanelWidthPx: (panelId, px) => {
          // Bounds keep a drag from making a panel unusable or eating the chart.
          const MIN = 160;
          const MAX = 640;
