    ...
    return futuresMarkets.map((market) => {
      ...
      if (isActiveMarket && activeMarketData) {
        ...
        tickerData = {
          ...
          // The futures ticker feed does not carry a funding rate, so `|| 0`
          // fabricated one: every futures row rendered a permanent green
          // 0.0000%. Pass the field through untouched — undefined means "this
          // feed supplies no rate", and FundingRateCell then renders nothing.
          // Funding itself IS settled; the operator-set rate lives on the
          // market row, not on the ticker.
          fundingRate: futuresData[tickerKey]?.fundingRate,
        };
      } else {
      ...
      return {
        ...
        rawPrice: toNum(tickerData.last),
        change: tickerData.last ? toNum(tickerData.change).toFixed(2) + "%" : "0.00%",
        rawChange: toNum(tickerData.change),
        volume: tickerData.last ? formatVolume(tickerData.quoteVolume || 0) : "0.00",
        rawVolume: toNum(tickerData.quoteVolume),
        isPositive: toNum(tickerData.change) >= 0,
        isTrending: market.isTrending || false,
        isHot: market.isHot || false,
        metadata,
        hasData,
        // `toNum(undefined)` is 0, and "0.0000%" is a truthy string, so the
        // absent-rate guard downstream never fired. Keep absent as absent.
        fundingRate:
          tickerData.fundingRate == null
            ? undefined
            : toNum(tickerData.fundingRate).toFixed(4) + "%",
        rawFundingRate:
          tickerData.fundingRate == null
            ? undefined
            : toNum(tickerData.fundingRate),
  ...
  const formattedMarkets = useMemo((): Market[] => {
    return markets.map((market) => {
      ...
      return {
        ...
        /*
          ─────────────────────────────────────────────────────────────────────
          `percentage`, NOT `change` — THEY ARE DIFFERENT QUANTITIES.

          The ecosystem and futures engines return BOTH: `change` is the
          absolute price delta (`close - open`) and `percentage` is the move as
          a percent (`ecosystem/utils/matchingEngine.ts:2888-2889`,
          `futures/utils/matchingEngine.ts:1167-1168`). Only the CEX ticker
          stream aliases them (`exchange/ticker/index.ws.ts:247`).

          This captured `percentage` into `tickerData` three lines up and then
          rendered `change` with a `%` after it. A BTC/USDT eco market moving
          60,000 -> 61,200 (+2%) published "+1200.00%" — and because the active
          market takes the branch above, the SELECTED row disagreed with the
          header (`trading-header.tsx:246-250`, which correctly reads
          `percentage`) on one screen.
          ─────────────────────────────────────────────────────────────────────
        */
        change: hasData
          ? toNum(tickerData.percentage ?? tickerData.change).toFixed(2) + "%"
          : null,
        rawChange: toNum(tickerData.percentage ?? tickerData.change),
        volume: hasData ? formatVolume(tickerData.quoteVolume || 0) : null,
        rawVolume: toNum(tickerData.quoteVolume),
        isPositive: toNum(tickerData.percentage ?? tickerData.change) >= 0,
        hasData,
      };
    });
    ...
    <div className="h-full flex flex-col bg-background overflow-hidden">
      ...
      >
        ...
        >
          ...
          <ColumnHeaders
            leftColumn={{ label: t("symbol"), sortField: "name" }}
            rightColumn={{ label: `24h ${t('change')}`, sortField: "change" }}
        ...
        >
          <SearchBar
            placeholder={`${tTradeComponents("search_futures")}…`}
            value={searchQuery}
            onChange={setSearchQuery}
          />
          ...
          <ColumnHeaders
            leftColumn={{ label: t("symbol"), sortField: "name" }}
            rightColumn={{ label: t("funding_rate"), sortField: "funding" }}
            sortCriteria={sortCriteria}
            onSort={handleSort}
          />
