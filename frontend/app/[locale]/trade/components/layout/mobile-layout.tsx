...
export default function MobileLayout({
  ...
  /*
    The order book is mounted on FIRST VISIT to the Book tab, and stays mounted
    after that.

    Every tab here is hidden with a CSS class rather than unmounted, which is
    right for the chart — remounting it is expensive — and wrong for the order
    book, which opens three WebSocket subscriptions and rebuilds a ladder on
    every animation frame. It was doing all of that from page load for every
    mobile visitor, including the ones who never opened the tab.

    Visit-gated rather than unmounted-when-hidden, so switching back to a book
    you have already opened is still instant and does not re-subscribe.
  */
  ...
  if (activeTab === "book" && !bookVisited) setBookVisited(true);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      >
        ...
        <TabsList className="w-full grid grid-cols-5 h-16 sm:h-14 rounded-none bg-surface-2 border-t border-border flex-shrink-0 pb-safe-or-2">
          <TabsTrigger
            value="chart"
