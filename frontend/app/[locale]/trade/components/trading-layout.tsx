...
/*
  There was a `PanelComponentRegistry` here mapping six panel ids to components,
  and nothing in the file ever read it — `renderPanel` carries the same mapping
  again as an if/else chain, and has to, because each panel takes a different set
  of props. Two copies of a mapping where one is unreachable is worse than one:
  the dead one looks like the dispatch table and is the first place someone adds
  a panel that then never renders.
*/
...
function TradingInterface({
  ...
  const renderPanel = (panelId: string) => {
    ...
    if (panelId === "markets") {
      ...
      // `metadata` carries the market's declared price/amount precision. Without
      // it the book inferred both from the price magnitude, so a market quoted to
      // four decimals around $2,000 had two of them rounded away.
      return <OrderBookPanel symbol={currentSymbol} marketType={getMarketType()} currency={currentMarket?.currency} pair={currentMarket?.pair} metadata={currentMarket?.metadata} />;
      ...
      return (
        <TradingFormPanel
          ...
          isFutures={isFutures}
          isEco={isEco || currentMarket?.isEco || false}
          onOrderSubmit={isFutures ? handleFuturesOrderSubmit : undefined}
        />
      );
    } else if (panelId === "orders") {
      // isEco (URL-derived, available on first render) must be consulted
      // before currentMarket, which resolves one commit later — otherwise
      // eco pages first subscribe SPOT order streams, then churn to eco.
      return <OrdersPanel pair={currentMarket?.pair} isEco={isEco || currentMarket?.isEco || false} symbol={currentSymbol} />;
    } else if (panelId === "alerts") {
      // The panel arms alerts against the market being looked at, so it needs
      // both the symbol and the family — BTC/USDT exists on spot, eco and
      // futures, and they are three different prices.
      return <AlertsPanel symbol={currentSymbol} marketType={getMarketType()} />;
    }
  ...
  return (
                        ...
                        <PanelGroup direction="vertical" className="h-full">
                          {dataPanels.map((panelId, index) => {
                            ...
                            return isCollapsed ? (
                              <div
                                key={panelId}
                                className="h-[30px] w-full flex justify-center bg-background border-b border-border"
                              >
                                <CollapsedPanel
                                  title={
                                    panelId === "orderbook"
    ...
    "
                ...
                <Panel
                  ...
                  className={cn(
                    "bg-background border-t border-border relative",
