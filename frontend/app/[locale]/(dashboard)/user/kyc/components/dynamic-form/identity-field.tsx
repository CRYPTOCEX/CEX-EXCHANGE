...
/**
 * How a document tile is painted. Only one tile is ever selected, so this is a
 * selection state and takes the accent (R2) — it is not five categories needing
 * five hues. Each entry is a complete class string: the old per-type hue name
 * was pasted into a template literal at render time, which Tailwind v4 cannot
 * see, so none of these tiles has ever painted a colour.
 */
const TILE_TONE = {
  card: {
    on: "border-primary bg-primary/5",
    off: "border-border hover:border-border-strong",
  },
  icon: {
    on: "bg-primary/10 text-primary-ink",
    off: "bg-muted text-muted-foreground",
  },
  // The selected tile's name stays on `foreground`: accent ink on its own tint
  // measures 4.07:1 in light mode, under the 4.5:1 body-text floor. The accent
  // is carried by the border and the icon, which only need 3:1.
  label: { on: "text-foreground", off: "text-muted-foreground" },
} as const;
    ...
    <div className="space-y-6">
      <div>
        <div className="flex justify-between items-center mb-2">
          ...
          {isComplete ? (
            <Badge
              variant="outline"
              className="bg-success/10 text-success-ink border-success/30 flex items-center gap-1"
            >
            ...
            <Badge
              variant="outline"
              className="bg-warning/10 text-warning-ink border-warning/30 dark:bg-warning/50"
            >
        ...
        <div className="mb-6">
          <div className="relative h-2 overflow-hidden rounded-full bg-muted">
            <m.div
              className="absolute left-0 top-0 h-full bg-linear-to-r from-primary to-primary/80 dark:to-primary/60"
              initial={{
                width: 0,
              }}
              animate={{
          ...
          {identityTypes.map((type) => {
            ...
            return (
              <m.button
                ...
                className={cn(
                  "relative flex flex-col items-center p-3 sm:p-5 rounded-xl border-2 transition-all",
                  "hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background",
                  isSelected ? TILE_TONE.card.on : TILE_TONE.card.off
                )}
                whileHover={{
                  y: -4,
                  transition: {
              ...
              >
                <m.div
                  className={cn(
                    ...
                    isSelected ? TILE_TONE.icon.on : TILE_TONE.icon.off
                  )}
                  initial={false}
                  animate={
                ...
                <span
                  className={cn(
                    "text-sm font-medium",
                    isSelected ? TILE_TONE.label.on : TILE_TONE.label.off
                  )}
                >
                  {type.label}
                </span>
      ...
      <AnimatePresence mode="wait">
        <m.div
          ...
          exit="exit"
        >
          <Card className="border-dashed overflow-hidden border-border-strong">
            <CardHeader className="pb-3 bg-linear-to-r from-primary/20 via-primary/10 to-transparent dark:from-primary/30 dark:via-primary/20">
            ...
            <CardContent className="p-5 space-y-6">
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <Label
                        htmlFor={field.id}
                        className={cn(
                          "text-sm font-medium flex items-center gap-1.5 text-foreground",
                    ...
                    <div
                      className={cn(
                        "relative border-2 border-dashed rounded-lg transition-all dark:bg-muted/50",
                        "hover:border-primary/50 focus-within:border-primary/70 dark:hover:border-primary/60 dark:focus-within:border-primary/80",
                        value?.[field.id]
                          ? "bg-muted/20 border-primary/30 shadow-sm dark:bg-muted/30 dark:border-primary/40"
                    ...
                    >
                      ...
                      >
                        {value?.[field.id] ? (
                          <div className="p-4 dark:bg-muted/30">
                            <div className="flex items-center gap-4">
                              <div className="relative w-20 h-20 rounded-lg overflow-hidden bg-muted flex-shrink-0 shadow-sm">
                                ...
                                {typeof value[field.id] === "string" ? (
                                  ...
                                  <>
                                    ...
                                    <div
                                      className={cn(
                                        "absolute inset-0 flex items-center justify-center bg-overlay/50 transition-opacity duration-200",
                                        previewVisible[field.id] !== false
                                    ...
                                    >
                                      <button
                                        type="button"
                                        className="text-overlay-foreground p-1"
                                        onClick={(e) => {
                                          e.preventDefault();
                                          e.stopPropagation();
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                uploadProgress[field.id] < 100 ? (
                                  ...
                                  <div className="flex items-center text-xs text-success gap-1 mb-2">
                                    <Check className="h-3 w-3" />
                                ...
                                <div className="flex gap-2 mt-2">
                                  ...
                                  <Button
                                    ...
                                    variant="ghost"
                                    size="sm"
                                    className="h-8 px-2 text-xs text-destructive-ink hover:bg-destructive/10 hover:text-destructive-ink"
                          ...
                          <div className="p-6 flex flex-col items-center gap-3 dark:bg-muted/20">
                            <div className="w-16 h-16 rounded-full bg-linear-to-br from-primary/20 to-primary/5 dark:from-primary/30 dark:to-primary/10 flex items-center justify-center shadow-sm">
                              {field.icon === "selfie" ? (
                            ...
                            <p className="text-sm font-medium text-primary/80 mb-1">
                              {field.icon === "selfie"
                                ? t("take_a_photo_or_upload_a_selfie")
              ...
              <div className="pt-3 mt-2 border-t border-dashed border-border-strong">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        ...
                        <Lock className="h-3.5 w-3.5 text-muted-foreground/70 text-subtle-foreground" />
                      </div>
                    </TooltipTrigger>
            ...
            <CardFooter className="bg-muted/30 px-5 py-3 flex justify-between">
