    ...
    switch (field.type) {
      ...
      case "PHONE":
        if (!/^\+\d{7,15}$/.test(value)) {
          return "Please enter a valid phone number in international format, e.g. +254711972926";
        }
        break;
      case "FILE":
      case "IMAGE":
    ...
    switch (field.type) {
            ...
            <Input
              ...
              onChange={(e) =>
                handleInputChange(
                  field.id,
                  field.type === "PHONE"
                    ? normalizePhoneE164(e.target.value)
                    : e.target.value
                )
              }
              onBlur={() => {
          ...
          >
            <div className="flex items-start space-x-2">
              ...
              <Label
                htmlFor={field.id}
                className={cn(
                  "text-sm font-medium leading-none peer-disabled:cursor-not-allowed text-foreground",
                  showError ? "text-destructive" : ""
                )}
              >
                {field.label}
            ...
            {field.description && (
              <p className="text-sm text-muted-foreground text-muted-foreground mt-1.5">
                {field.description}
              </p>
            )}
            <AnimatePresence>{renderErrorMessage()}</AnimatePresence>
      ...
      default:
        return (
          <div
            key={field.id}
            className="p-3 border rounded-md mb-4 bg-muted border-border-strong"
            data-field-id={field.id}
          >
