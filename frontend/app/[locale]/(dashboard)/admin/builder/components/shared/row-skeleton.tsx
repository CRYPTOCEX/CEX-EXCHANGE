...
export default function RowSkeleton({
  ...
  return (
    <div
      className={cn(
        "w-full bg-muted rounded-md overflow-hidden flex",
        className
      )}
    ...
    >
      {columns.map((width, index) => {
        ...
        return (
          ...
          >
            <div className="bg-muted h-full w-full rounded-sm border border-border-strong flex items-center justify-center shadow-sm">
              {showContent && (
                <div className="flex flex-col items-center justify-center space-y-1 p-1 w-full">
                  {/* Header line */}
                  <div className="w-3/4 h-1 bg-muted rounded-full"></div>

                  {/* Content lines */}
                  <div className="w-full h-1 bg-muted rounded-full"></div>
                  <div className="w-5/6 h-1 bg-muted rounded-full"></div>
                </div>
              )}
            </div>
          </div>
