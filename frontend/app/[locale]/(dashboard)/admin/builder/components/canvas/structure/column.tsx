...
export default function Column({
  ...
  // Only apply a min-height when the column is EMPTY — just enough for the
  // "Add Content" affordance to be clickable. Non-empty columns render at
  // their natural content height so edit mode matches preview.
  ...
  const renderElements = () => {
    ...
    for (let i = 0; i < column.elements.length; i++) {
      ...
      if (isInlineElement(element)) {
        ...
        // Wrap every inline-capable element in a flex container — even a
        // lone button — so column-level textAlign / justifyContent actually
        // positions it. Section-renderer does the same; without this, a
        // single button in a column always left-aligns regardless of
        // textAlign, diverging from preview.
        {
          // Mark all as processed
          inlineGroup.forEach((idx) => processedElements.add(idx));

          // Align the inline group exactly the way section-renderer does so
          // edit-mode layout matches preview/snapshot layout. Precedence:
          //   1. Explicit column-level justifyContent setting.
          //   2. Column's textAlign setting (center / right / left / justify).
          //   3. First inline element's textAlign.
          //   4. Default: flex-start (left).
          const firstEl = column.elements[inlineGroup[0]];
          const colSettings = (column.settings || {}) as Record<string, unknown>;
          const explicitJustify = colSettings.justifyContent as string | undefined;
          const alignmentSource =
            (colSettings.textAlign as string | undefined) ||
            (firstEl?.settings?.textAlign as string | undefined) ||
            "left";
          const justifyContent = (() => {
            if (explicitJustify) return explicitJustify;
          ...
          const firstElSettings = (firstEl?.settings || {}) as Record<string, unknown>;
          const inlineGroupStyle: React.CSSProperties = {
            ...
            gap: "16px",
            width: "100%",
            marginTop: firstElSettings.marginTop
              ? `${firstElSettings.marginTop}px`
              : undefined,
            marginBottom: firstElSettings.marginBottom
              ? `${firstElSettings.marginBottom}px`
              : undefined,
          };

          result.push(
            <div key={`inline-group-${i}`} style={inlineGroupStyle}>
              {inlineGroup.map((idx) => {
      ...
      // Apply element margins on the outer wrapper so edit-mode layout
      // matches preview (section-renderer applies margins on this wrapper).
      const elSettings = (element.settings || {}) as Record<string, any>;
      const elWrapperStyle: React.CSSProperties = {
        marginTop:
          elSettings.marginTop !== undefined
            ? `${elSettings.marginTop}px`
            : undefined,
        marginRight:
          elSettings.marginRight !== undefined
            ? `${elSettings.marginRight}px`
            : undefined,
        marginBottom:
          elSettings.marginBottom !== undefined
            ? `${elSettings.marginBottom}px`
            : undefined,
        marginLeft:
          elSettings.marginLeft !== undefined
            ? `${elSettings.marginLeft}px`
            : undefined,
      };

      result.push(
        <React.Fragment key={element.id}>
          {/* Margin lives on the drag-source wrapper to mirror preview:
              preview wraps each block element in a single margin div. */}
          <ElementDragSource
            id={element.id}
            sectionId={sectionId}
            rowId={rowId}
      ...
      >
        ...
        >
          <ResizableStructure
            ...
            color="green"
            label={`Column: ${formattedWidth}%${nestingLevel > 1 ? t("nested_level", { nestingLevel: String(nestingLevel) }) : ""}`}
            style={{
              ...getColumnStyles(),
          ...
          >
            {/* Children rendered directly inside the column container to
                match preview/section-renderer layout (no extra wrappers). */}
            {renderElements()}
