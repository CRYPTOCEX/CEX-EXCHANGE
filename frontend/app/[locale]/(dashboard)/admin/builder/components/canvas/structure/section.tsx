...
import { getSectionContainerClass } from "../../utils";
import { Save, Download } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { useTheme } from "next-themes";
import { useTranslations } from "next-intl";
import { gradientToCss } from "../../renderers/elements/color-utils";
...
export default function Section({
  ...
  const getBackgroundStyles = () => {
    ...
    // If the section background is a gradient with explicit hex/rgb stops,
    // emit it as an inline linear-gradient() because Tailwind classes like
    // `from-#6366f1` are not valid and will be ignored by Tailwind's JIT.
    const bg = section.settings?.backgroundColor;
    if (
      bg &&
      typeof bg === "object" &&
      "type" in bg &&
      ...
      const css = gradientToCss(bg.gradient, theme || "light");
      if (css) {
        styles.backgroundImage = css;
      }
    }

    // Handle background image (overrides gradient if both provided)
    ...
    if (section.settings?.backgroundColor) {
      if (typeof section.settings.backgroundColor === "object") {
        if (
          ...
          // Gradient: only emit Tailwind classes when stops use Tailwind
          // color names. Explicit hex/rgb/hsl stops are handled inline
          // via getBackgroundStyles() as a linear-gradient().
  ...
  const exportSection = (section: SectionType) => {
    ...
    toast({
      title: t("section_exported"),
      description: t("section_has_been_exported_as_json"),
    });
  };
    ...
    <SectionDragSource id={section.id} index={index} className="relative">
      <SectionDropTarget id={section.id} index={index} className="relative">
        <ResizableStructure
          ...
          color="orange"
          label={t("section", { type: String(section.type) })}
          style={{
            ...getBackgroundStyles(),
                ...
                <button
                  onClick={(e) => {
                    ...
                    toast({
                      ...
                      description: t("section_has_been_saved_to_your_library"),
                    });
                  }}
                  className={`p-1 text-warning-foreground hover:bg-warning`}
        ...
        >
          ...
          <div
            className={cn(
              ...
              getSectionContainerClass(section.type as any)
            )}
          >
