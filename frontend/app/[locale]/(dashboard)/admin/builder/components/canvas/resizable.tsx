...
const MeasurementLabel: React.FC<MeasurementLabelProps> = React.memo(
  ({ direction, value, isActive, isLinked, variant, toggleLink }) => {
    ...
    const valueColorClass =
      variant === "padding" ? "text-primary" : "text-success";
    ...
    return (
      <div
        className="absolute pointer-events-auto flex items-center gap-1 px-1 py-0.5 bg-card/90 dark:bg-surface-2/90 rounded-sm shadow-sm border border-border/50 dark:border-border-strong/50 backdrop-blur-sm z-50"
        style={labelPosition}
      >
        ...
        <button
          ...
          className={`w-3 h-3 flex items-center justify-center hover:bg-muted dark:hover:bg-muted rounded-sm transition-colors ${iconColorClass}`}
          title={`${isLinked ? t("unlink") : tCommon("link")} ${axis} ${variant}`}
        >
          {isLinked ? (
...
export function ResizableBase({
  ...
  const getPaddingHandleStyle = useCallback(
    (direction: "top" | "right" | "bottom" | "left"): React.CSSProperties => {
      ...
      const baseStyle: React.CSSProperties = {
        position: "absolute",
        backgroundColor: isActive ? "hsl(var(--primary) / 0.3)" : "transparent",
        border: isActive ? "1px solid hsl(var(--primary))" : "none",
  ...
  const getMarginHandleStyle = useCallback(
    (direction: "top" | "right" | "bottom" | "left"): React.CSSProperties => {
      ...
      const baseStyle: React.CSSProperties = {
        position: "absolute",
        backgroundColor: isActive ? "hsl(var(--success) / 0.2)" : "transparent",
        border: isActive ? "1px dashed hsl(var(--success))" : "none",
...
export function Resize({
  ...
  return (
    <div
      className={`absolute bg-transparent hover:bg-primary z-20 ${getResizePositionClasses(direction)} ${className || ""}`}
      style={style}
    ...
    <>
      {sides.map((side) => {
        ...
        return (
          <div
            ...
            className={`absolute cursor-${side === "top" || side === "bottom" ? "ns-resize" : "ew-resize"} z-10 pointer-events-auto transition-colors duration-150 ${isSideActive(side) ? "bg-primary/30" : "bg-transparent hover:bg-primary/20"}`}
      ...
      {sides.map((side) => {
        ...
        if (paddingValue !== undefined && paddingValue !== null)
          return null;
        const commonClasses =
          "absolute hover:bg-primary/20 hover:border border-primary cursor-" +
    ...
    <>
      {sides.map((side) => {
        ...
        return (
          <div
            ...
            className={`absolute cursor-${side === "top" || side === "bottom" ? "ns-resize" : "ew-resize"} z-10 pointer-events-auto transition-all duration-200 ${isSideActive(side) ? "bg-success/30" : "bg-transparent hover:bg-success/20"}`}
      ...
      {sides.map((side) => {
        ...
        if ((side === "left" || side === "right") && !shouldShowHorizontal)
          return null;
        const commonClasses =
          "absolute hover:bg-success/20 hover:border border-success/30 cursor-" +
          (side === "top" || side === "bottom" ? "ns-resize" : "ew-resize") +
          "z-10 pointer-events-auto";
        let style: React.CSSProperties = {};
        if (side === "top")
  ...
  if (element.type === "button") {
    return (
      <div
        ...
        className={cn(
          "relative",
          // Use negative outline-offset so the ring sits INSIDE the box
          // and does not push sibling layout.
          isSelected &&
            "outline outline-2 -outline-offset-2 outline-primary",
          hovered &&
            !isSelected &&
            "outline outline-1 -outline-offset-1 outline-dashed outline-border"
        )}
      >
        ...
        {(hovered || isSelected) && (
          <div
            className="absolute -top-2.5 right-1 z-30 flex items-center bg-primary text-primary-foreground text-xs rounded-md overflow-hidden"
        ...
        {(hovered || isSelected) && (
          <div
            className="absolute -top-2.5 left-1 z-30 bg-primary text-primary-foreground text-xs py-[2px] px-2 rounded-md"
            onClick={(e) => {
              e.stopPropagation();
          ...
          return (
            <div
              ...
              className={cn(
                ...
                // Use outline (doesn't affect layout) instead of border.
                // outline-offset is negative so the ring sits INSIDE the
                // content box and never pushes sibling layout.
                isSelected &&
                  "outline outline-2 -outline-offset-2 outline-primary pointer-events-auto",
                hovered &&
                  !isSelected &&
                  "outline outline-1 -outline-offset-1 outline-dashed outline-border pointer-events-auto",
            ...
            >
                ...
                {showTooltip && resizeDirection && (
                  <Tooltip open={true}>
                    <TooltipContent
                      side="top"
                      className="bg-surface-2 text-foreground px-3 py-2 rounded text-sm z-50 pointer-events-none shadow-md"
                      style={{
                        position: "fixed",
            ...
            >
              <div
                ...
                className={cn(
                  ...
                  type === "section" && !settings.backgroundColor && "bg-card",
                  type === "section" &&
                    !settings.backgroundColor &&
                    "",
                  // Use negative outline-offset so the selection/hover ring
                  // sits INSIDE the box and doesn't push sibling content.
                  isSelected &&
                    `outline outline-2 -outline-offset-2 ${colorClasses.outline}`,
                  hovered &&
                    !isSelected &&
                    "outline outline-1 -outline-offset-1 outline-dashed outline-border",
                  className
                ...
                {(hovered || isSelected) && (
                  <div
                    className={cn(
                      ...
                      colorClasses.bg,
                      "text-overlay-foreground text-xs py-[2px] px-2 rounded-md"
                    )}
                    onClick={(e) => {
                      e.stopPropagation();
                      onSelect(e);
                ...
                {(hovered || isSelected) && (
                  <div
                    className={cn(
                      ...
                      colorClasses.bg,
                      "text-overlay-foreground text-xs rounded-md overflow-hidden"
                    )}
                    onClick={(e) => e.stopPropagation()}
                  >
                    {extraControls}
