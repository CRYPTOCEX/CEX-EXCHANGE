 ...
 *
 * For anything that leaves the document's CSS scope. `html-to-image` either
 * sets the colour on a detached clone serialised into an SVG `<foreignObject>`,
 * or assigns it to `canvas.fillStyle`; neither has the document's custom
 * properties in scope, so `hsl(var(--background))` resolves to nothing there and
 * the capture comes out transparent. Read the token off <html> while it is still
 * in scope and hand over a real value.
 */
export function resolvedToken(name: string, fallback: string): string {
...
export function getDragPreviewStyle(type: string): React.CSSProperties {
  switch (type) {
    case "section":
      return {
        border: "2px dashed hsl(var(--chart-2))",
    ...
    case "row":
      return {
        border: "2px dashed hsl(var(--chart-1))",
    ...
    case "column":
      return {
        border: "2px dashed hsl(var(--chart-3))",
    ...
    case "element":
      return {
        border: "2px dashed hsl(var(--chart-4))",
