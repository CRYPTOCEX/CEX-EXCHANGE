...
import { gradientToCss } from "./elements/color-utils";
...
export default function SectionRenderer({
  ...
  const getBackgroundStyles = (): React.CSSProperties => {
    ...
    if (settings.backgroundColor) {
      if (isGradientColor(settings.backgroundColor)) {
        // Gradient: if any stop is an explicit CSS color (hex/rgb/hsl) we must
        // emit inline CSS because Tailwind will not recognize those class names.
  ...
  const renderRow = (row: Row): JSX.Element => {
    ...
    if (rowSettings.backgroundColor) {
      if (isGradientColor(rowSettings.backgroundColor)) {
        // Gradient: try inline CSS first for explicit hex/rgb stops.
        const inlineCss = gradientToCss(
          rowSettings.backgroundColor.gradient,
          theme || "light"
        );
        if (inlineCss) {
          rowStyles.backgroundImage = inlineCss;
        } else {
          rowClasses.push(
  ...
  const renderColumn = (
    ...
    if (columnSettings.backgroundColor) {
      if (isGradientColor(columnSettings.backgroundColor)) {
        // Gradient: try inline CSS first for explicit hex/rgb stops.
        const inlineCss = gradientToCss(
          columnSettings.backgroundColor.gradient,
          theme || "light"
        );
        if (inlineCss) {
          columnStyles.backgroundImage = inlineCss;
        } else {
          columnClasses.push(
        ...
        {column.elements
          ...
          .map((element, elemIndex) => {
            ...
            if (
              ...
              // Alignment precedence: explicit column-level justifyContent,
              // then column textAlign, then the first element's textAlign.
              // Keep this in sync with canvas/structure/column.tsx so edit
              // and preview render identically.
              ...
              const groupAlignment =
                ...
                inlineGroup[0]?.element.settings?.textAlign ||
                "left";
  ...
  if (!settings.backgroundColor) {
    sectionClasses.push("bg-card", "");
  }

  // Handle Tailwind background color classes
  if (settings.backgroundColor) {
    if (isGradientColor(settings.backgroundColor)) {
      // Gradient: only fall back to Tailwind classes when stops are Tailwind
      // color names. Explicit hex/rgb/hsl stops are handled inline via
      // getBackgroundStyles().
      const inlineCss = gradientToCss(
        settings.backgroundColor.gradient,
        theme || "light"
