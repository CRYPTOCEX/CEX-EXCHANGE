...
const renderHeading = (element: Element) => {
  ...
  const color = settings?.color || "hsl(var(--foreground))";
...
const renderButton = (element: Element) => {
  ...
  const backgroundColor = String(settings?.backgroundColor || "hsl(var(--primary))");
  const textColor = String(settings?.textColor || "hsl(var(--primary-foreground))");
...
const renderDivider = (element: Element) => {
  const { settings } = element;
  const color = settings?.color || "hsl(var(--border))";
...
const renderCard = (element: Element) => {
  ...
  const backgroundColor = settings?.backgroundColor || "hsl(var(--card))";
  const borderColor = settings?.borderColor || "hsl(var(--border))";
...
const renderFeature = (element: Element) => {
  ...
  const iconColor = settings?.iconColor || "hsl(var(--primary))";
  const alignment = settings?.alignment || "left";
...
const ElementRenderer = ({ element }: { element: Element }) => {
  ...
  switch (element.type) {
    ...
    default:
      return (
        <div className="p-4 border border-dashed border-border-strong rounded bg-muted text-center">
