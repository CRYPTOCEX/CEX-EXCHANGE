...
export const LineChart: React.FC<LineChartProps> = ({
  ...
  const [strokeColor, setStrokeColor] = useState<string>("hsl(var(--muted-foreground))");

  useEffect(() => {
    if (values.length < 2) {
      setStrokeColor("hsl(var(--muted-foreground))");
      return;
    }
    ...
    if (lastValue > prevValue) {
      setStrokeColor("hsl(var(--up))");
    } else if (lastValue < prevValue) {
      setStrokeColor("hsl(var(--down))");
    }
