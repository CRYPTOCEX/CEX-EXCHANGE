    ...
    return (
      <div className="flex items-center space-x-4 min-w-max">
        <Link
          ...
          className="flex items-center space-x-3 hover:bg-muted p-2 rounded-lg transition-colors group"
        >
          <div className="flex flex-col">
            <span className="font-medium text-sm group-hover:text-primary">
              {item.symbol}
            </span>
            <span className="text-xs text-subtle-foreground">{item.currency}</span>
          </div>
...
export const TrendingMarketsElement = memo<TrendingMarketsProps>(
  ({ element }) => {
    ...
    return (
      <div
        className="w-full bg-card border border-border-strong rounded-lg overflow-hidden"
