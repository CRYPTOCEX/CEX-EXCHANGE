...
interface ElementSettings {
  htmlId?: string;
  // Stored as a space-separated string per the canonical Element type, but we
  // tolerate the legacy string[] shape on read for in-flight data.
...
export function applyAdvancedSettings(
  ...
  // Apply HTML ID if set
  if (settings.htmlId) {
    attributes.id = settings.htmlId;
  }

  // Apply CSS classes if set — accept both the canonical space-separated
  // string form and the legacy array form.
  const cssClassTokens = Array.isArray(settings.cssClasses)
    ? settings.cssClasses
    : typeof settings.cssClasses === "string"
      ? settings.cssClasses.split(/\s+/)
      : [];
  if (cssClassTokens.length) {
    cssClassTokens.forEach((cls) => {
      if (cls?.trim()) {
