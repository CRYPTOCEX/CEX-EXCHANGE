...
export const HeadingElement = memo(
  ({
    ...
    return (
      ...
      >
        {isEditMode && onTextChange ? (
          <EditableContent
            content={element.content || "Heading"}
            isEditMode
            onTextChange={onTextChange}
          />
        ) : (
          <SafeHtml tag="div" html={element.content || "Heading"} />
        )}
      </HeadingTag>
    );
  }
    ...
    return (
      ...
      >
        {isEditMode && onTextChange ? (
          ...
          <SafeHtml tag="div" html={element.content || "Text content"} />
        )}
      </div>
    );
  }
...
export const ListElement = memo(
  ({
    ...
    const settings = element.settings || {};
    const listType = (settings.listType === "ol" ? "ol" : "ul") as "ul" | "ol";
    const textAlign = settings.textAlign || "left";
    ...
    // Render the <ul>/<ol> through SafeHtml so the sanitized <li> children are
    // direct descendants of the list tag (semantically correct + passes a11y).
    return (
      <SafeHtml
        tag={listType}
        html={
  ...
  ({
    ...
    return (
      ...
      >
        <blockquote
          className={cn(
            "quote-element outline-none italic border-l-4 border-border-strong pl-4",
            textAlignClass
          )}
          data-element-id={element.id}
          data-element-type="quote"
        >
          <SafeHtml tag="div" html={element.content || "Quote"} />
        </blockquote>
        <figcaption
          className={cn("mt-2 text-sm text-muted-foreground", textAlignClass)}
        >
          —
          <span className="font-medium">{author}</span>
          , <cite className="text-subtle-foreground">{source}</cite>
        </figcaption>
      </figure>
    );
  }
...
export const LinkElement = memo(
  ({
    ...
    return (
      ...
      >
        <SafeHtml tag="span" html={element.content || "Link"} />
      </a>
    );
  }
);
