...
import { resolvedToken } from "../utils";
        ...
        const generateSnapshotSafely = async (
          ...
          try {
            ...
            const enhancedOptions = {
              ...options,
              // Mirrors the capture wrapper below, which grounds on surface-2 in
              // dark and card in light. Resolved to a real value because the
              // clone is rendered outside the document, where `var(--x)` is not
              // in scope.
              backgroundColor: resolvedToken(
                theme === "dark" ? "surface-2" : "card",
                "#ffffff"
              ),
              filter: robustFilter,
  ...
  try {
    return (
      <div className="absolute left-[-9999px] top-[-9999px] overflow-hidden opacity-0 pointer-events-none">
        <div
          ref={sectionRef}
          className={`w-[1200px] ${theme === "dark" ? "dark bg-surface-2" : "bg-card"}`}
          data-theme={theme}
          data-snapshot="true"
          style={{
            fontFamily:
