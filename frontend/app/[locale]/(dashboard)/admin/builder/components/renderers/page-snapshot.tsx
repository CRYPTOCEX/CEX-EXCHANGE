...
export function PageSnapshot({
  ...
  // Only read by the effect's dependency array, and deliberately so: it is what
  // re-runs the capture after a theme flip, so `resolvedToken` re-reads the
  // tokens instead of baking the old theme into the snapshot.
    ...
    const generateSnapshot = async () => {
      ...
      try {
        ...
        const dataUrl = await toPng(pageRef.current, {
          backgroundColor: resolvedToken("background", "#ffffff"),
          width: 600,
          height: 400,
          style: {
            transform: "scale(0.3)",
