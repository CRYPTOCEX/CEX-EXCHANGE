...
const getComprehensiveClasses = (
  ...
  // Custom CSS classes — stored as a space-separated string per Element type,
  // but accept the legacy array form too so in-flight data doesn't break.
  if (settings.cssClasses) {
    const tokens = Array.isArray(settings.cssClasses)
      ? settings.cssClasses
      : String(settings.cssClasses).split(/\s+/);
    tokens.forEach((cls: string) => {
      if (cls?.trim()) {
...
export default function ElementRenderer({
   ...
   *
   * Debounced because contentEditable fires `input` on every keystroke, and each
   * `updateElement` pushes an entry onto the builder's undo history — typing a
   * sentence would otherwise cost a sentence's worth of undo steps.
   */
  const textCommitTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  useEffect(
    () => () => {
      if (textCommitTimer.current) clearTimeout(textCommitTimer.current);
    },
    []
  );
  ...
  const handleTextChange = useCallback(
    (e: React.FormEvent<HTMLDivElement>) => {
      // Read the HTML synchronously — the event target is not valid inside the
      // deferred callback.
      const html = e.currentTarget.innerHTML;
      if (textCommitTimer.current) clearTimeout(textCommitTimer.current);
      textCommitTimer.current = setTimeout(() => {
        updateElement(element.id, { ...element, content: html });
      }, 400);
    },
  ...
  const renderElement = useMemo(() => {
    ...
    if (!element || typeof element !== "object") {
      ...
      return (
        <div className="p-2 text-center text-destructive">{t("invalid_element")}</div>
      );
    }
        ...
        case "card":
          return (
            <CardElement
              element={element}
              renderChild={(child) => (
                <ElementRenderer element={child} isEditMode={isEditMode} />
              )}
            />
          );
        case "pricing":
