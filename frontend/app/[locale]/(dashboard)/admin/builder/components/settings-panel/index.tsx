  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-4 text-center">
      ...
      <Button
        ...
        className="bg-primary hover:bg-primary h-8 text-xs px-3 text-primary-foreground"
      >
...
function SettingsTabs({
  ...
  const isElement = selectedItemType === "element";

  // For structure types (section/row/column), omit the Content tab since
  // content editing is only meaningful for elements.
  const tabs = isElement
    ? [
  ...
  const defaultTab = isElement ? "content" : "design";

  return (
    <Tabs
      // Re-mount Tabs when switching between element and structure selections
      // so the internal activeTab state resets to the correct default.
      key={isElement ? "element" : "structure"}
...
export default function ElementSettingsPanel() {
  ...
  if (!selectedItemType) {
    return (
      <div className="w-80 border-l bg-card border-border flex flex-col h-full transition-all duration-300 ease-in-out">
  ...
  return (
    <div className="w-80 border-l bg-card border-border flex flex-col h-full transition-all duration-300 ease-in-out overflow-hidden">
      <PanelHeader
