...
export function Appearance({
  ...
  const handleBackgroundColorChange = (
    ...
    if (isGradient && gradientValue) {
      ...
      if (
        ...
        toast({
          title: t("gradient_limitation"),
          description: t("you_already_have_a_gradient_applied", { activeGradientProperty: String(activeGradientProperty) }),
          variant: "destructive",
        });
        return;
      }
  ...
  return (
    <div className="space-y-4 max-w-full">
      {hasGradient && (
        <div className="p-2 mb-2 bg-warning/10 border border-warning/30 rounded-md text-warning-ink text-xs leading-relaxed">
          <p>
            {t("a_gradient_is_currently_applied_to")}{" "}
            <strong>{gradientPropertyName}</strong>.{" "}
            {t("you_can_only_type")} (e.g. , text color and hover text color, background color and hover background color).
          </p>
        </div>
      )}
