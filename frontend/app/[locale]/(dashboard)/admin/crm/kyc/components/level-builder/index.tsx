...
export default function LevelBuilderComponent({
  ...
  if (error && isEdit) {
    return (
      <Alert variant="destructive">
        ...
        <div className="mt-4">
          <Button onClick={() => router.push("/admin/crm/kyc/level")}>
            {t("return_to_levels")}
          </Button>
        </div>
      </Alert>
  ...
  return (
      /*
        `isFullscreen ? "fixed inset-0 z-50 bg-background" : ""` used to hang off
        this class list. It was chrome suppression, not layout: the route was
        chromed, so the only way the builder could have the viewport was to paint
        over the `fixed top-0 z-50` admin header — which leaves that header
        mounted, live, and eating pointer events behind an invisible surface.

        Both routes are on the CHROMELESS list in
        `app/[locale]/(dashboard)/admin/layout.tsx` now, so there is nothing to
        cover and `h-screen w-full` already IS the viewport. `toggleFullscreen`
        keeps working and keeps meaning what it says — it calls
        `document.documentElement.requestFullscreen()`, which takes the BROWSER's
        own UI, something no class here could ever do.
      */
