...
export function FormPreview({
  ...
  return (
    <m.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="flex-1 overflow-auto p-4 bg-linear-to-br from-muted to-card"
    >
        <div className="flex items-center justify-between mb-6 sticky top-0 z-10 bg-card/80 dark:bg-surface-2/80 backdrop-blur-sm p-3 rounded-lg shadow-sm">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  <span>
                    {showFieldHighlights ? tCommon("hide_detail") : tCommon("show")}{" "}
                    {t("highlights")}
                  </span>
                </Button>
              </TooltipTrigger>
              <TooltipContent>
                {showFieldHighlights
                  ? t("hide_field_highlights")
                  : t("show_field_highlights")}
              </TooltipContent>
            </Tooltip>
          ...
          <AnimatePresence mode="wait">
            <m.div
              ...
              className={cn(
                "relative",
                viewMode === "mobile" &&
                  "border-8 border-border rounded-[32px] shadow-xl",
                viewMode === "tablet" &&
                  "border-[12px] border-border rounded-[24px] shadow-xl"
              )}
            >
              {(viewMode === "mobile" || viewMode === "tablet") && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-24 h-6 bg-muted rounded-b-lg z-10"></div>
              )}
              ...
              >
                <DynamicForm
                  fields={formFields}
                  title={levelName || t("verification_form")}
                  description={
                    levelDescription ||
                    t("please_complete_all_required_fields_below")
                  }
