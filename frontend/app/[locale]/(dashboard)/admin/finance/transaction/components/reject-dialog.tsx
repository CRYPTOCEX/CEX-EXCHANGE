    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="sm">
        ...
        <Textarea
          ...
          className="w-full p-2 border-border-strong rounded-md"
        />
        <DialogFooter>
          {/* A rejection reason is stored on the transaction AND emailed to the
              customer, so an empty one is not a valid rejection. */}
          <Button
            color="destructive"
            onClick={onReject}
            disabled={isLoading || rejectionMessage.trim().length < 3}
          >
