  ...
  const renderBarrierEditor = (orderType: BinaryOrderType) => {
    ...
    const addLevel = () => {
      const newLevel: BarrierLevel = {
        id: generateId(),
        label: `${isStrike ? tDashboardAdmin("strike") : tDashboardAdmin("barrier")} ${levels.length + 1}`,
        distancePercent: 0.5,
        profitPercent: 80,
        enabled: true,
      };
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{ backgroundColor: config.colorSoft }}
                >
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {isStrike ? tDashboardAdmin("strike_price_levels") : tDashboardAdmin("barrier_levels")} for{" "}
                    {config.label} orders
                  </p>
                </div>
              </div>
          ...
          <CollapsibleContent>
            <div className="px-4 pb-4 border-t">
                ...
                <div className="my-4 p-3 rounded-lg bg-muted/50">
                  ...
                  <div className="h-16 flex items-end gap-1">
                    {levels
                      ...
                      .map((level) => (
                          /* `level.id`, the same key the levels list below
                             uses. Without it React falls back to position, and
                             position is exactly what is unstable here: this
                             list is FILTERED by `enabled` and SORTED by
                             distance, so toggling a level or editing a
                             distance renumbers every bar after it. React then
                             reuses the wrong bar's DOM node and its open
                             tooltip shows another level's numbers. */
                      ...
                      <div className="flex-1 grid grid-cols-3 gap-3">
                        <div>
                          ...
                          <Input
                            ...
                            placeholder={tDashboard("level_name")}
                            className="h-8"
                          />
                        </div>
              ...
              <Button variant="outline" onClick={addLevel} className="w-full mt-3">
                ...
                Add {isStrike ? tDashboardAdmin("strike") : tDashboardAdmin("barrier")} Level
              </Button>
