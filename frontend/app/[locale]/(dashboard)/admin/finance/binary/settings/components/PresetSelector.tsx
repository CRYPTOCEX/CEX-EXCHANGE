...
/**
 * Risk posture, so these keep STATUS tokens rather than ramp slots — the
 * conservative/balanced/aggressive axis is exactly what success/primary/warning
 * mean, and the Shield/Scale/Flame icons say the same thing.
 *
 * `soft` is a real token alpha. The tint was `` `${color}20` ``, which only ever
 * worked while `color` was a six-digit hex.
 */
const PRESET_COLORS: Record<string, { color: string; soft: string }> = {
  conservative: { color: "hsl(var(--success))", soft: "hsl(var(--success) / 0.12)" },
  balanced: { color: "hsl(var(--primary))", soft: "hsl(var(--primary) / 0.12)" },
  aggressive: { color: "hsl(var(--warning))", soft: "hsl(var(--warning) / 0.12)" },
};

/** Presets come from the API, so an unknown id has to fall back to something. */
const presetColors = (id: string) => PRESET_COLORS[id] || PRESET_COLORS.balanced;
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {presets.map((preset) => {
          ...
          const { color, soft } = presetColors(preset.id);
              ...
              {selectedPresetData && (
                <>
                  {(() => {
                    ...
                    return (
                      <span style={{ color: presetColors(selectedPresetData.id).color }}>
                ...
                <div className="space-y-1">
                  {selectedPresetData.features.map((feature, index) => (
                    <div key={index} className="flex items-center gap-2 text-sm">
                      <Check
                        className="h-3 w-3"
                        style={{ color: presetColors(selectedPresetData.id).color }}
                      />
                      <span>{feature}</span>
                    </div>
                  ))}
            ...
            <Button
              ...
              style={{
                backgroundColor: selectedPresetData
                  ? presetColors(selectedPresetData.id).color
                  : undefined,
              }}
            >
              {t("apply_preset")}
