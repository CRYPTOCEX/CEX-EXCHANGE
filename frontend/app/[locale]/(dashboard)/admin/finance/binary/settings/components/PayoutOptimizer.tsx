...
export function PayoutOptimizer({ settings, onApplyOptimized }: PayoutOptimizerProps) {
  ...
  const getEdgeColor = (edge: number) => {
    if (edge < 5) return "text-destructive";
    if (edge > 12) return "text-warning";
    return "text-success";
  };

  const getEdgeBgColor = (edge: number) => {
    if (edge < 5) return "bg-destructive";
    if (edge > 12) return "bg-warning";
    return "bg-success";
  };
    ...
    <div className="space-y-6">
            ...
            <div className="p-4 rounded-lg bg-muted/50">
              ...
              <p className="text-xs text-muted-foreground mt-2">
                {analysis.overallEdge < 5
                  ? tDashboardAdmin("low_increase_payouts")
                  : analysis.overallEdge > 12
                  ? tDashboardAdmin("high_may_deter_traders")
                  : tDashboardAdmin("optimal_range")}
              </p>
            </div>
      ...
      <Card className="p-6 border-success/20 bg-success/5">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={handleApplyOptimized} className="bg-success hover:bg-success">
