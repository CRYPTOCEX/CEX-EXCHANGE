...
export default function WithdrawDetailClient() {
  ...
  const fetchTransaction = useCallback(async () => {
    ...
    try {
      ...
      setTransaction(data);
      // Coerce nullable / non-finite DB values to safe strings so the form
      // doesn't ship "null"/"undefined" back to the backend on submit.
      const safeAmount = Number(data.amount);
      const safeFee = Number(data.fee);
      setAmount(Number.isFinite(safeAmount) ? String(safeAmount) : "0");
      setFee(Number.isFinite(safeFee) ? String(safeFee) : "0");
  ...
  /**
   * WHICH ENDPOINT SETTLES A WITHDRAWAL, AND WHY IT DEPENDS ON THE WALLET TYPE.
   *
   * `PUT /api/admin/finance/withdraw/log/{id}` is a BOOKKEEPING route. Its
   * `handleWalletCompletion` is an empty function: it flips the status and books
   * the platform fee without sending anything anywhere. Using it to "approve" a
   * SPOT withdrawal marked the customer paid while no funds moved — every payout
   * had to be executed by hand outside the product.
   *
   * The route that actually pays is
   * `POST /api/admin/finance/wallet/{id}/withdraw/approve`: it atomically claims
   * PENDING -> PROCESSING (so a double-click cannot broadcast twice), calls
   * `exchange.withdraw()`, and auto-refunds the user if the exchange rejects.
   * It is SPOT-only — its `getWalletQuery` hard-filters `type: "SPOT"`.
   *
   *   SPOT  -> approve.post.ts   (real exchange payout)
   *   ECO   -> NEITHER. Ecosystem withdrawals are broadcast on-chain by
   *            WithdrawalQueue. Hand-completing one marks it paid without
   *            broadcasting, so approval is refused here with an explanation.
   *   FIAT  -> the bookkeeping PUT, which is correct: the operator really does
   *            pay by bank and then records it.
   *
   * Rejection always goes to `POST .../withdraw/reject`, for every wallet type:
   * it requires a reason, refunds SPOT via credit and ECO via ecoRefund, and —
   * unlike the PUT, which is PENDING-only — it accepts PROCESSING and TIMEOUT.
   * That is what un-freezes a withdrawal stranded mid-payout, which previously
   * had no recourse anywhere in the panel.
   */
  const walletType = transaction?.wallet?.type;
  ...
  const approveWithdrawal = async () => {
    ...
    if (walletType === "ECO") {
      toast({
        title: tCommon("error"),
        description:
          t("ecosystem_withdrawals_are_broadcast_on_chain"),
        variant: "destructive",
      });
      return;
    }

    // FIAT and any non-SPOT rail settle manually, so the bookkeeping PUT is the
    // honest record of an off-platform payment the operator has already made.
    if (walletType !== "SPOT") {
      await updateTransaction("COMPLETED");
      return;
    }

    setIsUpdating(true);
    try {
      const { error: approveError } = await $fetch({
        method: "POST",
        url: `/api/admin/finance/wallet/${transaction.id}/withdraw/approve`,
      });
  ...
  const rejectWithdrawal = async () => {
    ...
    setIsUpdating(true);
    try {
      const { error: rejectError } = await $fetch({
        method: "POST",
        url: `/api/admin/finance/wallet/${transaction.id}/withdraw/reject`,
        body: { message: rejectionMessage.trim() },
      });
  ...
  const updateTransaction = async (newStatus: string) => {
    ...
    if (!Number.isFinite(parsedAmount) || parsedAmount <= 0) {
      toast({
        title: tCommon("error"),
        description: tCommon("invalid_amount") || t("amount_must_be_a_positive_number"),
        variant: "destructive",
      });
      return;
    }
    if (!Number.isFinite(parsedFee) || parsedFee < 0) {
      toast({
        title: tCommon("error"),
        description: tCommon("invalid_fee") || t("fee_must_be_zero_or_a_positive_number"),
        variant: "destructive",
      });
      return;
    }
   ...
   * the DOCUMENT height changed as well: the scrollbar appeared and the page
   * jumped sideways too.
   *
   ...
   * `isLoading`. Approve and reject both refetch, and blanking a record the
   * operator is already looking at back to placeholders for that round trip
   * would be a second flash on top of the one they just caused. A refresh keeps
   * the last-known values on screen; only a page with nothing at all to show
   * gets placeholders.
   *
   * `loadFailed` is the OTHER state, and it is a CONCLUSION rather than a wait —
   * so it is a banner inside the page frame now, not a takeover of it. It covers
  ...
  // A withdrawal claimed into PROCESSING (or aged into TIMEOUT) that then failed
  // used to be frozen forever: the Manage tab showed "transaction locked" and the
  // only settle route accepted PENDING. `reject.post.ts` accepts all three states
  // and refunds correctly per wallet type, so these rows get a recovery lane —
  // reject-and-refund only, never approve, because approve.post.ts must own the
  // PENDING -> PROCESSING claim that prevents a double on-chain send.
  const isRecoverable =
    transaction?.status === "PROCESSING" || transaction?.status === "TIMEOUT";
    ...
    >
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-4">
                  ...
                  >
                    <div className="h-12 w-12 rounded-xl bg-destructive flex items-center justify-center shadow-lg">
                      <ArrowUpCircle className="h-6 w-6 text-destructive-foreground" />
                    </div>
                  </m.div>
                  ...
                  <div>
                    ...
                    {/* The `#` and the bullet are chrome — they are knowable
                        before the fetch and they render in both states. Only
                        the id fragment and the wallet summary wait, inside the
                        very elements that carry their typography. */}
      ...
      <div className="container mx-auto px-4 md:px-6 py-6">
        ...
        {/*
          ...
          entire route, which meant a failed fetch and a slow fetch produced two
          ...
          "there is no such withdrawal" is a CONCLUSION — it is only reachable
          once the request has finished — and pending is not.
        */}
        {loadFailed && (
            ...
            <div className="relative flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-sm">
                  <Loadable
                    loading={awaitingRecord}
                    placeholder={t("this_withdrawal_is_awaiting_review")}
                  >
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <CardContent className="px-6 pb-6 pt-4">
                ...
                <TabsContent value="details" className="mt-0 space-y-6">
                  ...
                  <div className="bg-destructive/20 rounded-xl p-6 border border-destructive/50">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div className="flex items-center gap-4">
                        <div className="grid h-14 w-14 shrink-0 place-items-center rounded-sm bg-destructive/15 text-destructive">
                        ...
                        <div>
                          ...
                          {/* The placeholder sits INSIDE the `text-2xl
                              ...
                              it follows a typography change on its own. The
                              minus sign is part of the figure, so it waits with
                              it rather than hanging there alone. */}
                  ...
                  {(metadata.bankName || metadata.accountNumber || metadata.address) && (
                    <div className="bg-card/70 dark:bg-muted/70 backdrop-blur-sm rounded-xl p-6 border border-border/60 dark:border-border-strong/60">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {metadata.bankName && (
                          <div className="flex items-center gap-3 p-3 bg-muted dark:bg-surface-2/50 rounded-lg">
                            <div className="h-8 w-8 rounded-sm bg-destructive/30 flex items-center justify-center">
                        ...
                        {metadata.accountNumber && (
                          <div className="flex items-center gap-3 p-3 bg-muted dark:bg-surface-2/50 rounded-lg">
                            <div className="h-8 w-8 rounded-sm bg-warning/30 flex items-center justify-center">
                              <CreditCard className="h-4 w-4 text-warning" />
                            </div>
                            <div>
                              <p className="text-xs font-medium text-muted-foreground">
                                {tCommon("account_number")}
                              </p>
                        ...
                        {metadata.address && (
                          <div className="flex items-center gap-3 p-3 bg-muted dark:bg-surface-2/50 rounded-lg md:col-span-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {/* Transaction Hash — genuinely optional data: an unbroadcast
                        withdrawal has none, so there is nothing to reserve. */}
                    {transaction?.trxId && (
                ...
                <TabsContent value="user" className="mt-0 space-y-6">
                  <div className="bg-destructive/20 rounded-xl p-6 border border-destructive/50">
                    <div className="flex items-center gap-4">
                      <Avatar className="h-16 w-16 border-2 border-border-strong shadow-lg">
                        ...
                        <AvatarFallback className="bg-destructive text-destructive-foreground text-xl font-medium">
                          {userInitials}
                        </AvatarFallback>
                      </Avatar>
                      <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-card/70 dark:bg-muted/70 backdrop-blur-sm rounded-lg p-4 border border-border/60 dark:border-border-strong/60">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-sm bg-destructive/30 flex items-center justify-center">
                    ...
                    <div className="bg-card/70 dark:bg-muted/70 backdrop-blur-sm rounded-lg p-4 border border-border/60 dark:border-border-strong/60">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-sm bg-warning/30 flex items-center justify-center">
                ...
                <TabsContent value="manage" className="mt-0 space-y-6">
                  {isRecoverable ? (
                    <div className="bg-warning/10 dark:bg-warning/20 rounded-xl p-6 border border-warning/30 dark:border-warning/50">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <p className="text-sm text-warning mt-1">
                              {t("this_withdrawal_was_claimed_for_payout")}
                            </p>
                          </div>
                          <Button
                            variant="destructive"
                    ...
                    /*
                      ...
                      shows, because it is the smallest of the three manage
                      states and the only one that is honest about not being
                      able to act. Its shell — tinted box, shield tile, padding —
                      is chrome and renders either way; the two sentences are
                      CONCLUSIONS about the withdrawal's status, so they wait.
                    ...
                    <>
                      ...
                      <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t border-border-strong">
                        <Button
                          ...
                          disabled={isUpdating || walletType === "ECO"}
                ...
                {/* Audit — who touched this withdrawal and why, plus the balance
