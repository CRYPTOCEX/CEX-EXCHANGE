...
export default function ProductsPage() {
  ...
  const getProductImage = (product: Product): string | null => {
    ...
    if (product.category === "exchange") {
      ...
      // Filename aliases where the CCXT id differs from the logo filename
      const filenameAlias: Record<string, string> = {
        gate: "gateio",
      };
      const fileBase = filenameAlias[baseName] || baseName;

      // Some exchanges have SVG versions
      const svgExchanges = ["binance", "binanceus", "kucoin", "kraken", "okx"];
      if (svgExchanges.includes(fileBase)) {
      ...
      // All other exchange logos are .jpg
      return `/img/exchanges/${fileBase}.jpg`;
    }
