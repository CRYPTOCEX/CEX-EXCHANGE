...
export function QueueManager({ onRefresh }: QueueManagerProps) {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      if (error) {
        toast.error(t("failed_to_fetch_queue_statistics"));
      } else {
        setStats(data.queue);
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Badge tone={statusTone(queueStatus)} appearance="soft">
                  {queueStatus === "healthy" ? t("healthy") : tCommon("degraded")}
                </Badge>
                <Button
                  size="sm"
                  variant="outline"
          ...
          <CardContent>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              <div className="p-4 rounded-lg border">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">{tCommon("failure_rate")}</span>
                  <Badge
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {t("based_on_completed_and_failed_notifications")}
                </p>
              </div>
              <div className="p-4 rounded-lg border">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">{t("queue_health")}</span>
                  <Badge tone={statusTone(queueStatus)} appearance="soft">
                    {queueStatus === "healthy" ? t("healthy") : tCommon("degraded")}
                  </Badge>
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  {queueStatus === "healthy"
                    ? t("queue_is_operating_normally")
                    : (stats?.waiting || 0) > 100
                      ? t("queue_backing_up_notifications_waiting")
                      : t("high_failure_rate_detected")}
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 p-4 rounded-lg border bg-muted/50">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium">{t("redis_backed_queue")}</p>
                  <p className="text-xs text-muted-foreground mt-1">
                    {t("notifications_are_persisted_to_redis_before")}
                  </p>
                </div>
              </div>
            </div>
          ...
          <CardContent>
            {queueItems.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">{t("queue_is_empty")}</p>
                <p className="text-xs mt-1">{t("notifications_will_appear_here_when_queued")}</p>
              </div>
            ) : (
                  ...
                  <TableBody>
                    {queueItems.map((item) => (
                      <TableRow key={item.id}>
                        <TableCell>
                          <Badge tone={statusTone(item.status)} appearance="soft">
