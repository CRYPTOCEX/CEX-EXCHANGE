...
interface TemplateSidebarProps {
   ...
   *
   * Needed because an empty `templates` array means two different things and
   * this sidebar used never to be shown in one of them — the manager above
   * replaced the entire viewport with a spinner instead. Now that the rail
   * renders from the first paint, "0 templates / 0 categories" and "No
   * templates found" would both be asserted as facts about a platform whose
   * template list has not arrived yet.
   */
  loading?: boolean;
}
...
export function TemplateSidebar({
   ...
   *
   * `groupedTemplates` is `{}` both while the fetch is in flight and when the
   * platform really has no templates, and the two want opposite treatments —
   * one is silent, the other is a message. Naming the distinction once here
   * keeps it out of the JSX, where it reads as "hide this while loading" and
   * would be indistinguishable from the defect that hides real content.
   */
  const showEmptyState = !loading && Object.keys(groupedTemplates).length === 0;
  ...
  return (
    <div className="flex h-full flex-col border-r bg-muted/20 overflow-hidden">
      ...
      <div className="border-b p-4 shrink-0">
        ...
        {/* The counts are the only unknown in this header — the back button,
            the heading, the search field and the separator dot are all
            knowable from the first paint. Placeholders sit INSIDE the spans so
            the row keeps its exact `text-xs` height and the list below it
            starts at the same y in both states. */}
      ...
      <ScrollArea className="flex-1 min-h-0">
        <div className="p-2 space-y-1">
          ...
          {/* "No templates found" is a CONCLUSION, and it can only be drawn
              once the list has arrived. Drawn during the fetch it is simply
              false — see `showEmptyState`. */}
          {showEmptyState && (
