...
export function DepositForm() {
  ...
  // Normalize chain names across exchanges (Binance uses ETH/BSC/TRX, KuCoin/OKX/MEXC/Bitget use ERC20/BEP20/TRC20, etc.)
  const normalizeChainName = (chain: string): string => {
    if (!chain) return '';
    const c = chain.toUpperCase().replace(/[ _-]/g, '');
  ...
  const getRequiredConfirmations = (chain: string): number => {
    ...
    return confirmationMap[normalizeChainName(chain)] || 12; // Default to 12 confirmations
  };
  ...
  const getBlockchainExplorerUrl = (chain: string, txHash: string): string => {
    ...
    return explorerMap[normalizeChainName(chain)] || `https://blockchair.com/search?q=${txHash}`;
  };
  ...
  const getEstimatedTime = (chain: string): string => {
    ...
    return timeMap[normalizeChainName(chain)] || '5-15 minutes';
  };
