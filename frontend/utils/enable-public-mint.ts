...
export async function enablePublicMint(contractAddress: string): Promise<{
  ...
  try {
    ...
    // togglePublicMint is onlyOwner. Check before sending so a non-owner gets a
    // clear message instead of an opaque on-chain revert (and pays no gas).
    try {
      ...
      if (owner.toLowerCase() !== me.toLowerCase()) {
        return {
          success: false,
          error:
            "Only the collection owner can enable public minting. Connect the wallet that deployed this contract.",
        };
      }
    } catch (error) {
      console.warn("[ENABLE PUBLIC MINT] Could not verify owner:", error);
    }
