 ...
 *
 * Mirrors the backend's `TRADE_STATUS_TRANSITIONS`, which is the authority:
 * - PENDING: NO (can only go to PAYMENT_SENT, CANCELLED, EXPIRED)
 * - PAYMENT_SENT: YES (can go to COMPLETED, DISPUTED)
 * - COMPLETED / CANCELLED / EXPIRED: NO — terminal, escrow is settled
 *
 * COMPLETED used to return true here, on the strength of a "within time limit"
 * rule that does not exist: `COMPLETED: []` in
 * `(ext)/p2p/utils/validation.ts` is terminal, and `trade/[id]/dispute.post.ts`
 * refuses the submission outright. The trade room therefore showed a live
 * "Something's wrong" button on a settled trade, let the customer pick a reason
 * and write a long description, and answered with a 400 — at the moment they
 * had just decided they had been defrauded.
 */
export function canDispute(status: string): boolean {
  return normalizeP2PStatus(status) === P2P_STATUS.PAYMENT_SENT;
}
