...
export const imageUploader = async ({
  ...
  /**
   * Never rejects. Callers branch on `success`, and several of them leave a
   * "Saving…" spinner running until this resolves — so a rejection here does
   * not surface as an error, it hangs the form. Reading the file and decoding
   * it both reject on their own (a directory picked as a file, an image the
   * browser cannot decode), outside the narrower try that used to wrap only
   * the network call.
   */
  try {
    return await uploadImage({ file, dir, size, oldPath });
  } catch (error: any) {
...
const uploadImage = async ({
  ...
  try {
    ...
    if (error) {
      // Keep the server's own words. "File upload failed" tells an operator
      // nothing, where "Request body too large (this endpoint accepts at most
      // 14 MB)" or "File type not allowed" tells them exactly what to change.
      // Coerced rather than passed straight in, so a non-string error cannot
      // reach the operator as "[object Object]".
      throw new Error(
        typeof error === "string" ? error : (error as any)?.message || "File upload failed"
      );
    }
