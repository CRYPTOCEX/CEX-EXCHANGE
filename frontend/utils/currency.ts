 ...
 *
 * NOTE: this is a FIAT-OR-CRYPTO heuristic, not a safety check. It answers
 * "how many decimals does this deserve", and the list is deliberately partial.
 * Do NOT use it to decide whether `Intl` will accept a code — that question has
 * a different and much simpler answer; see `isIntlCurrencyCode` below.
 */
...
/**
 * Will `Intl` accept this as `{ style: "currency", currency }`?
 *
 * The bar is WELL-FORMEDNESS, not existence: exactly three ASCII letters.
 * `Intl` has never heard of ETH either and formats it happily as `ETH 3.00`;
 * what it refuses is a code of any other length, and it refuses it by throwing
 * a `RangeError` ("Invalid currency code : USDT"). On a platform whose money
 * columns are free-text tickers — USDT, USDC, MATIC, AVAX — that throw lands
 * during RENDER and takes the page to the error boundary, which is how
 * /admin/nft/listing's analytics tab went blank on a card labelled "GMV (USDT)".
 */
export function isIntlCurrencyCode(currency: unknown): currency is string {
  return typeof currency === "string" && /^[A-Za-z]{3}$/.test(currency.trim());
}

/**
 * Money the way `Intl` would print it, for a code `Intl` may not accept.
 *
 * Use this ANYWHERE a currency code arrives from data — an order, a listing, a
 * payment intent, an analytics payload. The symbol path is unchanged for real
 * currencies (`$1,234.56`), and a ticker `Intl` rejects falls back to the shape
 * `Intl` itself uses for the codes it merely doesn't recognise: code, space,
 * grouped figure — so `USDT 5,000.50` sits beside `ETH 3.00` and reads as the
 * same kind of number.
 *
 * The five hand-rolled `validCurrencyCodes` arrays scattered through this
 * codebase are the OTHER way to avoid the throw, and they are worse: each lists
 * 20-50 codes, so a perfectly real AED or SAR amount silently takes the crypto
 * branch. Well-formedness is the actual rule, and it needs no list.
 */
export function formatMoney(
  ...
  if (!Number.isFinite(value)) return code ? `${code} 0.00` : "0.00";
  ...
  const figure = new Intl.NumberFormat(locale, numberOptions).format(value);
...
export function formatAmount(
  ...
  /* The currency branch above has refused a non-number since it was written;
     this one handed `amount` straight to `Intl` and printed the literal string
     "NaN" on the screen for an absent value. Two branches of one function
     disagreeing about what a missing amount looks like is how a defect ends up
     visible on one screen and invisible on the next. */
  const value = toNumber(amount);
  if (!Number.isFinite(value)) {
    return (0).toFixed(defaultDecimals);
  }
...
export function formatCurrencySafe(
  ...
  /*
    `toNumber`, not `isNaN(amount)`.

    The signature says `number` and the call sites do not honour it — every
    DECIMAL on this platform arrives from the API as a STRING, so `amount` is
    routinely one. `isNaN` coerces with `Number()`, which happens to be strict,
    but `Intl.format` then received the STRING and formatted it by its own
    coercion rules. Two different parsers deciding what the same value means is
    how a figure comes to be refused by one and accepted by the other.
    `toNumber` also refuses the shape `parseFloat` launders — see utils/numeric.ts.
  */
  const value = toNumber(amount);
  if (!Number.isFinite(value)) {
...
export function formatPnL(
  ...
  // `up`/`down` rather than `success`/`destructive`: this is direction of money,
  // which is exactly what those two tokens are reserved for. Identical pixels
  // today (`--up` aliases `--success`), but they diverge the day the operator
  // rebrands or the light-mode contrast fix lands.
  const colorClass = amount > 0 ? "text-up" : amount < 0 ? "text-down" : "text-muted-foreground";
...
export function formatPercentChange(
  ...
  const colorClass = percent > 0 ? "text-up" : percent < 0 ? "text-down" : "text-muted-foreground";
