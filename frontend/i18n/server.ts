...
/*
 * Side-effect import, and it must stay at the top.
 *
 * It installs the disk-backed chunk reader (see the header of
 * `loader-fs.server.ts`). Every server caller reaches the loaders through this
 * module or through the `next-intl/server` alias that re-exports it, so
 * importing it here is what guarantees the reader is in place before the first
 * `loadCoreTranslations()` runs. Without it the server has no reader at all and
 * silently falls back to the full 1.45 MB catalogue on every request.
 */
...
/**
 * The optimized loaders, re-exported HERE ON PURPOSE.
 *
 * Reaching them through this module is what guarantees the disk reader above is
 * installed first. Importing them straight from `@/i18n/loader` on the server
 * would compile and run, and would then quietly return the whole catalogue
 * because no reader would be registered.
 *
 * `loadLayoutMessages(locale)` is what `app/[locale]/layout.tsx` should call —
 * its doc comment in `loader.ts` spells out the call shape and why it takes no
 * pathname.
 */
export {
  loadLayoutMessages,
...
export type { MessagesWithMeta, I18nMessagesMeta } from "./types";
