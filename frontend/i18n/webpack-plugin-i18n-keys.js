  ...
  extractKeysFromLocale(namespaceKeys, localeData) {
    ...
    for (const [namespace, keys] of Object.entries(namespaceKeys)) {
      ...
      for (const key of keys) {
        ...
        // A dotted key may be stored FLAT rather than nested (see
        // getNestedValue in i18n/utils.ts). Walking alone missed those, so the
        // key was dropped from the chunk entirely and the page rendered the key
        // path. Copy it across in the shape it was authored in - re-nesting it
        // here would collide wherever a flat key is also a prefix of another
