...
import { useTranslationContext, requestNamespaceRescue } from "./context";
import {
  getNestedValue,
  hasNestedKey,
  humanizeKey,
...
export function useTranslations(
  ...
  return useMemo(() => {
    ...
    /*
     * `raw` still answers with the key on a miss, ON PURPOSE.
     *
     * It is the documented escape hatch for callers that want the stored value
     * or nothing, and some of them TEST for `value === key` to detect a miss.
     * Humanizing here would break that test silently. `t()` and `rich()` are
     * the paths that put text on screen, and those two humanize.
     */
    ...
    translator.rich = (
      ...
      /*
       * A miss here used to `return key`, which painted the raw key path — the
       * same defect `t()` had. It cannot be detected with `raw(key) === key`
       * either: a translation whose text happens to equal its key would read as
       * missing. Ask the messages directly, then fall back the way `t()` does.
       */
      if (!hasNestedKey(namespaceMessages, key)) {
