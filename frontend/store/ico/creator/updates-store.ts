...
export const useTokenUpdateStore = create<TokenUpdateStore>((set, get) => ({
  ...
  postUpdate: async (updateData) => {
    ...
    if (!error) {
      /*
       * The route answers `{ message, update }`, not the update itself. Pushing
       * `data` prepended the WHOLE envelope into a list of update records, so
       * the row the creator had just written rendered with an empty title and
       * empty body until the page was reloaded. Nothing reported it: $fetch
       * never throws and every missing field is simply `undefined`.
       */
      const created = (data as any)?.update;
      // Optimistically prepend the new update.
      set((state) => ({
        updates: created ? [created, ...state.updates] : state.updates,
        isSubmitting: false,
      }));
  ...
  editUpdate: async (updateData) => {
    ...
    if (!error) {
      /*
       * Same envelope as postUpdate: `{ message, update }`. `data.id` was
       * therefore always `undefined`, so `u.id === data.id` matched no row and
       * the edit never reached the list — the creator saw the OLD text after a
       * save that had reported success. Had it matched, it would have replaced
       * the record with the envelope, which is the other half of the same bug.
       */
      const edited = (data as any)?.update;
      set((state) => ({
        updates: edited
          ? state.updates.map((u) => (u.id === edited.id ? edited : u))
          : state.updates,
        isSubmitting: false,
      }));
      // Trigger notifications fetch
