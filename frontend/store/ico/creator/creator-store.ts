...
export const useCreatorStore = create<CreatorStore>((set, get) => ({
  ...
  fetchPerformance: async (range = "30d") => {
    ...
    // Performance keeps its OWN loading/error pair. It used to share
    // `isLoadingStats`/`statsError` with the aggregate stat cards, so one
    // failing chart request replaced four perfectly good stat cards with
    // "Error loading stats" — and switching the range put them all back into
    // skeletons.
    ...
    if (!error && data) {
      ...
      set({ performanceError: error, isLoadingPerformance: false });
    }
  },
