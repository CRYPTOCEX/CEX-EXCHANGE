...
interface Extension {
  ...
  /**
   * Blockchain rows only. The list endpoint spreads `toJSON()` for all three
   * product types, so a `chain` reaches the client on those rows and nothing
   * else — and it is the STABLE identity for a chain, which both
   * `storeArtForProduct` and `getProductShowcase` key on. It was already being
   * read off this object untyped by `productArt`.
   */
  chain?: string;
}
