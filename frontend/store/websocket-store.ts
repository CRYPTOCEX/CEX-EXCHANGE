...
export const useWebSocketStore = create<WebSocketState>()((set, get) => ({
  ...
  createConnection: async (
    ...
    wsManager.on("error", (error) => {
      // Only the first failure of an outage is news — the retries after it are
      // the same message on a timer — and a hidden tab losing its socket is not
      // something the operator can act on.
      const visible =
        typeof document === "undefined" ||
        document.visibilityState === "visible";
      if (visible && !error?.attempt) {
        console.warn(
          `WebSocket ${path} could not connect.${error?.willRetry === false ? " No further retries." : " Retrying."}`
        );
      }
