...
export interface AiInvestment {
  ...
  type: "SPOT" | "ECO";
  amount: number;
  /** Null until the investment settles — the outcome is withheld while ACTIVE. */
  profit?: number | null;
  /** Null until the investment settles — the outcome is withheld while ACTIVE. */
...
export interface AiInvestmentPlan {
  ...
  maxAmount: number;
  /**
   * NOTE: `defaultProfit` and `defaultResult` are intentionally absent. They
   * hold the operator's configured outcome for the plan and are never served
   * to users — an investment must read as undecided until it settles.
   */
  trending?: boolean;
  durations?: Array<{
    id: string;
