...
/*
  Kept in step with the chart engine's own `TimeFrame` (components/(ext)/
  chart-engine/types/index.ts), which is the union the toolbar offers. Every
  market type's backend accepts all thirteen - the spot GRID, the ecosystem
  `intervals`, forex's `FX_INTERVAL_MS` and `DEX_INTERVALS` - so a host that
  could only name eight of them was the narrower end of the pipe.

  `8h` is in the spot exchange's table and in none of the other three, and `1M`
  has no fixed millisecond width; neither is offered.
*/
...
export interface BinaryMarket {
  ...
  isTrending?: boolean;
  /**
   * Which price feed backs this market. ECOSYSTEM markets are priced by the ecosystem
   * (and by the AI Market Maker when one is running), so the chart must read that series
   * rather than the centralized exchange — otherwise the candles a user trades against
   * are not the ones their order settles on. Absent on installs predating the column,
   * which are all exchange-backed.
   */
      ...
      isLoading: false,
        ...
        fetchWalletData: async (currency, forceRefresh = false) => {
          try {
              ...
              if (cached) {
                try {
                  ...
                  if (now - timestamp < 30000 && cachedData?.balance !== undefined) { // 30 seconds cache
                    const cachedBalance = toNumber(cachedData.balance);
                    if (Number.isFinite(cachedBalance)) {
                      set({
                        realBalance: cachedBalance,
                        isLoadingWallet: false,
            ...
            if (!error && data?.balance !== undefined) {
              ...
              /*
                ───────────────────────────────────────────────────────────────
                COERCED HERE, AT THE ONE PLACE THE VALUE ENTERS.

                `wallet.balance` is `DECIMAL(36,18)` and `backend/src/db.ts` sets
                no `decimalNumbers`, so `/api/finance/wallet/SPOT/{cur}` returns
                it as the STRING "1000.500000000000000000".
                `finance/wallet/utils.ts` hands back the raw row, so nothing
                upstream coerces it either.

                This state is typed `realBalance: number | null`, and the type
                was simply wrong — which is why every downstream reader trusted
                it. `header.tsx` then computed

                    (realBalance ?? 0) + Σ pending stakes

                and on a string that CONCATENATES: "Total Equity" rendered
                `1000.500000000000000000250` where the true figure is 1250.50.
                The `.toLocaleString("en-US", {maximumFractionDigits: 2})` that
                follows is `String.prototype.toLocaleString`, which ignores its
                options entirely, so nothing rounded it back into shape.

                One coercion at the boundary rather than five at the render
                sites: the same value is read by the header balance, the account
                switcher, Total Equity and the risk calculator, and a fix at any
                one of them leaves the other three wrong.
                ───────────────────────────────────────────────────────────────
              */
              const freshBalance = toNumber(data.balance);
              set({
                realBalance: Number.isFinite(freshBalance) ? freshBalance : null,
                isLoadingWallet: false,
                // If in real mode, update the displayed balance
                ...(get().tradingMode === "real" && Number.isFinite(freshBalance)
          ...
          try {
            ...
            if (!error && data) {
              ...
              const activeOrders: Order[] = ordersArray.map((order: any) => {
                ...
                /*
                  ─────────────────────────────────────────────────────────────
                  THE CONTRACT'S OWN RATE, NOT A RE-DERIVED ONE.

                  `binaryOrder.profitPercentage` is the rate FROZEN when the
                  order was accepted (`BinaryOrderService.ts` computes it from
                  the barrier level, the side multiplier and the cumulative
                  duration adjustment, then persists it) and it is what
                  settlement pays. The route returns the whole row, so it is
                  already here.

                  This re-derived it instead, from a duration lookup keyed on
                  `round((closedAt - createdAt) / 60000)`. Expiries are
                  boundary-aligned, so that arithmetic routinely lands on a
                  minute count no configured duration has — and the fallback was
                  a hardcoded **85**, above the platform's own RISE_FALL default
                  of 72. An open position showed "+850.00" on a 1,000 stake that
                  settles at 720.00.

                  The lookup stays as a FALLBACK for a row that predates the
                  column, and the hardcoded 85 stays behind that, but neither is
                  reached while the contract can speak for itself.
                  ─────────────────────────────────────────────────────────────
                */
                const orderType: BinaryOrderType = order.type || 'RISE_FALL';
                const frozenRate = toNumber(order.profitPercentage);
                const duration = binaryDurations.find(d => d.duration === durationMinutes);
                const profitPercentage = Number.isFinite(frozenRate) && frozenRate > 0
                  ? frozenRate
                  : duration
                    ? getProfitPercentageForType(duration, orderType)
                    : 85; // Last resort for a row with neither
        ...
        cancelOrder: async (orderId: string) => {
          ...
          try {
            ...
            // Backend: DELETE /api/exchange/binary/order/:id (handler title "Cancel Binary Order")
            const isDemo = tradingMode === "demo";
            const { data, error } = await $fetch({
              url: `/api/exchange/binary/order/${orderId}?isDemo=${isDemo}`,
              method: "DELETE",
            });
        ...
        // TODO(CRITICAL): No backend handler exists for early close of binary orders.
        // The previous implementation posted to POST /api/exchange/binary/order/:id/close
        // which does not exist server-side. Until a backend endpoint is agreed on with
        // product, this function short-circuits with a user-facing error instead of
        // mutating local state based on a phantom network response.
        closeOrderEarly: async (orderId: string) => {
          const { orders } = get();

          // Find the order (kept so callers get consistent error messages)
          const order = orders.find(o => o.id === orderId);
          if (!order) {
            return { success: false, error: "Order not found" };
          }

          const message = "Early close is not yet available — please wait for expiry.";
          try {
            toast.error(message);
          } catch {
            // toast may not be available in non-browser contexts; ignore
          }
