...
export const useNftStore = create<NFTStore>((set, get) => ({
  ...
  fetchCategories: async () => {
    /**
     * `options`, not the browse list.
     *
     * This used to call `/api/nft/category` with no params. That endpoint is
     * paginated and defaults to ten per page, and the store kept `data.items`
     * while discarding `pagination` — so the Category selector on the
     * collection-create and mint forms silently showed the first TEN
     * categories and no way to reach the rest. `options` is unpaginated and
     * already excludes retired categories.
     */
    const { data, error } = await $fetch({
      url: "/api/nft/category/options",
      method: "GET",
      silent: true,
    });
    ...
    if (error) {
      ...
      // Kept tolerant of the paginated shape: this store is also hydrated from
      // server components that may still hand it a `{items}` envelope.
