...
export interface RelatedFAQSuggestion {
  id: string;
  question: string;
  category?: string;
}
...
export const useAIStore = create<AIState>((set) => ({
  ...
  findRelatedFAQs: async (question, answer, excludeId) => {
    set({ isFindingRelated: true, error: null });
    try {
      const { data, error } = await $fetch<{
        related: RelatedFAQSuggestion[];
      }>({
        url: "/api/admin/faq/ai/related",
        method: "POST",
        body: { question, answer, excludeId },
      });
      if (data && !error) {
        set({ isFindingRelated: false });
        // The route returns whole rows, not bare ids, so the caller can render
        // the suggestions without needing the admin FAQ list loaded.
        return data.related ?? [];
      } else {
        const errMsg = error || "Failed to find related FAQs";
        set({ isFindingRelated: false, error: errMsg });
        throw new Error(errMsg);
      }
    } catch (err) {
      set({
