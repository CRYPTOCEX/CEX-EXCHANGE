...
function transformFormDataToApiPayload(formData: P2POfferFormData): any {
  return {
    ...
    // Cap min/max so they don't exceed total in crypto terms
    // min/max are in fiat (e.g. USD), total is in crypto — max fiat = total * price
    amountConfig: (() => {
      const { total, min, max, ...rest } = formData.amountConfig;
      const finalPrice = formData.priceConfig?.finalPrice || formData.priceConfig?.value || 1;
      const maxAllowedFiat = finalPrice > 0 ? total * finalPrice : total;
      return {
        ...rest,
        total,
        min: min !== undefined && min > 0 ? Math.min(min, maxAllowedFiat) : min,
        max: max !== undefined && max > 0 ? Math.min(max, maxAllowedFiat) : max,
      };
    })(),
...
export const createOfferSlice = (
  ...
  fetchOffers: async (params?: { limit?: number; sort?: string }) => {
    try {
      ...
      // `sort` used to be appended as a PATH segment, so any value other than
      // the one route that happens to exist (`/offer/popularity`) fell through
      // to `/offer/[id]` and came back as "Offer not found". Only the sorts
      // that are genuinely their own endpoint may take the path form; anything
      // else goes to the list endpoint as a query parameter.
      const PATH_SORTS = ["popularity"];
      const sort = params?.sort;
      const usePathSort = !!sort && PATH_SORTS.includes(sort);

      const { data, error } = await $fetch({
        url: `/api/p2p/offer${usePathSort ? `/${sort}` : ""}`,
        params: {
          limit: params?.limit || 10,
          ...(sort && !usePathSort ? { sort } : {}),
        },
        silentSuccess: true,
      });
      ...
      if (error) {
        set({
          offersError: errorMessage(error, "Failed to fetch offers"),
          isLoadingOffers: false,
        });
        return;
      }
  ...
  fetchMarketStats: async () => {
    try {
      ...
      if (error) {
        set({
          marketStatsError: errorMessage(error, "Failed to fetch market stats"),
          isLoadingMarketStats: false,
        });
        return;
      }
  ...
  fetchAvailablePaymentMethods: async () => {
    try {
      ...
      if (error) {
        set({
          availablePaymentMethodsError: errorMessage(
            error,
  ...
  fetchOfferById: async (id: string) => {
    try {
      ...
      const { data, error } = await $fetch({
        ...
        silentSuccess: true,
        // The one caller — `offer/[id]/edit/client.tsx` — reads
        // `offerByIdError` straight back off the store and renders it in
        // `EditOfferErrorState`. The toast was the same sentence a second time.
        silent: true,
      });

      if (error) {
        set({
          offerByIdError: errorMessage(error, "Failed to fetch offer details"),
          isLoadingOfferById: false,
        });
        return null;
      }
  ...
  submitOffer: async () => {
    try {
      ...
      if (error) {
        set({ submitOfferError: errorMessage(error, "Failed to create offer") });
        return false;
      }
