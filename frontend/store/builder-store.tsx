...
/**
 * Appends a new history entry to the existing history array,
 * truncating from the front if we exceed the cap.
 */
  ...
  devtools(
    (set, get) => ({
      ...
      addElement: (element: Element) => {
        const { page, history, selectedColumnId } = get();
        ...
        // history: patch-delta (prev -> next) for proper undo
        const historyEntry = createHistoryEntryFromChange(
          page,
          newPage,
          "Add element"
        );
        const newHistory = pushHistory(history, historyEntry);
      ...
      selectElement: (
        ...
        set({
          ...
          selectedSectionId: containerInfo?.sectionId ?? null,
          selectedRowId: containerInfo?.rowId ?? null,
          selectedColumnId: containerInfo?.columnId ?? null,
      ...
      addSection: (section: Section) => {
        ...
        set({
          ...
          canUndo: newHistory.length > 0,
          canRedo: false,
          // Leave selection untouched — user clicks to select.
        });
      },
      ...
      addRow: (sectionId: string, row: Row, parentColumnId?: string) => {
        ...
        const newPage = { ...page, sections: updatedSections };
      ...
      selectRow: (
        ...
        set({
          ...
          selectedSectionId: sectionId,
          selectedRowId: rowId,
          selectedColumnId: parentColumnId ?? null,
      ...
      selectColumn: (
        ...
        set({
          selectedElementId: null,
          selectedSectionId: sectionId,
          selectedRowId: parentRowId ?? rowId,
      ...
      getSelectedColumn: () => {
        ...
        const sections = Array.isArray(page.sections) ? page.sections : [];
        // Registry lookups yield undefined on a miss; the BuilderState
        // contract is null.
        return findColumnById(sections, selectedColumnId) ?? null;
      },
