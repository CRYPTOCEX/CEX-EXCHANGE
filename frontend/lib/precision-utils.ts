...
/**
 * Coerce a possibly-string/undefined/null numeric value to a real number.
 *
 * Market/ticker/order data reaches the UI through WebSocket JSON and Sequelize
 * DECIMAL columns, both of which can deliver numeric fields as STRINGS (e.g.
 * "50000.00"). Calling `.toFixed()` on a string throws
 * "X.toFixed is not a function" and crashes whole trade panels. Route values
 * through this helper before any numeric formatting so a string is coerced
 * rather than exploding, and a non-numeric value falls back to `fallback`.
 */
...
/**
 * Number.toFixed that never throws on string/undefined/null inputs.
 */
export function safeToFixed(value: unknown, digits = 2, fallback = 0): string {
  return toNum(value, fallback).toFixed(digits);
}
