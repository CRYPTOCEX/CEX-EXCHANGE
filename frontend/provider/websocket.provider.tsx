...
import { useAnnouncementsStore } from "@/store/announcement-store";
...
export const WebSocketProvider = ({
  ...
  useEffect(() => {
    ...
    const messageHandler = (msg: any) => {
      ...
      /*
        System announcements, on the SAME socket and the same SUBSCRIBE above.

        `api/user/index.ws.ts` answers that frame with every active
        announcement, and the admin routes broadcast each later change to the
        whole `/api/user` route — neither needs a subscription key. This
        handler simply never looked at them, so the operator's notice reached
        the browser and stopped there.

        `getState()` rather than a ref: a zustand action has a stable identity
        for the life of the store, so there is nothing to keep in sync and
        nothing that could re-run this effect and rebuild the socket.

        No sound. A notification is addressed to one person and is news; an
        announcement is a wall notice that re-arrives on every reconnect.
      */
      if (msg.type === "announcements") {
        useAnnouncementsStore.getState().handleAnnouncementMessage({
          method: msg.method,
          payload: msg.payload,
        });
      }
